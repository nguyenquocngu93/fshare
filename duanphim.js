(function () {
    ...
    /* ════════════════════════════════════════════════════════════
       KKPHIM/OPHIM (giữ nguyên code cũ - đã rút gọn ở đây)
    ════════════════════════════════════════════════════════════ */
    ...
    function getBaseName(name) {
        if (!name) return '';
        return name.replace(/[\s\-]*[\(\[]?\s*[Ss]eason\s*\d+\s*[\)\]]?/gi, '').replace(/[\s\-]*[\(\[]?\s*[Pp]h[aầ]n\s*\d+\s*[\)\]]?/gi, '').replace(/[\s\-]*[\(\[]?\s*[Mm][uù]a\s*\d+\s*[\)\]]?/gi, '').replace(/[\s\-]*\bS\d+\b/g, '').trim();
    }

    function searchSource(source, keyword, cb) {
        reguest(source.api + 'v1/api/tim-kiem?keyword=' + encodeURIComponent(keyword) + '&limit=30',
            function (data) {
                var items = [];
        ...
        reguest(source.api + 'v1/api/phim/' + slug,
            function (data) {
                if (data && data.status === 'success' && data.data) {
                    ...
                    reguest(source.api + 'phim/' + slug, function (data2) {
                        cb({ movie: data2.movie || data2.item || data2 || {}, episodes: data2.episodes || [] });
    ...
    function findAllSeasons(source, keyword, title, orig, year, cb) {
        var baseKeyword = getBaseName(keyword), baseTitle = getBaseName(title), baseOrig = getBaseName(orig);
        var terms = [];
        ...
        function doSearch() {
            ...
            var term = terms[searchIdx]; searchIdx++;
        ...
        function processResults(results) {
            ...
            if (!bestGroup || bestScore < 10) {
                ...
                cb({
                    ...
                    seasons: [{ season_num: 1, slug: first.slug, name: first.name || '', origin_name: first.origin_name || '', year: first.year || '', type: first.type || '' }]
                });
                return;
            }
    ...
    function getAnimeRomaji(card, callback) {
        ...
        if (romajiCache[cacheKey]) { callback(romajiCache[cacheKey]); return; }
        var type = getMediaType(card), endpoint = type === 'series' ? 'tv' : 'movie';
        reguest('https://api.themoviedb.org/3/' + endpoint + '/' + card.id + '?api_key=' + TMDB_API_KEY + '&append_to_response=alternative_titles,translations',
            function (data) {
    ...
    function extractRomaji(tmdbData) {
        if (tmdbData.alternative_titles) {
            ...
            for (var i = 0; i < titles.length; i++) {
                var t = titles[i];
                if (t.iso_3166_1 === 'JP' && t.title && isLatin(t.title)) return t.title;
            }
        }
        if (tmdbData.translations && tmdbData.translations.translations) {
            ...
            for (var i = 0; i < trans.length; i++) {
                if (trans[i].iso_639_1 === 'ja' && trans[i].data) {
                    ...
                    if (jaTitle && isLatin(jaTitle)) return jaTitle;
                }
            }
        }
        var orig = tmdbData.original_title || tmdbData.original_name || '';
        if (orig && isLatin(orig) && isAnime(tmdbData)) return orig;
        return null;
    }
    ...
    function showSeasonMenu(source, result, card) {
        ...
        Lampa.Select.show({
            ...
            onSelect: function (item) { Lampa.Noty.show('Đang tải ' + item.season.name + '...'); loadSeasonEpisodes(source, item.season, card, movieName); },
            onBack: function () { Lampa.Controller.toggle('full'); }
        });
    }
    ...
    function searchJackett(card) {
        ...
        if (!url || !key) { Lampa.Noty.show('❌ Chưa cấu hình Jackett!'); return; }
        ...
        getAnimeRomaji(card, function (romaji) {
            ...
            if (romaji) {
                ...
                console.log('[Jackett] 🎌 Romaji: "' + trimmedRomaji + '"');
            }
            if (title) {
            ...
            if (orig && orig !== title && orig !== romaji) {
                searchTerms.push(trimLast2Chars(orig) + (year ? ' ' + year : ''));
            }
            ...
            searchJackettMultipleTerms(searchTerms, 0, [], card, function (allResults) {
                if (!allResults || !allResults.length) { Lampa.Noty.show('❌ Không tìm thấy'); return; }
    ...
    function searchJackettMultipleTerms(terms, index, accumulated, card, callback) {
        if (index >= terms.length) { callback(accumulated); return; }
        var term = terms[index];
        fetchJackettResultsByQuery(term, function (results) {
            results.forEach(function (r) {
                var exists = accumulated.some(function (a) { return a.hash === r.hash || a.title === r.title; });
                if (!exists) accumulated.push(r);
            });
            if (accumulated.length >= 15) callback(accumulated);
            else searchJackettMultipleTerms(terms, index + 1, accumulated, card, callback);
        });
    }

    function fetchJackettResultsByQuery(query, callback) {
        var url = getJackettUrl(), key = getJackettKey();
        reguest(url + '/api/v2.0/indexers/all/results?apikey=' + encodeURIComponent(key) + '&Query=' + encodeURIComponent(query) + '&Category[]=2000&Category[]=5000&Category[]=5070',
            function (data) {
                var d = typeof data === 'string' ? JSON.parse(data) : data;
                if (!d || !d.Results || !Array.isArray(d.Results)) { callback([]); return; }
                var results = d.Results.map(function (r) {
                    ...
                    return { title: r.Title || 'Unknown', quality: quality, info: (seeds ? '👤 ' + seeds + '  ' : '') + '💾 ' + fmtBytes(size), size: size, seeds: seeds, hash: hash, magnet: link, tracker: r.Tracker || 'Jackett' };
                }).filter(Boolean);
    ...
    function showPackMenu(results, movieTitle, label, card) {
        ...
        Lampa.Select.show({
            ...
            items: results.map(function (r) {
                return { title: '[' + (r.tracker || label) + ']' + (r.quality ? ' ' + r.quality : '') + ' — ' + r.title, subtitle: r.info || '', r: r };
            }),
            onSelect: function (item) {
                var r = item.r;
    ...
    function searchTorrent(card, season, episode) {
        ...
        if (imdbId) {
            ...
            reguest('https://api.themoviedb.org/3/' + (type === 'series' ? 'tv' : 'movie') + '/' + card.id + '/external_ids?api_key=' + TMDB_API_KEY,
                function (d) {
                    var id = d && d.imdb_id;
    ...
    function showStreamsMenu(results, movieTitle, card) {
        ...
        Lampa.Select.show({
            ...
            items: results.map(function (r) {
                ...
                return { title: qualityBadge + r.title, subtitle: r.info || '', r: r };
            }),
            onSelect: function (item) {
                var r = item.r;
    ...
    function fetchTorrentioResults(card, season, episode, callback) {
        ...
        function run(id) {
            ...
            if (sType === 'series' && season && episode) {
                idWithSeason = id + ':' + season + ':' + episode;
            }
            ...
            if (engine === 'aio') {
                ...
                // 🔥 FIX: URL format với/không có season:episode
                var url = base + '/stream/' + sType + '/' + idWithSeason + '.json';
                
                console.log('[AIO] 🔍 Request URL:', url);
                console.log('[AIO] 📋 Type:', sType, '| ID:', id, '| Full:', idWithSeason);
                
                reguest(url,
                    function (data) {
                        console.log('[AIO] ✅ Response received');
                        console.log('[AIO] 📊 Response type:', typeof data);
                        
                        // Debug full response
                        if (typeof data === 'object') {
                            console.log('[AIO] 🗂️ Keys:', Object.keys(data));
                            console.log('[AIO] 📦 Data sample:', JSON.stringify(data).substring(0, 500));
                        }
                        ...
                        if (!streams.length) {
                            console.warn('[AIO] ❌ No streams extracted');
                            console.log('[AIO] 🔄 Fallback to Torrentio...');
                        ...
                        console.log('[AIO] ✅ Parsed results:', parsed.length);
                        
                        if (!parsed.length) {
                            console.error('[AIO] ❌ Parse failed, fallback');
                            fetchFromTorrentio(sType, idWithSeason, callback);
                            return;
                        }
                    ...
                    function (err) {
                        console.error('[AIO] ❌ Request error:', err);
                        Lampa.Noty.show('⚠️ AIO lỗi: ' + err + ', chuyển Torrentio...');
                        fetchFromTorrentio(sType, idWithSeason, callback);
                    }
                );
            } else {
    ...
    function parseStreams(streams, source) {
        if (!Array.isArray(streams)) { console.error('[Parse] ⚠️ Not array'); return []; }
        ...
        for (var i = 0; i < streams.length; i++) {
            ...
            var seedM = info.match(/👤\s*(\d+)|Seeds?:\s*(\d+)/i);
            var seeds = seedM ? parseInt(seedM[1] || seedM[2]) : 0;
            ...
            if (!hash && st.magnetUri) {
                var magMatch = st.magnetUri.match(/btih:([a-f0-9]{40})/i);
                if (magMatch) hash = magMatch[1];
            }
            hash = (hash || '').toLowerCase().replace(/[^a-f0-9]/g, '');
            if (!hash || hash.length !== 40) {
                console.warn('[Parse] ⚠️ No hash at', i);
                continue;
            }
            parsed.push({
                title: name, quality: quality,
                info: (seeds ? '👤 ' + seeds + '  ' : '') + (sz ? '💾 ' + sz : ''),
                size: parseSize(sz), seeds: seeds, hash: hash,
                fileIdx: st.fileIdx || st.file_idx || 0, tracker: tracker
            });
        }
        console.log('[Parse] ✅ Parsed', parsed.length, '/', streams.length);
        if (parsed.length === 0 && streams.length > 0) {
            console.error('[Parse] ❌ CRITICAL: 0 from', streams.length);
    ...
    function testAioConnection() {
        var base = getAioUrl();
        if (!base) { Lampa.Noty.show('❌ Chưa cấu hình AIO!'); return; }
        console.log('═══════════════════════════════════════');
    ...
    Lampa.Listener.follow('full', function (e) {
        ...
        if (!card) return;
        var $ctx = e.object && e.object.activity ? e.object.activity.render() : (e.object && e.object.render ? e.object.render() : $('body'));
        if ($ctx.find('.view--kkphim').length) return;
        var isSeries = getMediaType(card) === 'series';
        function mkBtn(cls, inner, label, fn) {
            var $b = $('<div class="full-start__button selector ' + cls + '"><svg viewBox="0 0 24 24" fill="none" width="44" height="44" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">' + inner + '</svg><span>' + label + '</span></div>');
            $b.on('hover:enter', fn);
            return $b;
        }
        var $kk = mkBtn('view--kkphim', '<rect x="2" y="2" width="20" height="20" rx="3"/><path d="M9.5 8.5L16 12L9.5 15.5V8.5Z" fill="currentColor" stroke="none"/>', 'KKPhim', function () { searchAndPlay('kkphim', card); });
        var $op = mkBtn('view--ophim', '<circle cx="12" cy="12" r="10"/><path d="M9.5 8.5L16 12L9.5 15.5V8.5Z" fill="currentColor" stroke="none"/>', 'OPhim', function () { searchAndPlay('ophim', card); });
        var engineLabel = getTorrentEngine() === 'aio' ? 'AIO' : 'Torrentio';
        var $tr = mkBtn('view--kkparser-torrent', '<circle cx="12" cy="12" r="10"/><polyline points="8 12 12 16 16 12"/><line x1="12" y1="8" x2="12" y2="16"/>', engineLabel, function () { if (isSeries) askTorrentTV(card); else searchTorrent(card, null, null); });
        var $jk = mkBtn('view--kkparser-jackett', '<circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>', 'Jackett', function () { searchJackett(card); });
    ...
    function buildSettings() {
        ...
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'h1', type: 'title', default: '' }, field: { name: '════ TORRSERVER ════' } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'torrserver_url', type: 'input', values: '', default: '' }, field: { name: '📡 TorrServer URL', description: 'VD: 192.168.1.100:8090' }, onChange: function (v) { setSetting('torrserver_url', v); } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'torrserver_pass', type: 'input', values: '', default: '' }, field: { name: '🔑 Password' }, onChange: function (v) { setSetting('torrserver_pass', v); } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'test_ts', type: 'button', default: '' }, field: { name: '⚡ Test Speed' }, onChange: testTorrServerSpeed });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'h2', type: 'title', default: '' }, field: { name: '════ TORRENT ENGINE ════' } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'torrent_engine', type: 'select', values: { 'torrentio': 'Torrentio', 'aio': 'AIOStreams' }, default: 'torrentio' }, field: { name: '🔧 Engine' }, onChange: function (v) { setSetting('torrent_engine', v); } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'torrentio_config', type: 'input', values: '', default: '' }, field: { name: '⚙️ Torrentio Config' }, onChange: function (v) { setSetting('torrentio_config', v); } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'aio_url', type: 'input', values: '', default: '' }, field: { name: '🔗 AIOStreams URL' }, onChange: function (v) { setSetting('aio_url', v); } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'test_aio', type: 'button', default: '' }, field: { name: '🧪 Test AIO' }, onChange: testAioConnection });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'h3', type: 'title', default: '' }, field: { name: '════ JACKETT ════' } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'jackett_url', type: 'input', values: '', default: '' }, field: { name: '🔍 Jackett URL' }, onChange: function (v) { setSetting('jackett_url', v); } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'jackett_key', type: 'input', values: '', default: '' }, field: { name: '🔑 API Key' }, onChange: function (v) { setSetting('jackett_key', v); } });
        Lampa.SettingsApi.addParam({ component: 'kkparser', param: { name: STG_PREFIX + 'ver', type: 'static', default: '' }, field: { name: '📦 Version 3.8.0', description: '🔧 AIO Fallback Debug' } });
    }

    function start() {
        initSettings();
        console.log('[KKPhim Parser] v3.8.0 — 🔧 AIO Fallback Fixed ✅');
        console.log('[KKPhim Parser] 💡 Nếu AIO vẫn fallback, xem Console logs để debug');
    }
