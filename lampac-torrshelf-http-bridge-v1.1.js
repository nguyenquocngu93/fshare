/**
 * TorrShelf HTTP Bridge v1.1 for Lampa Online.
 * Direct HTTP sources only: UHDMovies, 4KHDHub, MoviesDrive, HDHub4u.
 * No Stremio addon, magnet, Jackett, or TorrServer is used here.
 */
(function () {
  'use strict';
  if (window.__torrshelf_http_bridge_v1_1) return;
  window.__torrshelf_http_bridge_v1_1 = true;
  ...
  function addButton(card) {
    if (!card || !cardParams(card).title || $('.' + BUTTON_CLASS).length) return;
  ...
  function start() {
    ...
    addSettings();
    console.log('[TorrShelf HTTP Bridge v1.1] ready');
  }
