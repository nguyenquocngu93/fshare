...
(function () {
  ...
  function startPlugin() {
    ...
    function openKKPhim(e) {
      if (e) { e.preventDefault(); e.stopPropagation(); }
      console.log('[KKPhim Parser] menu item triggered');
      try {
        if (!window.Lampa || !Lampa.Activity || typeof Lampa.Activity.push !== 'function') {
          console.error('[KKPhim Parser] Lampa.Activity chưa sẵn sàng');
          return;
        }
        Lampa.Activity.push({
          title: 'KKPhim',
        ...
        console.error('[KKPhim Parser] Lỗi khi push Activity:', err);
        if (Lampa.Noty) Lampa.Noty.show('KKPhim: lỗi mở danh mục — xem console');
      }
    }

    // Bind cả hover:enter (remote/TV) lẫn click native (chuột/cảm ứng trên trình duyệt),
    // vì hover:enter chỉ fire khi phần tử nằm trong focus context của Lampa.Controller.
    menuItem.on('hover:enter', openKKPhim);
    menuItem.on('click', openKKPhim);
    ...
    console.log('[KKPhim Parser] v4.0.1 OK — Fork LNUM pattern (+ click fallback)');
  }
