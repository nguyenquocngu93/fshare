...
function gEp1(eps){
    for(var i=0;i<(eps||[]).length;i++)
        if(eps[i]&&eps[i].server_data&&eps[i].server_data.length)
            return eps[i].server_data[0];
    return null;
}
function pLogo(imgs){
...
function eScr(scroll){
    ...
    var p={'transform':'none','overflow-y':'auto','overflow-x':'hidden',
           '-webkit-overflow-scrolling':'touch',height:'100%',
           'padding-bottom':'8em','touch-action':'pan-y'};
...
function oTSearch(){
    ...
    try{if(Lampa.Input&&Lampa.Input.edit){Lampa.Input.edit({title:'Tìm TMDB',value:'',free:true},go);return;}}catch(e){}
    go(window.prompt('Tìm TMDB:'));
}
...
function pStream(st){
    ...
    var size = '';
    var lines = raw.split('\n');
    // Thử từng dòng trước (AIOStreams để size/seeds mỗi dòng riêng)
    for(var li=0;li<lines.length;li++){
        var line = lines[li].trim();
        var lm = line.match(/💾\s*([\d.,]+\s*(?:TB|GB|GiB|MB|MiB))/i);
        if(lm){size=lm[1].trim();break;}
        lm = line.match(/^([\d.,]+)\s*(TB|GB|GiB|MB|MiB)\b/i);
        if(lm){size=lm[1]+' '+lm[2];break;}
    }
    // Fallback: tìm trong toàn raw
    if(!size){
        var sm=raw.match(/💾\s*([\d.,]+\s*(?:TB|GB|GiB|MB|MiB))/i);
        if(sm)size=sm[1].trim();
        else{
            sm=raw.match(/\b([\d.,]+)\s*(GB|GiB)\b/i);
            if(sm)size=sm[1]+' '+sm[2];
            else{sm=raw.match(/\b([\d.,]+)\s*(MB|MiB)\b/i);if(sm)size=sm[1]+' '+sm[2];}
        }
    }

    /* --- seeds --- */
    var seeds = '';
    for(var si2=0;si2<lines.length;si2++){
        var ln2=lines[si2].trim();
        var se=ln2.match(/👤\s*(\d+)/);
        if(se){seeds=se[1];break;}
        se=ln2.match(/^(\d+)\s*(?:seed|peer)/i);
        if(se){seeds=se[1];break;}
    }
    if(!seeds){
        var se2=raw.match(/👤\s*(\d+)/);
        if(se2)seeds=se2[1];
        else{se2=raw.match(/(\d+)\s*seed/i);if(se2)seeds=se2[1];}
    }

    /* --- quality --- */
    var qual='';
    var qm=raw.match(/\b(2160p|4K|UHD|1080p|720p|480p|HDR10?|BluRay|WEB-?DL|HDTV|CAM)\b/i);
...
async function lGenres(type){
    ...
    try{var r=await tFetch('/genre/'+type+'/list?language='+tmLang());_gc[type]=r.genres||[];return _gc[type];}
    catch(e){return[];}
}
async function tPersonC(pid){
    try{return await tFetch('/person/'+pid+'/combined_credits?language='+tmLang());}
    catch(e){return null;}
}
var TFN={
...
function mBest(items,title,orig,year){
    ...
    for(var i=0;i<items.length;i++){
        var n1=nStr(items[i].name||items[i].title||''),n2=nStr(items[i].origin_name||items[i].original_name||'');
        if((nT&&(n1===nT||n2===nT))||(nO&&(n1===nO||n2===nO))){
            if(!year||!items[i].year||String(items[i].year)===String(year))return items[i];
        }
    }
    for(var j=0;j<items.length;j++){
        var m1=nStr(items[j].name||items[j].title||''),m2=nStr(items[j].origin_name||items[j].original_name||'');
...
async function fSlugs(title,orig,year){
    var r={kkphim:null,ophim:null},terms=[title];
...
function exSn(name,slug){
    var m=name.match(/season\s*(\d+)/i)||name.match(/phần\s*(\d+)/i)||
          slug.match(/season-(\d+)/i)||slug.match(/phan-(\d+)/i)||name.match(/S(\d+)/);
...
async function fSeasonSlugs(source,title,orig){
    var results=[];
    try{
        var items=await sSrc(source,title);
        if(!items.length&&orig)items=await sSrc(source,orig);
        var nT=nStr(title),nO=nStr(orig);
        for(var i=0;i<items.length;i++){
            ...
            if(!match&&results.length>0){
                var bs=nStr(results[0].slug),cs=nStr(it.slug);
...
function bMag(h){
    ...
    ['udp://tracker.opentrackr.org:1337/announce','udp://open.stealth.si:80/announce'].forEach(function(t){m+='&tr='+encodeURIComponent(t);});
    return m;
}
async function playTS(stream,title,poster,fi){
    ...
    try{
        ...
        var files=[];
        if(info&&info.file_stats)files=info.file_stats
            .filter(function(f){return(f.path||'').toLowerCase().match(/\.(mp4|mkv|avi|mov|flv|webm|ts|m2ts)$/);})
        ...
        if(!files.length)Lampa.Player.play({title:title,url:tsU('/stream/fname?link='+hash+'&index=0&play')});
            ...
            Lampa.Select.show({title:'Chọn file',
...
async function fStreams(type,imdb,s,e){
    ...
    if(d.streams&&d.streams.length){
        console.log('[KK] raw[0] name:',d.streams[0].name);
        console.log('[KK] raw[0] title:',d.streams[0].title);
    }
...
async function oTorMov(tid,title,poster,imdb){
    ...
    try{var id=imdb||await gImdb('movie',tid);if(!id){Lampa.Noty.show('Không có IMDB');return;}var st=await fStreams('movie',id);if(!st.length){Lampa.Noty.show('Không có stream');return;}showStr(st,title,poster);}
    catch(e){Lampa.Noty.show('Lỗi: '+(e.message||''));}
}
async function oTorTV(tid,title,poster,imdb){
    Lampa.Noty.show('Tải season...');
    try{
        var id=imdb||await gImdb('tv',tid);if(!id){Lampa.Noty.show('Không có IMDB');return;}
        var sn=await gSeasons(tid);
        if(sn.length>1){Lampa.Select.show({title:'Chọn Season',items:sn.map(function(s){return{title:s.name+(s.episode_count?' ('+s.episode_count+' tập)':''),value:s};}),onSelect:function(a){pTorEp(a.value,id,title,poster);},onBack:function(){Lampa.Controller.toggle('content');}});}
...
function pTorEp(season,imdb,title,poster){
    ...
    Lampa.Select.show({title:season.name,items:items,
        onSelect:async function(a){
            ...
            try{var st=await fStreams('tv',imdb,a.value.s,a.value.e);if(!st.length){Lampa.Noty.show('Không có');return;}showStr(st,lb,poster);}
            catch(e){Lampa.Noty.show('Lỗi: '+(e.message||''));}
        },
        onBack:function(){Lampa.Controller.toggle('content');}
    });
...
function mkPpl(list,key,click){
    return(list||[]).map(function(p){
        var av=p.profile_path
            ?'<img src="'+TMDB_W500+p.profile_path+'">'
            :'<div class="kk-cast-empty"></div>';
        var cls=(click&&p.id)?' selector" data-person-id="'+p.id+'"':'"';
        return'<div class="kk-cast-card'+cls+'>'+
              '<div class="kk-cast-img">'+av+'</div>'+
              '<div class="kk-cast-name">'+E(p.name||'')+'</div>'+
              '<div class="kk-cast-role">'+E(p[key]||'')+'</div>'+
              '</div>';
    }).join('');
}
function bCast(c){
    c.find('.kk-cast-card[data-person-id]').each(function(){
        var card=$(this);
        bE(card,function(){
            var pid=card.attr('data-person-id');
            if(pid)Lampa.Activity.push({url:'',title:card.find('.kk-cast-name').text()||'',component:'kkphim_person',person_id:parseInt(pid),page:1});
        });
    });
}
function mkC(item){
    ...
    h+='</div>'+
       '<div class="kk-card-name">'+E(n.name)+'</div>'+
       '<div class="kk-card-year">'+E(n.year)+'</div>'+
       '</div>';
    var c=$(h);
...
function mkGrid(name,fetchFn,titleFn){
    Lampa.Component.add(name,function(obj){
        ...
        this.create=function(){
            ...
            scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+E(titleFn(obj))+'</div>').append(grid).append(lm));
            bE(lm,function(){if(!ld&&hm)doL();});
            doL();
        };
        function doL(){
            ld=true;lm.text('Đang tải...');
            fetchFn(obj,page).then(function(items){
                if(!items.length){hm=false;lm.text(page<=1?'Không có':'Hết');}
                else{items.forEach(function(i){grid.append(mkTC(i).addClass('kk-card--grid'));});page++;lm.text('Tải thêm');}
                ld=false;comp.activity.loader(false);comp.start();
            }).catch(function(){ld=false;lm.text('Lỗi');comp.activity.loader(false);});
        }
...
function mkSB(css,l1,l2){
    return $('<div class="kk-src-btn '+css+' selector">'+
             '<div class="kk-sb-main">'+l1+' <span class="kk-arrow">▼</span></div>'+
             (l2?'<div class="kk-sb-sub">'+l2+'</div>':'')+
             '</div>');
}
function bMovExp(sk,sn,slug,title,css){
    var w=$('<div style="width:100%"></div>');
    var btn=mkSB(css,'▶ '+E(sn),'Bấm để xem');
    ...
    bE(btn,function(){
        ...
        if(op&&!ld){
            ...
            fDet(SOURCES[sk],slug).then(function(det){
                ...
                box.empty();fillE(box,det.episodes,title);
...
function bTVExp(sk,sn,slug,title,orig,css){
    var w=$('<div style="width:100%"></div>');
    var btn=mkSB(css,'▶ '+E(sn),'Chọn season/tập');
    var box=$('<div class="kk-ep-box"></div>');
    w.append(btn).append(box);
    var ld=false,op=false;
    bE(btn,function(){
        op=!op;btn.toggleClass('kk-open',op);box.toggleClass('kk-show',op);
        if(op&&!ld){
            ld=true;box.html('<div class="kk-ep-ld">⏳ Tìm seasons...</div>');
...
function bDetExp(eps,title,sn,css){
    var w=$('<div style="width:100%"></div>');
    var total=0;(eps||[]).forEach(function(sv){total+=(sv.server_data||[]).length;});
    var btn=mkSB(css,'▶ '+E(sn),total+' tập');
    var box=$('<div class="kk-ep-box"></div>');
    w.append(btn).append(box);
    if(!eps||!eps.length||total===0){
        btn.removeClass(css).addClass('kk-src-btn--no');
        btn.html('⚠️ '+E(sn)+' - Không có tập');
        return w;
    }
    if(total===1){
        var ep=gEp1(eps);
        if(ep){
            ...
            btn.find('.kk-sb-main').html('▶ '+E(sn));
            btn.find('.kk-sb-sub').text('Phát ngay');
            btn.find('.kk-arrow').remove();
            bE(btn,function(){if(link)Lampa.Player.play({title:title,url:link});else Lampa.Noty.show('Không có link');});
            return w;
        }
    }
    fillE(box,eps,title);
    var op=false;
    bE(btn,function(){op=!op;btn.toggleClass('kk-open',op);box.toggleClass('kk-show',op);});
    return w;
}
...
function shSn(c,sMap,sNums,title){
    ...
    sNums.forEach(function(sn){
        var item=$('<div class="kk-sn-it selector">'+
                   '<span class="kk-sn-nm">📺 Season '+sn+'</span>'+
                   '<span class="kk-sn-bd">'+sMap[sn].length+'</span>'+
                   '</div>');
        bE(item,function(){ldSn(c,sMap[sn],title,sn,function(){shSn(c,sMap,sNums,title);});});
        c.append(item);
    });
}
    ...
    for(var i=0;i<entries.length;i++){
        try{
            ...
            if(det&&det.episodes&&det.episodes.length){
                ...
                fillE(c,det.episodes,title+' S'+pd(sNum));
                return;
            }
        }catch(e){}
    }
...
function fillE(c,eps,title){
    eps.forEach(function(sv){
        ...
        else if(snl.indexOf('lồng')>-1||snl.indexOf('long tieng')>-1)icon='🎤';
        ...
        (sv.server_data||[]).forEach(function(ep){
            ...
            bE(chip,function(){if(link)Lampa.Player.play({title:title+' - '+(ep.name||''),url:link});else Lampa.Noty.show('Không có link');});
            grid.append(chip);
        });
        c.append(grid);
    });
...
function inCSS(){
    ...
    var l=document.createElement('link');l.id='kk-css';l.rel='stylesheet';l.href=CSS_URL;
    document.head.appendChild(l);
}
function addM(){
    function ins(){
        if($('.menu__item[data-action="kkphim"]').length)return;
        var m=$('<li class="menu__item selector" data-action="kkphim">'+
                '<div class="menu__ico"><svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M4 4h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zm2 2v2h2V6H6zm4 0v2h2V6h-2zm4 0v2h2V6h-2zm4 0v2h2V6h-2zM6 10v8h12v-8H6z"/></svg></div>'+
...
function startPlugin(){
    ...
    Lampa.Component.add('kkphim_settings',function(){
        ...
        this.create=function(){
            ...
            w.append(g0);
            var g5=mg('🎨 Giao diện');var cg=s.card_style||'3';
            ...
            var tA=si2('🧪 Test AIO','Inception tt1375666','Nhấn');
            bE(tA,async function(){
                ...
                try{
                    ...
                    var d=await r.json();
                    if(d.streams&&d.streams.length){
                        console.log('[KK] AIO test name:',d.streams[0].name);
                        console.log('[KK] AIO test title:',d.streams[0].title);
                    }
                    var ps2=(d.streams||[]).map(pStream);
                    var ws=ps2.filter(function(x){return x.size;}).length;
                    var wsd=ps2.filter(function(x){return x.seeds;}).length;
                    stA.attr('class','kk-stg-status kk-stg-status--ok')
                       .text('✅ '+ps2.length+' streams | size:'+ws+' | seeds:'+wsd);
                }catch(e){stA.attr('class','kk-stg-status kk-stg-status--err').text('❌ '+(e.message||''));}
            });
            gA.append(tA).append(stA);w.append(gA);
            var g7=mg('🗃️ Cache');
            var cc=si2('Xóa cache genres','','Xóa');bE(cc,function(){_gc={movie:null,tv:null};Lampa.Noty.show('OK');});g7.append(cc);w.append(g7);
            var g4=$('<div class="kk-stg-group"></div>');
            var cl=si2('🗑️ Xóa toàn bộ','Khôi phục mặc định','Xóa');cl.find('.kk-stg-value').css('color','#f87171');
            bE(cl,function(){localStorage.removeItem(STG_KEY);_gc={movie:null,tv:null};Lampa.Activity.backward();});
            g4.append(cl);w.append(g4);
            w.append('<div class="kk-stg-ver">KKPhim v2.9.1</div>');
        ...
        function mi(n,d,val,prompt,key,s){
            var it=si2(n,d,val);
            bE(it,function(){
                try{if(Lampa.Input&&Lampa.Input.edit){Lampa.Input.edit({title:prompt,value:s[key]||'',free:true,nosave:true},function(v){v=(v||'').trim();var o={};o[key]=v;ss(o);s[key]=v;it.find('.kk-stg-value').text(v||val);});return;}}catch(e){}
                var v=window.prompt(prompt,s[key]||'');if(v!==null){v=v.trim();var o={};o[key]=v;ss(o);s[key]=v;it.find('.kk-stg-value').text(v||val);}
            });
            return it;
        }
        this.start=function(){aCtrl(scroll);eScr(scroll);};
    ...
    Lampa.Component.add('kkphim_person',function(obj){
        ...
        this.create=function(){
            ...
            Promise.all([tFetch('/person/'+pid+'?language='+tmLang()),tPersonC(pid)]).then(function(res){
                ...
                w.append('<div class="kk-person-header">'+
                         '<div class="kk-person-avatar">'+(av?'<img src="'+av+'">':'')+'</div>'+
                         '<div class="kk-person-info">'+
                         '<div class="kk-person-name">'+E(p.name||'')+'</div>'+
                         (p.birthday?'<div class="kk-person-meta">🎂 '+E(p.birthday)+'</div>':'')+
                         (p.known_for_department?'<div class="kk-person-meta">🎬 '+E(p.known_for_department)+'</div>':'')+
                         '</div></div>');
                ...
                if(cr&&cr.cast){
                    ...
                    if(mv.length){
                        var r1=$('<div class="kk-row"></div>'),rl=$('<div class="kk-row-list"></div>'),mr=$('<div class="kk-row-more selector">Thêm ('+mv.length+')</div>');
                        bE(mr,function(){Lampa.Activity.push({url:'',title:p.name+' - Movie',component:'kkphim_person_films',person_id:pid,film_type:'movie',page:1});});
                        mv.slice(0,10).forEach(function(c){rl.append(mkTC(c));});
                        r1.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">🎬 Phim lẻ</div>').append(mr)).append(rl);
                        w.append(r1);
                    }
                    if(tv.length){
                        var r2=$('<div class="kk-row"></div>'),rl2=$('<div class="kk-row-list"></div>'),mr2=$('<div class="kk-row-more selector">Thêm ('+tv.length+')</div>');
                        bE(mr2,function(){Lampa.Activity.push({url:'',title:p.name+' - TV',component:'kkphim_person_films',person_id:pid,film_type:'tv',page:1});});
                        tv.slice(0,10).forEach(function(c){rl2.append(mkTC(c));});
    ...
    Lampa.Component.add('kkphim_person_films',function(obj){
        ...
        this.create=function(){
            comp.activity.loader(true);cScr(scroll);
            grid.css('grid-template-columns','repeat('+cardSt()+',minmax(0,1fr))');
            scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+E(obj.title||'')+'</div>').append(grid).append(lm));
            bE(lm,function(){rMore();});
        ...
        function rMore(){
            var b=all.slice(rd,rd+pp);if(!b.length){lm.text('Hết');return;}
            b.forEach(function(c){grid.append(mkTC(c).addClass('kk-card--grid'));});
            rd+=b.length;lm.text(rd>=all.length?'Hết':'Tải thêm');
        }
        this.start=function(){aCtrl(scroll);eScr(scroll);};
        this.pause=function(){};this.stop=function(){};
        this.render=function(){return scroll.render();};
    ...
    Lampa.Component.add('kkphim_main',function(){
        ...
        this.create=function(){
            ...
            var tb=$('<div class="kk-topbar">'+
                     '<div class="kk-topbar-title">'+E(sc.name)+'</div>'+
                     '<div class="kk-topbar-btns"><div class="kk-btn selector">🔍</div><div class="kk-btn selector">⚙️</div></div>'+
                     '</div>');
            ...
            Object.keys(SOURCES).forEach(function(k){
                var s2=SOURCES[k],on=k===sc.key;
                var btn=$('<div class="kk-srcbtn selector '+(on?'kk-srcbtn--on':'kk-srcbtn--off')+'">'+E(s2.name)+'</div>');
                bE(btn,function(){if(on)return;ss({source:k});comp.create();});
                sb.append(btn);
            });
            ...
            cats.forEach(function(cat){
                net.silent(sApi()+cat.api+'?page=1',function(res){
                    ...
                    if(list.length){
                        var row=$('<div class="kk-row"></div>'),mr=$('<div class="kk-row-more selector">Xem thêm</div>'),rl=$('<div class="kk-row-list"></div>');
                        bE(mr,function(){Lampa.Activity.push({url:'',title:cat.name,component:'kkphim_category',cat:cat,page_num:1,mode:'api'});});
                        list.slice(0,12).forEach(function(i){rl.append(mkC(i));});
                        row.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">'+E(cat.name)+'</div>').append(mr)).append(rl);
                        scroll.append(row);
                    }
        ...
        this.pause=function(){};this.stop=function(){};
        this.render=function(){return scroll.render();};
        this.destroy=function(){net.clear();scroll.destroy();};
    });
    ...
    Lampa.Component.add('kkphim_tmdb_main',function(){
        ...
        this.create=function(){
            comp.activity.loader(true);cScr(scroll);
            var tb=$('<div class="kk-topbar">'+
                     '<div class="kk-topbar-title" style="color:#01b4e4">TMDB</div>'+
                     '<div class="kk-topbar-btns"><div class="kk-btn selector">🔍</div><div class="kk-btn selector">⚙️</div></div>'+
                     '</div>');
            ...
            Object.keys(SOURCES).forEach(function(k){
                var s2=SOURCES[k];
                var btn=$('<div class="kk-srcbtn selector kk-srcbtn--off">'+E(s2.name)+'</div>');
                bE(btn,function(){ss({source:k});Lampa.Activity.push({url:'',title:'KKPhim',component:'kkphim_main',page:1});});
                sb.append(btn);
            });
            ...
            scroll.append(sb);
            /* random row */
            var rr=$('<div class="kk-row"></div>'),rm=$('<div class="kk-row-more selector">Thêm</div>'),rl=$('<div class="kk-row-list"></div>');
            ...
            rr.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">🎲 Ngẫu nhiên</div>').append(rm)).append(rl);
            scroll.append(rr);
            Promise.all([tRand('movie'),tRand('tv')]).then(function(res){
                ...
                items.slice(0,12).forEach(function(i){rl.append(mkTC(i));});
            }).catch(function(){});
            var ld2=0;
            secs.forEach(function(sec){
                ...
                fn(1).then(function(res){
                    ...
                    if(items.length){
                        var row=$('<div class="kk-row"></div>'),mr=$('<div class="kk-row-more selector">Thêm</div>'),rl2=$('<div class="kk-row-list"></div>');
                        bE(mr,function(){Lampa.Activity.push({url:'',title:sec.name,component:'kkphim_tmdb_list',listType:sec.lt,page_num:1});});
                        items.slice(0,12).forEach(function(i){rl2.append(mkTC(i));});
                        row.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">'+E(sec.name)+'</div>').append(mr)).append(rl2);
                        scroll.append(row);
                    }
    ...
    Lampa.Component.add('kkphim_tmdb_genre',function(obj){
        ...
        function doL(){
            ...
            Promise.all(ps).then(function(){
                ...
                for(var i=0;i<all.length;i++){var key=all[i].media_type+'_'+all[i].id;if(!rs[key]){rs[key]=true;grid.append(mkTC(all[i]).addClass('kk-card--grid'));}}
                ld=false;comp.activity.loader(false);comp.start();
            }).catch(function(){ld=false;comp.activity.loader(false);});
        }
        this.start=function(){aCtrl(scroll);eScr(scroll);};
        this.pause=function(){};this.stop=function(){};
        this.render=function(){return scroll.render();};
    ...
    Lampa.Component.add('kkphim_tmdb_detail',function(obj){
        ...
        async function bDet(tmdb,logos,slugs){
            ...
            var extra='';
            if(tmdb.number_of_episodes)extra=tmdb.number_of_episodes+' tập';
            if(tmdb.number_of_seasons)extra+=(extra?' | ':'')+tmdb.number_of_seasons+' season';
            ...
            if(tmdb.credits){
                castH=mkPpl((tmdb.credits.cast||[]).slice(0,12),'character',true);
                var dirs=(tmdb.credits.crew||[])
                    .filter(function(c){return c.job==='Director'||c.job==='Creator'||c.job==='Series Director';})
                    .filter(function(p,i,a){return a.findIndex(function(x){return x.name===p.name;})===i;})
                    .slice(0,10);
                if(dirs.length){
                    dir=dirs.map(function(c){return c.name;}).join(', ');
                    dirH=mkPpl(dirs.map(function(c){return{id:c.id,name:c.name,profile_path:c.profile_path,job:c.job||'Đạo diễn'};}), 'job',true);
                }
            }
            ...
            if(slugs.kkphim){
                if(mt==='movie')act.append(bMovExp('kkphim','KKPhim',slugs.kkphim,t,'kk-src-btn--kkphim'));
                else act.append(bTVExp('kkphim','KKPhim',slugs.kkphim,t,o,'kk-src-btn--kkphim'));
            }else{act.append('<div class="kk-src-btn kk-src-btn--no">KKPhim – N/A</div>');}
            if(slugs.ophim){
                if(mt==='movie')act.append(bMovExp('ophim','OPhim',slugs.ophim,t,'kk-src-btn--ophim'));
                else act.append(bTVExp('ophim','OPhim',slugs.ophim,t,o,'kk-src-btn--ophim'));
    ...
    Lampa.Component.add('kkphim_category',function(obj){
        ...
        var grid=$('<div class="kk-grid"></div>'),lm=$('<div class="kk-loadmore selector">Tải thêm</div>'),ld=false,hm=true;
        this.create=function(){
            this.activity.loader(true);cScr(scroll);
            grid.css('grid-template-columns','repeat('+cardSt()+',minmax(0,1fr))');
            scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+E(title)+'</div>').append(grid).append(lm));
            bE(lm,function(){if(!ld&&hm)doL();});doL();
        };
        function hr(res){
            var list=((res&&res.items)||(res&&res.data&&res.data.items)||[]).map(nm).filter(function(i){return i&&i.slug;});
            if(!list.length){hm=false;lm.text('Hết');comp.activity.loader(false);ld=false;comp.start();return;}
            list.forEach(function(i){grid.append(mkC(i).addClass('kk-card--grid'));});
            page++;ld=false;lm.text('Tải thêm');comp.activity.loader(false);comp.start();
        }
        function doL(){
            ld=true;lm.text('Đang tải...');
            var url=(mode==='category'&&catSlug)?sApi()+'v1/api/the-loai/'+catSlug+'?page='+page:sApi()+apiPath+'?page='+page;
            net.silent(url,hr,function(){ld=false;lm.text('Lỗi');comp.activity.loader(false);});
        }
        this.start=function(){aCtrl(scroll);eScr(scroll);};
        this.pause=function(){};this.stop=function(){};
        this.render=function(){return scroll.render();};
    ...
    Lampa.Component.add('kkphim_search',function(obj){
        ...
        this.create=function(){
            this.activity.loader(true);cScr(scroll);
            grid.css('grid-template-columns','repeat('+cardSt()+',minmax(0,1fr))');
            scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">🔍 '+E(kw)+'</div>').append(grid).append(lm));
            bE(lm,function(){if(!ld&&hm)doL();});doL();
        };
        function hr(res){
            var list=((res&&res.items)||(res&&res.data&&res.data.items)||[]).map(nm).filter(function(i){return i&&i.slug;});
            if(!list.length){hm=false;lm.text(page===1?'Không có':'Hết');comp.activity.loader(false);ld=false;comp.start();return;}
            list.forEach(function(i){grid.append(mkC(i).addClass('kk-card--grid'));});
            page++;ld=false;lm.text('Tải thêm');comp.activity.loader(false);comp.start();
        }
        function doL(){
            ld=true;lm.text('Đang tải...');
            net.silent(sApi()+'v1/api/tim-kiem?keyword='+encodeURIComponent(kw)+'&page='+page,hr,function(){
                net.silent(sApi()+'tim-kiem?keyword='+encodeURIComponent(kw)+'&page='+page,hr,function(){
                    ld=false;lm.text('Lỗi');comp.activity.loader(false);
                });
            });
        }
        this.start=function(){aCtrl(scroll);eScr(scroll);};
    ...
    Lampa.Component.add('kkphim_detail',function(obj){
        ...
        this.create=function(){
            ...
            net.silent(sApi()+'phim/'+movie.slug,function(res){
                if(rnd)return;
                ldAll(nm(res.movie||res||{}),res.episodes||[]);
            },function(){comp.activity.loader(false);});
        };
        async function ldAll(data,eps){
            ...
            try{
                ...
                if(tid){
                    try{tmdb=await tFetch('/'+tt+'/'+tid+'?language='+tmLang()+'&append_to_response=credits,images');}catch(e){}
                    try{logos=await tFetch('/'+tt+'/'+tid+'/images');}catch(e){}
                }
        ...
        function bld(data,eps,tmdb,logos,tt){
            ...
            if(tmdb){
                ...
                if(tmdb.credits){
                    castH=mkPpl((tmdb.credits.cast||[]).slice(0,12),'character',true);
                    var dirs=(tmdb.credits.crew||[])
                        .filter(function(c){return c.job==='Director'||c.job==='Creator';})
                        .filter(function(p,i,a){return a.findIndex(function(x){return x.name===p.name;})===i;})
                        .slice(0,10);
                    if(dirs.length){
                        dir=dirs.map(function(c){return c.name;}).join(', ');
            ...
            body.find('.kk-genre[data-slug]').each(function(){
                var g=$(this);
                bE(g,function(){var slug=g.attr('data-slug');if(slug)Lampa.Activity.push({url:'',title:g.attr('data-title')||'',component:'kkphim_category',mode:'category',category_slug:slug,page_num:1});});
            });
            var act=$('<div class="kk-actions"></div>');
            ...
            if(pCats.length&&pCats[0]&&pCats[0].slug){
                ...
                fetch(sApi()+'v1/api/the-loai/'+pCats[0].slug+'?page=1')
                    ...
                    .then(function(res){
                        ...
                        sl.empty();
                        if(items.length)items.slice(0,20).forEach(function(i){sl.append(mkC(i));});
                        else sl.html('<div class="kk-ep-ld">Không có</div>');
                    }).catch(function(){sl.empty();});
            }
            if(hasTmdb){
                var tid2=gTid(data);
                ...
                tSimilar(tt,tid2,1).then(function(res){
                    var items=(res.results||[]).slice(0,20);tl.empty();
                    if(items.length)items.forEach(function(i){if(!i.media_type)i.media_type=tt;tl.append(mkTC(i));});
                    else tl.html('<div class="kk-ep-ld">Không có</div>');
                }).catch(function(){tl.empty();});
            }
            scroll.append(dw);
        }
        this.start=function(){aCtrl(scroll);eScr(scroll);};
