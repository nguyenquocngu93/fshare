(function () {
    ...
    /* ============================================================
       Torrentio + Knaben Parser for Lampa  —  v3.1.0
    ...
    /* ============================================================
       TITLE PARSER — trích badges từ tên torrent
    ============================================================ */
    function parseTorrentTitle(title) {
        ...
        // Voice/language tags — dòng từ Torrentio thường dạng "RUS - HDRezka"
        var voiceMatches = t.match(/\b(RUS|UKR|ENG|FRE|GER|SPA|ITA|JPN|CHI)(?:\s*[-–]\s*[^\s|,\n]+)?/gi) || [];
        ...
        // HDR
        if (/\bHDR\b/i.test(t))             badges.push({ text: 'HDR',          cls: 'torr-badge--hdr' });
        if (/Dolby.?Vision|DV\b/i.test(t))  badges.push({ text: 'Dolby Vision', cls: 'torr-badge--hdr' });
                ...
                setTimeout(function () {
                    $.ajax({
                        ...
                        success: function (d) {
                            ...
                            var vidFiles = files.filter(function (f) {
                                return /\.(mp4|mkv|avi|mov|wmv|m4v|ts)$/i.test(f.path || '');
                            });
                            var list = vidFiles.length ? vidFiles : files;
                            ...
                            if (list.length > 1) {
                                Lampa.Select.show({
                                    ...
                                    items: list.map(function (f, i) {
                                        return {
                                            title: (f.path || 'File ' + i).split('/').pop(),
                                            index: f.id !== undefined ? f.id : i
                                        };
                                    }),
                                    onSelect: function (item) {
                                        var url = tsUrl + '/stream?link=' + encodeURIComponent(useHash) +
                                                  '&index=' + item.index + '&play';
                                        Lampa.Player.play({ title: title, url: url, poster: '' });
                                    },
                                    onBack: function () { Lampa.Activity.backward(); }
                                });
                            } else {
                                var url = tsUrl + '/stream?link=' + encodeURIComponent(useHash) +
                                          '&index=' + (fileIdx || 0) + '&play';
                                Lampa.Player.play({ title: title, url: url, poster: '' });
                            }
                        },
                        error: function () {
                            // Fallback: dùng hash trực tiếp
                            var url = tsUrl + '/stream?link=' + encodeURIComponent(hash) +
                                      '&index=' + (fileIdx || 0) + '&play';
                            Lampa.Player.play({ title: title, url: url, poster: '' });
                        }
                    });
    ...
    /* ============================================================
       COMPONENT: KẾT QUẢ — dùng Lampa.Scroll đúng chuẩn
    ============================================================ */
    function ResultsComponent(object) {
        ...
        function makeCard(r) {
            var badges = parseTorrentTitle(r.title);
            ...
            statsEl.innerHTML =
                'Bitrate: <b>–</b>  ' +
                'Seeds: <b style="color:#27ae60">' + r.seeds + '</b>  ' +
                'Leechers: <b style="color:#e74c3c">' + r.peers + '</b>';
            if (r.sizeStr) {
        ...
        // ✅ FIX SCROLL: dùng đúng pattern của Lampa
        this.start = function () {
    ...
    function doSearch(card, season, episode) {
        ...
        // 1. Torrentio
        if (imdbId) {
            fetchTorrentio(imdbId, type, season, episode, onPartDone);
        } else {
            ...
            net.silent(
                ...
                function (data) {
                    ...
                    if (id) {
                        card.imdb_id = id;
                        fetchTorrentio(id, type, season, episode, onPartDone);
                    } else {
                        onPartDone([]);
                    }
                },
    ...
    $('<style>').text([
        '.torr-card {',
        '  display:block; padding:14px 16px; margin:3px 0;',
        ...
        '}',
        '.torr-card__title {',
        '  font-size:.86em; color:#eee; margin-bottom:7px; line-height:1.4;',
        '}',
        '.torr-card__badges {',
        '  display:flex; flex-wrap:wrap; gap:4px; margin-bottom:5px;',
        '}',
        '.torr-badge {',
        '  padding:2px 7px; border-radius:3px; font-size:.7em;',
        ...
        '.torr-badge--sub   { background:rgba(255,167,38,.2);   color:#ffcc80; }',
        ...
        '.torr-card__info {',
        '  font-size:.72em; color:#777; margin-bottom:4px;',
        '}',
        '.torr-card__stats {',
        '  display:flex; align-items:center; gap:12px;',
        '  font-size:.75em; color:#999; position:relative;',
        '}',
        '.torr-card__size {',
        '  margin-left:auto;',
        '  background:rgba(255,255,255,.08);',
        '  padding:2px 10px; border-radius:4px;',
        '  color:#eee; font-size:.95em; white-space:nowrap;',
        '}'
    ].join('\n')).appendTo('head');

    console.log('[Torrentio+Knaben] v3.1.0 loaded');
