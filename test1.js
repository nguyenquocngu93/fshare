 ...
 *
 * Version: 2.1.0 - Thêm main() với Lampa.Api.partNext()
 */
(function () {
  'use strict';
  ...
  function getKKList(srcKey, cat, title, page) {
    return listAPI(srcKey, cat, page || 1).then(function (res) {
      return {
        title: title,
        results: (res.items || []).map(function (i) { return convertItem(i, srcKey); }).filter(Boolean),
        page: page || 1,
  ...
  function list(params, oncomplite, onerror) {
    ...
    getKKList(srcKey, cat, '', page).then(function (data) {
      oncomplite(data);
    }).catch(function () {
      if (onerror) onerror();
    });
  ...
  function addPlugin() {
    ...
    console.log('[KKPhim Parser] v2.1.0 OK — Home + List ready');
  }
