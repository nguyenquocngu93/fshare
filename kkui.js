/* KKPhim Plugin v4.5.3 - Final Fixed (Tested on Lampa) */
(function(){
'use strict';
...
function applyFontScale(){
    ...
    var r=fs/100;
    var css=''
    +'.kk-card,.kk-card-h,.kk-pfc{font-size:'+fs+'%!important;}'    +'.kk-card-origin,.kk-card-h .kk-card-origin{font-size:'+(0.95*r)+'em!important;}'
    ...
    +'.kk-hm-badge{font-size:'+(1.5*r)+'em!important;}'
    ...
    +'.kk-coll-header-name{font-size:'+(1.4*r)+'em!important;font-weight:800!important;}'    +'.kk-coll-header-count{font-size:'+(1.0*r)+'em!important;}'
    +'.kk-coll-overview{font-size:'+(.2*r)+'em!important;line-height:1.6!important;}'
    ...
    +'.kk-card-ep--type{font-size:0.72em;font-weight:600;padding:0.2em 0.55em;border-radius:0.3em;background:rgba(1,180,228,0.15);color:#01b4e4;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:6em;}'
    ;
...
// ✅ FIX: Hàm parse tmdb_id từ slug — ĐÃ ĐƯỢC ĐỊNH NGHĨA TRƯỚC KHI DÙNG
function parseTmdbIdFromSlug(slug) {
    if (!slug) return null;
    var m = slug.match(/(?:tmdid-|)(\d+)(?:\.html|\/)?$/);
...
function cTio(raw){
    if(!raw)return'';raw=String(raw).trim();if(!raw)return'';var m=raw.match(/torrentio\.strem\.fun\/([^\/]+?)\/manifest\.json/i);if(m)return m[1];m=raw.match(/torrentio\.strem\.fun\/configure\/([^\s]+)/i);if(m)return m[1].replace(/\/+$/,'');m=raw.match(/torrentio\.strem\.fun\/([^\/]+?)\/stream\//i);if(m)return m[1];if(raw.indexOf('torrentio.strem.fun')>-1){raw=raw.replace(/^https?:\/\/torrentio\.strem\.fun\/?/i,'').replace(/\/(manifest\.json|stream\/.*|configure\/?.*)?$/i,'').replace(/^\/+|\/+$/g,'');if(raw&&raw.indexOf('=')>-1)return raw.replace(/\|/g,'%7C');return'';}raw=raw.replace(/^\/+|\/+$/g,'').replace(/\|/g,'%7C');return raw.indexOf('=')===-1?'':raw;
}
...
function bE(el,fn){
    ...
    el.on('touchmove',function(e){        var t=((e.originalEvent||e).touches||[])[0];
    ...
    el.on('click',function(e){
        if(tc||mv)return;e.preventDefault();e.stopPropagation();fn.call(this,e);
    });
...
function pStream(st){
    ...
    var size='';
    var sP=[/💾\s*([\d.,]+\s*(?:TB|GB|GiB|MB|MiB))/i,/\b([\d.,]+)\s*(TB)\b/i,/\b([\d.,]+)\s*(GB|GiB)\b/i,/\b([\d.,]+)\s*(MB|MiB)\b/i];
    for(var i=0;i<sP.length;i++){
        var sm=rawAll.match(sP[i]);
        if(sm){size=sm[2]?sm[1]+' '+sm[2]:sm[1].trim();break;}
    }
    var seeds='';
    var sdP=[/👤\s*(?:Seeders?:?\s*)?(\d+)/i,/Seeders?:?\s*(\d+)/i,/(\d+)\s*seed/i];
    for(var j=0;j<sdP.length;j++){
        var se=rawAll.match(sdP[j]);
        if(se){seeds=se[1];break;}
    }
    var source='';
    var srP=[/🔗\s*(?:Source:?\s*)?([\w\.\-]+)/i,/Source:?\s*([\w\.\-]+)/i,/\[([A-Z0-9\-]+)\]/];
    for(var k=0;k<srP.length;k++){
        var srm=rawAll.match(srP[k]);
    ...
    if(bh.bingeGroup){
        var bg=String(bh.bingeGroup);
        if(!qual){
            var bqm=bg.match(/\b(2160p|4K|UHD|1080p|720p|480p|HDR10?|BluRay|WEB-?DL|HDTV|CAM)\b/i);
            if(bqm)qual=bqm[1].toUpperCase();
        }
    }
    if(!qual&&filename){        var fqm=filename.match(/\b(2160p|4K|UHD|1080p|720p|480p|HDR10?|BluRay|WEB-?DL|HDTV|CAM)\b/i);
        if(fqm)qual=fqm[1].toUpperCase();
    }
...
async function lGenres(type){
    ...
    try{
        var r=await tFetch('/genre/'+type+'/list?language='+tmLang());
        _gc[type]=r.genres||[];
        return _gc[type];
    }catch(e){return[];}
}
...
async function tDiscoverNetwork(networkId,p){
    return await tFetch('/discover/tv?language='+tmLang()+'&sort_by=popularity.desc&with_networks='+networkId+'&page='+(p||1));
}

async function tCollectionDetail(collectionId){
    try{return await tFetch('/collection/'+collectionId+'?language='+tmLang());}catch(e){return null;}
}

async function tSeasonDetail(tvId,seasonNum){
    try{return await tFetch('/tv/'+tvId+'/season/'+seasonNum+'?language='+tmLang());}catch(e){return null;}
}
...
async function tSearchM(q,p){
    return await tFetch('/search/multi?language='+tmLang()+'&query='+encodeURIComponent(q)+'&page='+(p||1));}

async function tDetFull(type,id){
    return await tFetch('/'+type+'/'+id+'?language='+tmLang()+'&append_to_response=credits,images,similar,external_ids');
}

async function tImgFull(type,id){
    return await tFetch('/'+type+'/'+id+'/images');
}

async function tDiscover(type,gid,p){
    return await tFetch('/discover/'+type+'?language='+tmLang()+'&sort_by=popularity.desc&with_genres='+gid+'&page='+(p||1));
}

async function tRec(type,id,p){
    return await tFetch('/'+type+'/'+id+'/recommendations?language='+tmLang()+'&page='+(p||1));
}

async function tRand(type){
    return await tFetch('/'+type+'/popular?language='+tmLang()+'&page='+(Math.floor(Math.random()*10)+1));
}
...
async function playTS(stream,title,poster,fi){
    ...
    try{
        ...
        var files=[];        if(info&&info.file_stats)files=info.file_stats.filter(function(f){
            return(f.path||'').toLowerCase().match(/\.(mp4|mkv|avi|mov|flv|webm|ts|m2ts)$/);
        }).sort(function(a,b){return(a.id||0)-(b.id||0);});
        var playFile=function(fileUrl){
            Lampa.Player.play({title:title,url:fileUrl});
        };
...
async function fStreams(type,imdb,s,e){
    ...
    if(!r.ok)throw new Error(eng+' '+r.status);    var d=await r.json();
...
function injectTorrentMenuCSS(){
    ...
    if($('#'+id).length)return;
    $('head').append('<style id="'+id+'">.selectbox .selectbox-item__title{white-space:pre-wrap!important;overflow:visible!important;text-overflow:unset!important;-webkit-line-clamp:unset!important;display:block!important;}.selectbox .selectbox-item{height:auto!important;max-height:none!important;}</style>');
}
        ...
        if(sn.length>1){
            Lampa.Select.show({
                ...
                items:sn.map(function(s){
                    return{title:s.name+(s.episode_count?' ('+s.episode_count+' tập)':''),value:s};                }),
...
function mkTC(item){
    ...
    var typeLabel=d.media_type==='tv'?'TV Series':'Movie';
    var h='<div class="kk-card selector"><div class="kk-card-img">'+img;
    if(d.vote)h+='<div class="kk-card-q">⭐ '+E(d.vote)+'</div>';
    h+='<div class="kk-card-ep kk-card-ep--type">'+typeLabel+'</div></div><div class="kk-card-body"><div class="kk-card-name">'+E(d.name)+'</div>';
...
function mkTCH(item){
    ...
    if(d.vote)h+='<div class="kk-card-q">⭐ '+E(d.vote)+'</div>';
    h+='<div class="kk-card-ep kk-card-ep--type">'+typeLabel+'</div></div><div class="kk-card-h-body"><div class="kk-card-name">'+E(d.name)+'</div>';
    if(d.origin_name&&d.origin_name!==d.name)h+='<div class="kk-card-origin">'+E(d.origin_name)+'</div>';
    h+='<div class="kk-card-meta">';
    if(d.year)h+='<span class="kk-card-year">'+E(d.year)+'</span>';
    h+='</div></div></div>';
...
// ✅ SỬA: mkSourceCard — phân biệt TV/Movie chính xác
function mkSourceCard(item,source){
    var p=fImg(item.poster_url||item.thumb_url,source);

    // XÁC ĐỊNH LOẠI PHIM
    var isTV = (item.episode_total && item.episode_total > 0) ||
               (item.type && /(?:series|tv|bo|phim-bo|hoat-hinh|anime)/i.test(item.type)) ||               (item.category && item.category.some(c => /(?:phim bộ|tv|series|hoạt hình)/i.test(c.name || '')));
    var typeLabel = isTV ? 'TV Series' : 'Movie';
...
// ✅ SỬA: mkSourceCardH — tương tự
function mkSourceCardH(item,source){
    var p=fImg(item.thumb_url||item.poster_url,source);

    var isTV = (item.episode_total && item.episode_total > 0) ||
               (item.type && /(?:series|tv|bo|phim-bo|hoat-hinh|anime)/i.test(item.type)) ||
               (item.category && item.category.some(c => /(?:phim bộ|tv|series|hoạt hình)/i.test(c.name || '')));
    var typeLabel = isTV ? 'TV Series' : 'Movie';
...
function mkCastList(castArr,hasTmdb){
    ...
    castArr.forEach(function(c){
        ...
        if(hasTmdb&&c.id){
            ...
            bE(card,function(){
                Lampa.Activity.push({url:'',title:c.name||'',component:'kkphim_person',person_id:c.id,page:1});
            });
...
function mkProductionSection(companies){
    ...
    companies.forEach(function(comp){
        ...
        card.html('<div class="kk-prod-logo">'+(logoUrl?'<img src="'+logoUrl+'" loading="lazy">':'<div class="kk-prod-logo-empty">'+E((comp.name||'?')[0])+'</div>')+'</div><div class="kk-prod-name">'+E(comp.name||'')+'</div>'+(comp.origin_country?'<div class="kk-prod-country">'+E(comp.origin_country)+'</div>':''));
...
function mkNetworkSection(networks){
    ...
    networks.forEach(function(net){
        ...
        var card=$('<div class="kk-prod-card selector"></div>');
        card.html('<div class="kk-prod-logo">'+(logoUrl?'<img src="'+logoUrl+'" loading="lazy">':'<div class="kk-prod-logo-empty">'+E((net.name||'?')[0])+'</div>')+'</div><div class="kk-prod-name">'+E(net.name||'')+'</div>'+(net.origin_country?'<div class="kk-prod-country">'+E(net.origin_country)+'</div>':''));
        bE(card,function(){
            Lampa.Activity.push({url:'',title:net.name||'',component:'kkphim_network',network_id:net.id,network_name:net.name||'',network_logo:logoUrl,page_num:1});
        });
        list.append(card);
    });
    sec.append(list);
...
function mkTagsSection(keywords){
    ...
    keywords.forEach(function(kw){
        ...
        bE(tag,function(){
            Lampa.Activity.push({url:'',title:kw.name||'',component:'kkphim_keyword',keyword_id:kw.id,keyword_name:kw.name||'',page_num:1});
        });
        list.append(tag);
    });
    sec.append(list);
...
function mkSeasonsSection(seasons,tvId,tvTitle){
    ...
    seasons.forEach(function(s){
        ...
        card.html('<div class="kk-season-poster">'+(ps?'<img src="'+ps+'" loading="lazy">':'<div class="kk-season-poster-empty">S'+s.season_number+'</div>')+'</div><div class="kk-season-info"><div class="kk-season-name">'+E(s.name||'Season '+s.season_number)+'</div>'+(s.episode_count||s.air_date?'<div class="kk-season-meta">'+E([(s.air_date||'').slice(0,4),s.episode_count?s.episode_count+' tập':''].filter(Boolean).join(' · '))+'</div>':'')+'</div>');
        bE(card,function(){
            Lampa.Activity.push({url:'',title:tvTitle+' - '+s.name,component:'kkphim_season_detail',tv_id:tvId,season_number:s.season_number,tv_title:tvTitle,page:1});
        });
        list.append(card);
    });
    sec.append(list);
...
function injectExtraCSS(){
    ...
    $('head').append('<style id="'+id+'">'+
        ...
        '.kk-coll-bg{position:absolute;inset:0;}.kk-coll-bg img{width:100%;height:100%;object-fit:cover;}.kk-coll-ov{position:absolute;inset:0;background:linear-gradient(90deg,rgba(0,0,0,0.85) 0%,rgba(0,0,0,0.4) 60(0,0,0,0.15) 100%);}.kk-coll-inner{position:relative;z-index:1;display:flex;align-items:center;gap:0.8em;padding:0.8em;height:100%;}.kk-coll-poster{width:3.5em;height:5.2em;flex-shrink:0;border-radius:0.3em;overflow:hidden;}.kk-coll-poster img{width:100%;height:100%;object-fit:cover;}.kk-coll-info{flex:1;min-width:0;}'+
            ...
            fetchFn(obj,page).then(function(items){
                ...
                if(!items.length){
                    ...
                    items.forEach(function(i){
                        curGrid.append((cm==='hgrid'?mkTCH(i):mkTC(i)).addClass('kk-card--grid'));
                    });
                    page++;
                }
                ld=false;
...
Lampa.Component.add('kkphim_settings',function(){
    ...
    this.create=function(){
        cScr(scroll);
        var s=ls(),w=$('<div class="kk-stg-wrap"></div>');
        w.append('<div class="kk-stg-title">⚙️ Cài đặt</div>');
        var gSrc=mg('📺 Bật/Tắt nguồn phim');
        Object.keys(SOURCES).forEach(function(k){
            var sc=SOURCES[k],enabled=isSourceEnabled(k);
            gSrc.append(mo(sc.name,'API: '+sc.api,enabled,function(){
                ss({'source_'+k+'_enabled':!enabled});
                Lampa.Noty.show(sc.name+': '+(enabled?'Đã tắt':'Đã bật'));
                comp.create();
            }));
        });
        w.append(gSrc);
        var gFont=mg('🔠 Cỡ font');
        var fs=String(s.font_scale||'100');        [{k:'85',n:'Nhỏ'},{k:'100',n:'Mặc định'},{k:'115',n:'Lớn'},{k:'130',n:'Rất lớn'}].forEach(function(o){
            gFont.append(mo(o.n,'Cỡ chữ toàn plugin',fs===o.k,function(){
                ss({font_scale:o.k});
                applyFontScale();
                comp.create();
            }));
        ...
        [{k:'hgrid',n:'📐 Lưới ngang',d:'2 cột, backdrop ngang'},{k:'poster',n:'🖼️ Poster lớn',d:'1 card full'}].forEach(function(o){
            gCard.append(mo(o.n,o.d,cm===o.k,function(){
                ss({card_mode:o.k});
                comp.create();
            }));
        });
        ...
        [{k:'10',n:'10'},{k:'15',n:'15'},{k:'20',n:'20'},{k:'30',n:'30'},{k:'50',n:'50'}].forEach(function(o){
            gRow.append(mo(o.n+' phim','',rc===o.k,function(){
                ss({row_count:o.k});
                comp.create();
            }));
        });
        ...
        [{k:'hgrid',n:'📐 Ngang (x2)'},{k:'vgrid',n:'🖼️ Dọc (tùy chỉnh)'}].forEach(function(o){
            gCat.append(mo(o.n,'',ctm===o.k,function(){
                ss({cat_mode:o.k});
                comp.create();
            }));
        });
        ...
        [{k:'2',n:'2 cột'},{k:'3',n:'3 cột'},{k:'4',n:'4 cột'}].forEach(function(o){
            g5.append(mo(o.n,'',cg===o.k,function(){
                ss({card_style:o.k});
                comp.create();
            }));
        });
        ...
        [{k:'vi-VN',n:'Tiếng Việt'},{k:'en-US',n:'English'},{k:'ja-JP',n:'日本語'},{k:'ko-KR',n:'한국어'},{k:'zh-CN',n:'中文'}].forEach(function(o){
            g6.append(mo(o.n,o.k,cl2===o.k,function(){
                ss({tmdb_lang:o.k});
                _gc={movie:null,tv:null};                comp.create();
            }));
        });
        w.append(g6);
        var gE=mg('🎯 Nguồn Torrent');
        ...
        g1.append(mi('Địa chỉ','192.168.1.100:8090',s.torrserver_host||'Chưa cài','Địa chỉ','torrserver_host',s));
        ...
        var tTS=si2('🧪 Test','','Test');
        bE(tTS,async function(){
            var h=tsHost();
            if(!h){stTS.show().html('<span style="color:#f87171">❌ Chưa nhập</span>');return;}
            stTS.show().html('<span style="color:#fbbf24">⏳...</span>');
            try{
                var r=await fetch(tsU('/echo'),{method:'GET',headers:tsH()});
                stTS.html(r.ok?'<span style="color:#4ade80">✅ OK</span>':'<span style="color:#f87171">❌ '+r.status+'</span>');
            }catch(e){
                stTS.html('<span style="color:#f87171">❌ '+(e.message||'Lỗi')+'</span>');
            }
        });
        ...
        gA.append(mi('URL','Dán full URL',s.aio_url||'Chưa cài','AIO URL','aio_url',s));
        w.append(gA);
        var gHist=mg('🕘 Dữ liệu');
        var clearHist=si2('Xóa lịch sử','','Xóa');
        bE(clearHist,function(){
        ...
        var cl=si2('🗑️ Reset','Khôi phục mặc định','Xóa');
        cl.find('.kk-stg-value').css('color','#f87171');
        bE(cl,function(){
            localStorage.removeItem(STG_KEY);
            localStorage.removeItem(HIST_KEY);
            _gc={movie:null,tv:null};
            applyFontScale();
            Lampa.Activity.backward();        });
        g4.append(cl);
        w.append(g4);
        w.append('<div class="kk-stg-ver">KKPhim v4.5.3 - Final Fixed</div>');
        scroll.append(w);
        comp.start();
    };
    ...
    function mo(n,d,on,cb){
        var it=si2(n,d,on?'✅':'○');
        if(on)it.find('.kk-stg-value').css('color','#4ade80');
        bE(it,cb);
        return it;
    }
            ...
            try{
                if(Lampa.Input&&Lampa.Input.edit){
                    Lampa.Input.edit({title:prompt,value:s[key]||'',free:true,nosave:true},function(v){
                        v=(v||'').trim();
                        var o={};o[key]=v;ss(o);s[key]=v;it.find('.kk-stg-value').text(v||val);
                    });
                    return;
                }
            }catch(e){}
...
Lampa.Component.add('kkphim_person_films',function(obj){
    ...
    var spinner=$('<div class="kk-row-loading" style="display:none"><div class="kk-row-spinner"></div></div>');    var endMsg=$('<div class="kk-loadmore" style="display:none;cursor:default;background:transparent">Hết</div>');
    this.create=function(){
        comp.activity.loader(true);
        cScr(scroll);
        initGrid();
    ...
    function rMore(){
        ...
        b.forEach(function(c){
            curGrid.append((cm==='hgrid'?mkTCH(c):mkTC(c)).addClass('kk-card--grid'));
        });
        rd+=b.length;
...
Lampa.Component.add('kkphim_main',function(){
    var scroll=new Lampa.Scroll({mask:true,over:true}),comp=this;
    var currentTab='kkphim';    var tabKK,tabOP,tabTMDB;
        ...
        srcCats.forEach(function(cat){
            fetch(source.api+cat.api+'?page=1').then(function(r){
                ...
                if(list.length){
                    ...
                    row.append(mkRowList(list.slice(0,cnt),false,source));                    contentContainer.append(row);
                }
    ...
    function loadTMDBTab(){
        ...
        var rr=$('<div class="kk-row"></div>');        rr.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">🎲 Ngẫu nhiên</div>'));
        var randC=$('<div></div>');
        rr.append(randC);
...
Lampa.Component.add('kkphim_source_detail',function(obj){
    ...
    this.create=function(){
        ...
        fDet(source,slug).then(async function(det){
            ...
            try {
                ...
                // === PARSE TMDB_ID TỪ SLUG ===
                tmdbId = parseTmdbIdFromSlug(slug);
                if (tmdbId) {
                    tmdbData = await tDetFull(mediaType, tmdbId);
                } else {
                    // Fallback: search bằng tên
                    var searchType = mediaType;
                    var res = await tFetch('/search/'+searchType+'?language='+tmLang()+'&query='+encodeURIComponent(origin||title)+'&page=1');
            ...
            } catch(e) {                console.warn('TMDB fetch error:',e);
            }
    ...
    function buildDetail(movie,episodes,tmdbData,mediaType){
        ...
        if(tmdbData){
            ...
            // === LOGO: ưu tiên từ images, nếu không thì dùng poster làm fallback ===
            var logo=pLogo(tmdbData.images||{});
            if(logo&&logo.file_path){
                logoH='<div class="kk-logo"><img src="'+TMDB_W500+logo.file_path+'"></div>';
            } else if(poster){
                // Fallback: dùng poster làm logo (nhỏ hơn, nhưng vẫn hiển thị)
                logoH='<div class="kk-logo"><img src="'+poster+'"></div>';            }
        ...
        if(episodes.length===0){
            ...
            episodes.forEach(function(server){                var serverName=server.server_name||'Server';
        ...
        if(movie.actor&&movie.actor.length&&!cast.length){
            ...
            movie.actor.forEach(function(actor){                var card=$('<div class="kk-cast-card"><div class="kk-cast-img"><div class="kk-cast-empty"></div></div><div class="kk-cast-info"><div class="kk-cast-name">'+E(actor)+'</div></div></div>');
...
Lampa.Component.add('kkphim_source_genre',function(obj){
    ...
    var page=obj.page_num||1,gridContainer=$('<div></div>'),ld=false,done=false,curGrid=null;
    var spinner=$('<div class="kk-row-loading" style="display:none"><div class="kk-row-spinner"></div></div>');    var endMsg=$('<div class="kk-loadmore" style="display:none;cursor:default;background:transparent">Đã hết</div>');
...
mkGridInfinite('kkphim_tmdb_list',function(obj,page){
    ...
    return fn(page).then(function(r){
        return(r.results||[]).filter(function(i){return i.media_type!=='person';});
    });
},function(obj){return obj.title||'TMDB';});

mkGridInfinite('kkphim_tmdb_search',function(obj,page){
    return tSearchM(obj.keyword||'',page).then(function(r){
        return(r.results||[]).filter(function(i){return i.media_type!=='person';});
    });
...
mkTabbedPage('kkphim_company',function(obj,p){
    return tDiscoverCompany('movie',obj.company_id,p);
},function(obj,p){
    return tDiscoverCompany('tv',obj.company_id,p);
},function(obj){
 var header=$('<div class="kk-page-header"></div>');
    ...
    tCompanyDetail(obj.company_id).then(function(cd){
        if(cd&&cd.origin_country)header.find('.kk-page-header-meta').text('Hãng sản xuất · '+cd.origin_country);
    }).catch(function(){});
    return header;
});
    ...
    function doL(){
        ...
        tDiscoverNetwork(obj.network_id,page).then(function(r){
            ...
            if(!items.length){
                ...
                items.forEach(function(i){i.media_type='tv';});
                var cm=catMode();
                items.forEach(function(i){
                    curGrid.append((cm==='hgrid'?mkTCH(i):mkTC(i)).addClass('kk-card--grid'));
                });
...
mkTabbedPage('kkphim_keyword',function(obj,p){
    return tDiscoverKeyword('movie',obj.keyword_id,p);
},function(obj,p){
    return tDiscoverKeyword('tv',obj.keyword_id,p);
},function(obj){
    return $('<div class="kk-keyword-header"></div>').html('<div class="kk-keyword-header-badge">'+E(obj.keyword_name||'')+'</div>');
});
...
Lampa.Component.add('kkphim_tmdb_genre',function(obj){
    ...
    this.create=function(){
        ...
        scroll.append(gb);        var titleEl=$('<div class="kk-grid-title" style="padding:0 1.5em">'+E(obj.title||'')+'</div>');
        ...
        Promise.all([lGenres('movie'),lGenres('tv')]).then(function(res){
            ...
            (res[0]||[]).concat(res[1]||[]).forEach(function(g){
                if(!sn[g.id]){sn[g.id]=true;mg2.push(g);}
            });
            ...
            mg2.forEach(function(g){
                ...
                bE(ch,function(){
                    Lampa.Activity.push({url:'',title:g.name,component:'kkphim_tmdb_genre',genre_id:g.id,page_num:1});
                });
                gb.append(ch);
...
Lampa.Component.add('kkphim_collection',function(obj){
    ...
    this.create=function(){
        ...
        tCollectionDetail(obj.collection_id).then(function(coll){
            ...
            header.html('<div class="kk-coll-header-bg">'+(bk?'<img src="'+bk+'">':'')+'</div><div class="kk-coll-header-overlay"></div><div class="kk-coll-header-inner">'+(ps?'<div class="kk-coll-header-poster"><img src="'+ps+'"></div>':'')+'<div class="kk-coll-header-info"><div class="kk-coll-header-label">Bộ sưu tập</div><div class="kk-coll-header-name">'+E(coll.name||'')+'</div><div class="kk-coll-header-count">'+((coll.parts||[]).length)+' phim</div></div></div>');
            scroll.append(header);
            ...
            parts.forEach(function(p){
                p.media_type='movie';
                grid.append((cm==='hgrid'?mkTCH(p):mkTC(p)).addClass('kk-card--grid'));
            });
...
Lampa.Component.add('kkphim_season_detail',function(obj){
    ...
    this.create=function(){
        ...
        Promise.all([tSeasonDetail(tvId,seasonNum),gImdb('tv',tvId)]).then(function(res){
            ...
            if(!season){
                comp.activity.loader(false);                return;
            }
...
Lampa.Component.add('kkphim_tmdb_detail',function(obj){
    ...
    this.create=function(){
        ...
        tDetFull(mt,tid).then(async function(tmdb){
            ...
            _detCtx={imdbId:imdb};            var keywords=[];
            try{keywords=await tKeywords(mt,tid);}catch(e){}
            bDet(tmdb,logos,imdb,keywords);
        }).catch(function(e){
            comp.activity.loader(false);
            Lampa.Noty.show('Lỗi: '+(e.message||''));
        });
    ...
    async function bDet(tmdb,logos,imdb,keywords){
        cScr(scroll);
        ...
        var country='';
        if(tmdb.production_countries&&tmdb.production_countries.length)
            country=tmdb.production_countries.map(function(c){return c.iso_3166_1;}).join(', ');
        else if(tmdb.origin_country&&tmdb.origin_country.length)
            country=tmdb.origin_country[0];
        ...
        // ⚠️ Nếu không có logo nhưng có poster → dùng poster làm logo (đã sửa)
        else if(ps) logoH='<div class="kk-logo"><img src="'+ps+'"></div>';

        var gh=gHtml(tmdb.genres,true),dirsArr=[],castArr=[];
        if(tmdb.credits){
            castArr=(tmdb.credits.cast||[]).slice(0,15);
            dirsArr=(tmdb.credits.crew||[]).filter(function(c){
                return c.job==='Director'||c.job==='Creator'||c.job==='Series Director';
            }).filter(function(p,i,a){
                return a.findIndex(function(x){return x.name===p.name;})===i;
            }).slice(0,8);
        ...
        var hero=mkHero(bk,ps,logoH,tH,o2,{
            ...
            vote:v,
            year:y,
            runtime:rt+(epExtra?' · '+epExtra:''),            country:country,
            genres:genreStr,
            status:status,
            _title:t
        });
        ...
        body.find('.kk-genre[data-gid]').each(function(){
            ...
            bE(g,function(){
                Lampa.Activity.push({
                    url:'',
                    title:g.attr('data-gname')||'',
                    component:'kkphim_tmdb_genre',
                    genre_id:g.attr('data-gid'),
                    page_num:1
                });
            });
        });
        ...
        if(mt==='tv'){
            try{
                var tmdbSeasons=await gSeasons(tid);
                if(tmdbSeasons&&tmdbSeasons.length)dw.append(mkSeasonsSection(tmdbSeasons,tid,t));
            }catch(e){}
        }
        ...
        if(mt==='tv'&&tmdb.networks&&tmdb.networks.length)
            dw.append(mkNetworkSection(tmdb.networks));

        if(mt==='movie'&&tmdb.belongs_to_collection)
            dw.append(mkCollectionSection(tmdb.belongs_to_collection));
