...
describe("HTTP app", () => {
  ...
  it("cache-busts frontend assets and forces JS/CSS revalidation", async () => {
    ...
    assert.match(css, /\.cw-detail-gradient\{background:linear-gradient\(to bottom,rgba\(5,7,10,\.01\) 0%,rgba\(5,7,10,\.04\) 42%,rgba\(5,7,10,\.18\) 63%,rgba\(5,7,10,\.70\) 84%,#05070a 100%\)/);
    ...
    assert.match(css, /\.cw-detail-body\{padding-top:52px\}/);
    ...
    assert.match(js, /progress=Math\.max\(0,Math\.min\(1,scrollY\/Math\.max\(220,hero\.offsetHeight\*\.92\)\)\),zoomEnd=\.20,holdEnd=\.34,zoomOut=Math\.min\(1,progress\/zoomEnd\),motion=Math\.max\(0,Math\.min\(1,\(progress-holdEnd\)\/\(1-holdEnd\)\)\)/);
    assert.match(js, /logoProgress=Math\.max\(0,Math\.min\(1,\(motion-\.18\)\/.82\)\)/);
    assert.match(js, /reveal=Math\.max\(0,Math\.min\(1,\(logoProgress-\.18\)\/.72\)\)/);
    assert.match(js, /backdropScale=1\.15-\.15\*zoomOut,backdropY=-10\*logoProgress,logoY=-58\*logoProgress/);
