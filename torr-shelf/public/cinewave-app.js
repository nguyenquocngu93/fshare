...
function updateTmdbSettings(tmdb){
  ...
  if(!tmdb.configured){els.tmdbConfigBadge.textContent='OFF';els.tmdbConfigBadge.classList.remove('public');els.tmdbConfigStatus.textContent='Chưa có credential. Dán Read Access Token hoặc API Key v3 rồi lưu.';return;}
...
async function saveTmdbCredential(value){
  ...
  const original=submit.textContent;submit.disabled=true;submit.textContent='Đang lưu…';els.tmdbConfigStatus.textContent='Đang lưu và kiểm tra TMDB…';
  try{
    const result=await api('/api/settings/tmdb',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({credential})});
    els.tmdbCredentialInput.value='';state.health={...(state.health||{}),tmdb:result.tmdb};updateSettings();
