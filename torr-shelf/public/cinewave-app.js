...
function updateDetailHeaderMotion(){
  ...
  const heroLogo=$('.cw-detail-logo,.cw-detail-title-visual h1'),backdrop=$('#detailBackdrop img'),backdropScale=1.04-.04*backdropProgress,backdropY=-8*logoProgress,logoY=-64*logoProgress;if(heroLogo){heroLogo.style.transform=`translateY(${logoY}px) scale(${1-logoProgress*.2})`;heroLogo.style.opacity=String(1-reveal*.88);}if(backdrop)backdrop.style.transform=`translateY(${backdropY}px) scale(${backdropScale})`;
}
...
function showView(name){if(name!=='detail')resetDetailHeaderMotion();state.view=name;document.body.classList.toggle('cw-detail-open',name==='detail');els.views.forEach(v=>v.classList.toggle('hidden',v.id!==`${name}View`));$$('[data-go]').forEach(b=>b.classList.toggle('active',b.dataset.go===name));if(name!=='home')clearInterval(state.heroTimer);else startHero();resetScroll();if(name==='detail')requestAnimationFrame(updateDetailHeaderMotion);}
...
function detailMarkup(item){
  ...
  const compactCredit=(person)=>`<button class="cw-stremio-credit-pill" data-person="${person.id}" type="button">${esc(person.name)}</button>`;
  const creditGroup=(label,people,limit)=>{
    const visible=(people||[]).slice(0,limit);
    return visible.length?`<section class="cw-stremio-credit-group"><h2>${esc(label)}</h2><div>${visible.map(compactCredit).join('')}</div></section>`:'';
  };
  ...
  const share='<button class="cw-detail-icon" data-share-current type="button" aria-label="Chia sẻ" title="Chia sẻ"><svg viewBox="0 0 24 24"><circle cx="18" cy="5" r="2.5"/><circle cx="6" cy="12" r="2.5"/><circle cx="18" cy="19" r="2.5"/><path d="m8.2 10.8 7.5-4.4M8.2 13.2l7.5 4.4"/></svg></button>';
  const library='<button class="cw-detail-icon" data-library-current type="button" aria-label="Thêm vào thư viện" title="Thư viện"><svg viewBox="0 0 24 24"><path d="M4 5h16v14H4zM8 2v6M16 2v6M8 13h8M12 9v8"/></svg></button>';
  ...
  const heart='<button class="cw-detail-meta-icon" data-library-current type="button" aria-label="Thêm vào thư viện" aria-pressed="false"><svg viewBox="0 0 24 24"><path d="M20.8 4.9a5.5 5.5 0 0 0-7.8 0L12 6l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.9-8.3a5.5 5.5 0 0 0-.1-7.8z"/></svg></button>';
  const metaFacts=[
  ...
  const stremioCredits=`<div class="cw-stremio-credits">${creditGroup(item.mediaType==='tv'?'Creator':'Director',item.directors,3)}${creditGroup('Cast',item.cast,6)}${creditGroup('Writer',item.writers,5)}</div>`;
...
els.detailContent.addEventListener('click',e=>{
  ...
  const openSettings=e.target.closest('[data-open-addon-settings]');if(openSettings)return navigate('settings');
