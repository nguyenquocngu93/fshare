/* KKPhim Plugin v4.6.0 - Fixed Slug + Search */
(function(){
'use strict';
...
// NEW: getBaseName - loại bỏ "Season X", "Phần X" khỏi tên
...
// NEW: getBaseSlug - normalize slug bỏ phần season
...
function getTypeLabel(item){var t=getItemType(item);return t==='tv'?'TV':'Film';}

/* ---- FONT SCALE ---- */
function applyFontScale(){
    var id='kkphim-font-scale';$('#'+id).remove();
    var fs=fontScale(),r=fs/100;
...
function cTio(raw){if(!raw)return'';raw=String(raw).trim();if(!raw)return'';var m=raw.match(/torrentio\.strem\.fun\/([^\/]+?)\/manifest\.json/i);if(m)return m[1];m=raw.match(/torrentio\.strem\.fun\/([^\/]+?)\/stream\//i);if(m)return m[1];if(raw.indexOf('torrentio.strem.fun')>-1){raw=raw.replace(/^https?:\/\/torrentio\.strem\.fun\/?/i,'').replace(/\/(manifest\.json|stream\/.*|configure\/?.*)?$/i,'').replace(/^\/+|\/+$/g,'');if(raw&&raw.indexOf('=')>-1)return raw.replace(/\|/g,'%7C');return'';}raw=raw.replace(/^\/+|\/+$/g,'').replace(/\|/g,'%7C');return raw.indexOf('=')===-1?'':raw;}
...
/* ---- TORRENT STREAM PARSE ---- */
function pStream(st){var rawName=String(st.name||'');var rawDesc=String(st.description||'');var rawTitle=String(st.title||'');var rawAll=rawName+'\n'+rawDesc+'\n'+rawTitle;var name=rawName.split('\n')[0].trim()||rawTitle.split('\n')[0].trim()||'?';var qual='';var qm=rawAll.match(/\b(2160p|4K|UHD|1080p|720p|480p|HDR10?|BluRay|WEB-?DL|HDTV|CAM)\b/i);if(qm)qual=qm[1].toUpperCase();var size='';var sP=[/\s*([\d.,]+\s*(?:TB|GB|GiB|MB|MiB))/i,/\b([\d.,]+)\s*(GB|GiB)\b/i,/\b([\d.,]+)\s*(MB|MiB)\b/i];for(var i=0;i<sP.length;i++){var sm=rawAll.match(sP[i]);if(sm){size=sm[2]?sm[1]+' '+sm[2]:sm[1].trim();break;}}var seeds='';var sdP=[/\s*(?:Seeders?:?\s*)?(\d+)/i,/(\d+)\s*seed/i];for(var j=0;j<sdP.length;j++){var se=rawAll.match(sdP[j]);if(se){seeds=se[1];break;}}if(st.behaviorHints&&typeof st.behaviorHints==='object'){var bh=st.behaviorHints;if(!size&&bh.videoSize){var bytes=Number(bh.videoSize);if(!isNaN(bytes)&&bytes>0){if(bytes>=1073741824)size=(bytes/1073741824).toFixed(1)+' GB';else if(bytes>=1048576)size=(bytes/1048576).toFixed(0)+' MB';}}}return{name:name,infoHash:st.infoHash||'',fileIdx:st.fileIdx,url:st.url||'',size:size,seeds:seeds,quality:qual};}
function fmtStream(s){var parts=[s.name];if(s.quality&&s.name.toUpperCase().indexOf(s.quality)===-1)parts[0]+=' ['+s.quality+']';var meta=[];if(s.size)meta.push('💾 '+s.size);if(s.seeds)meta.push('👤 '+s.seeds);if(meta.length)parts.push(meta.join(' '));return parts.join('\n');}
...
/* ---- SEARCH & SLUG - FIXED ---- */
// FIXED: sSrc - thêm limit=30, xử lý nhiều response format
async function sSrc(source,kw,page){
    try{
        ...
        var d=await r.json();
        var items=[];
        // FIXED: xử lý nhiều dạng response
    ...
    }catch(e){console.warn('sSrc ['+source.key+']:',e.message||e);return[];}
}

// FIXED: mScore - thêm base name matching
function mScore(item,title,orig,year){
    ...
    // Year bonus
    if(score>0&&year&&item.year){var iy=parseInt(item.year),ty=parseInt(year);if(iy===ty)score+=30;else if(Math.abs(iy-ty)<=1)score+=15;}
    return score;
}

function mBest(items,title,orig,year){
    if(!items||!items.length)return null;
    var scored=items.map(function(it){return{item:it,score:mScore(it,title,orig,year)};}).sort(function(a,b){return b.score-a.score;});
...
// FIXED: fSlugs - dùng base terms, tìm song song nhiều terms
...
// FIXED: fDet - fallback 2 endpoints
...
// FIXED: exSn - thêm patterns đầy đủ
...
// FIXED: fSeasonSlugs - nhóm theo base slug như code tham khảo
async function fSeasonSlugs(source,title,orig,targetSeason,tmdbTotalSeasons){
    ...
    try{
        ...
        if(title&&title!==baseTitle&&terms.indexOf(title)===-1)terms.push(title);
        if(!terms.length)terms.push(title);
        // Thu thập tất cả items, deduplicate
        ...
        // Build results từ best group
        var bestSeasons=groups[bestBase];
        bestSeasons.sort(function(a,b){return a.season-b.season;});
        var seenSn={};
        for(var i=0;i<bestSeasons.length;i++){
            var s=bestSeasons[i];
            if(seenSn[s.season])continue;
            if(tmdbTotalSeasons&&s.season>tmdbTotalSeasons)continue;
            seenSn[s.season]=true;
            results.push({slug:s.slug,name:s.name,season:s.season,source:source,year:s.year});
        }
...
/* ============================================================
   PHẦN 2: UI CARDS, HERO, BODY, EPISODE PLAYER
============================================================ */
...
function mkSourceCard(item,source){
    ...
    bE(c,async function(){
        ...
        else{Lampa.Noty.show('TMDB không tìm thấy');openSourceDirect(item,source);}
...
function mkCollectionSection(collection){if(!collection||!collection.id)return $('<div></div>');var sec=$('<div style="padding:2.5em 1.5em 0;background:#1a1a1e;"></div>');var bk=collection.backdrop_path?TMDB_W500+collection.backdrop_path:'';var ps=collection.poster_path?TMDB_W200+collection.poster_path:'';var card=$('<div class="kk-coll-card selector"></div>');card.html('<div class="kk-coll-bg">'+(bk?'<img src="'+bk+'" loading="lazy">':'')+'</div><div class="kk-coll-ov"></div><div class="kk-coll-inner">'+(ps?'<div class="kk-coll-poster"><img src="'+ps+'" loading="lazy"></div>':'')+'<div class="kk-coll-info"><div class="kk-coll-label">Bộ sưu tập</div><div class="kk-coll-name">'+E(collection.name||'')+'</div></div></div>');bE(card,function(){Lampa.Activity.push({url:'',title:collection.name||'',component:'kkphim_collection',collection_id:collection.id,page_num:1});});sec.append(card);return sec;}
...
function bTVExp(sk,sn,slug,title,orig,css,tmdbSeasons){
    ...
    bE(btn,function(){
        ...
        if(op&&!ld){
            ld=true;
            box.html('<div class="kk-ep-ld">⏳ Tìm seasons...</div>');
...
function injectExtraCSS(){
    var id='kk-extra-css-v460';if($('#'+id).length)return;
    ...
    +'.kk-prod-name{font-size:0.75em;font-weight:600;color:rgba(255,255,255,0.85);text-align:center;}'
    +'.kk-tags-list{display:flex;flex-wrap:wrap;gap:0.4em;padding:0.4em 0 0.5em;}'
    +'.kk-tag-chip{display:inline-flex;align-items:center;padding:0.3em 0.8em;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);border-radius:2em;font-size:0.8em;color:rgba(255,255,255,0.7);cursor:pointer;transition:all 0.2s;}'
    ...
    +'.kk-coll-bg{position:absolute;inset:0;}.kk-coll-bg img{width:100%;height:100%;object-fit:cover;}'
    ...
    +'.kk-coll-label{font-size:0.7em;text-transform:uppercase;color:rgba(1,180,228,0.8);font-weight:600;}'
    ...
    +'.kk-season-info{padding:0.4em 0.4em 0.5em;}.kk-season-name{font-size:0.75em;font-weight:600;color:rgba(255,255,255,0.88);}'
    ...
    +'.kk-sn-it{display:flex;justify-content:space-between;align-items:center;padding:0.5em 0.8em;border-radius:0.4em;cursor:pointer;font-size:0.9em;background:rgba(255,255,255,0.04);margin-bottom:0.3em;transition:background 0.15s;}'
    ...
    +'.kk-sd-poster{width:6.5em;flex-shrink:0;border-radius:0.5em;overflow:hidden;aspect-ratio:2/3;}'
    ...
    +'.kk-netflix-ep{display:flex;gap:0;border-radius:0.5em;overflow:hidden;margin-bottom:0.5em;cursor:pointer;transition:all 0.2s;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);}'
    ...
    +'.kk-netflix-ep-title{font-size:0.88em;font-weight:600;color:rgba(255,255,255,0.92);margin-bottom:0.1em;}'
...
Lampa.Component.add('kkphim_source_search',function(obj){
    ...
    var page=1,gridContainer=$('<div></div>'),ld=false,done=false,curGrid=null;
    ...
    this.create=function(){comp.activity.loader(true);cScr(scroll);initGrid();scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+E(obj.title||'')+'</div>').append(gridContainer).append(spinner).append(endMsg));scroll.render().find('.scroll__body').on('scroll',function(){if(ld||done)return;if(this.scrollTop+this.clientHeight>=this.scrollHeight-400)doL();});doL();};
    function doL(){ld=true;spinner.show();sSrc(source,kw,page).then(function(items){spinner.hide();if(!items||!items.length){done=true;endMsg.show().text(page<=1?'Không tìm thấy':'Đã hết');comp.activity.loader(false);ld=false;comp.start();return;}var cm=catMode();items.forEach(function(i){curGrid.append((cm==='hgrid'?mkSourceCardH(i,source):mkSourceCard(i,source)).addClass('kk-card--grid'));});page++;ld=false;comp.activity.loader(false);comp.start();}).catch(function(){ld=false;spinner.hide();endMsg.show().text('Lỗi');comp.activity.loader(false);});}
...
Lampa.Component.add('kkphim_main',function(){
    ...
    this.create=function(){
        ...
        bE($(tb.find('.kk-btn')[0]),function(){try{if(Lampa.Input&&Lampa.Input.edit){Lampa.Input.edit({title:'Tìm kiếm',value:'',free:true},function(kw){if(kw)searchInCurrentTab(kw.trim());});return;}}catch(e){}var kw=window.prompt('Tìm kiếm:');if(kw)searchInCurrentTab(kw.trim());});
        ...
        loadTab(currentTab);
        function switchTab(tab){if(currentTab===tab)return;currentTab=tab;tabKK.toggleClass('kk-type-tab--active',tab==='kkphim');tabOP.toggleClass('kk-type-tab--active',tab==='ophim');tabTMDB.toggleClass('kk-type-tab--active',tab==='tmdb');contentContainer.empty();comp.activity.loader(true);loadTab(tab);}
        function loadTab(tab){if(tab==='tmdb')loadTMDBTab();else loadSourceTab(SOURCES[tab]);}
        function searchInCurrentTab(kw){if(currentTab==='tmdb'){Lampa.Activity.push({url:'',title:'TMDB: '+kw,component:'kkphim_tmdb_search',keyword:kw,page_num:1});}else{var source=SOURCES[currentTab];Lampa.Activity.push({url:'',title:source.name+': '+kw,component:'kkphim_source_search',source_key:currentTab,keyword:kw,page_num:1});}}
    };
    ...
    function loadTMDBTab(){
        ...
        if(hist&&hist.tmdb_id){tRec(hist.media_type||'movie',hist.tmdb_id,1).then(function(res){var items=(res.results||[]).slice(0,cnt);if(items.length){items.forEach(function(i){if(!i.media_type)i.media_type=hist.media_type||'movie';});var rowRec=$('<div class="kk-row"></div>');rowRec.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">💡 Vì bạn đã xem "'+E(hist.name||'')+'"</div>'));rowRec.append(mkRowList(items,true));contentContainer.prepend(rowRec);}}).catch(function(){});}
        var rr=$('<div class="kk-row"></div>');rr.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">🎲 Ngẫu nhiên</div>'));var randC=$('<div></div>');rr.append(randC);contentContainer.append(rr);
        Promise.all([tRand('movie'),tRand('tv')]).then(function(res){var items=[];(res[0].results||[]).forEach(function(i){i.media_type='movie';items.push(i);});(res[1].results||[]).forEach(function(i){i.media_type='tv';items.push(i);});for(var si=items.length-1;si>0;si--){var sj=Math.floor(Math.random()*(si+1));var st=items[si];items[si]=items[sj];items[sj]=st;}randC.replaceWith(mkRowList(items.slice(0,cnt),true));}).catch(function(){});
        var loaded=0,total=tmdbSecs.length;
        tmdbSecs.forEach(function(sec){var fn=TFN[sec.lt];if(!fn){loaded++;if(loaded>=total){comp.activity.loader(false);comp.start();}return;}fn(1).then(function(res){var items=(res.results||[]).filter(function(i){return i.media_type!=='person';});if(items.length){var row=$('<div class="kk-row"></div>');var mr=$('<div class="kk-row-more selector">Xem thêm</div>');bE(mr,function(){Lampa.Activity.push({url:'',title:sec.name,component:'kkphim_tmdb_list',listType:sec.lt,page_num:1});});row.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">'+E(sec.name)+'</div>').append(mr));row.append(mkRowList(items.slice(0,cnt),true));contentContainer.append(row);}loaded++;if(loaded>=total){comp.activity.loader(false);comp.start();}}).catch(function(){loaded++;if(loaded>=total){comp.activity.loader(false);comp.start();}});});
    }
    this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){scroll.destroy();};
});

/* ---- TMDB LIST / SEARCH INFINITE ---- */
...
/* ---- NETWORK, GENRE, COLLECTION, SEASON DETAIL ---- */
...
Lampa.Component.add('kkphim_season_detail',function(obj){
    ...
    this.create=function(){
        ...
        Promise.all([tSeasonDetail(tvId,seasonNum),gImdb('tv',tvId)]).then(function(res){
            ...
            header.html('<div class="kk-sd-poster">'+(ps?'<img src="'+ps+'">':'<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:1.8em;color:rgba(255,255,255,0.12);">S'+seasonNum+'</div>')+'</div><div class="kk-sd-info"><div class="kk-sd-title">'+E(season.name||'Season '+seasonNum)+'</div><div class="kk-sd-meta">'+E(tvTitle+' · '+(season.air_date||'')+' · '+(season.episodes||[]).length+' tập')+'</div>'+(season.overview?'<div class="kk-sd-overview">'+E(season.overview.substring(0,250))+'</div>':'')+'</div>');
            ...
            (season.episodes||[]).forEach(function(ep){
                ...
                item.html('<div class="kk-netflix-ep-still">'+(still?'<img src="'+still+'" loading="lazy">':'<div class="kk-netflix-ep-still-empty">'+ep.episode_number+'</div>')+'</div><div class="kk-netflix-ep-body"><div class="kk-netflix-ep-title">'+E(ep.name||'Episode '+ep.episode_number)+'</div>'+(ep.overview?'<div class="kk-netflix-ep-desc">'+E(ep.overview)+'</div>':'')+'</div>');
                bE(item,function(){if(imdbId)playEpTorrent(imdbId,seasonNum,ep.episode_number,tvTitle,ps||'');else Lampa.Activity.push({url:'',title:tvTitle,component:'kkphim_tmdb_detail',tmdb_id:tvId,media_type:'tv',page:1});});
                epList.append(item);
            });
...
Lampa.Component.add('kkphim_tmdb_detail',function(obj){
    ...
    this.create=function(){
        ...
        tDetFull(mt,tid).then(async function(tmdb){
            ...
            if(!imdb)try{imdb=await gImdb(mt,tid);}catch(e){}_detCtx={imdbId:imdb};
    ...
    async function bDet(tmdb,logos,slugs,tmdbSeasons,keywords){
        ...
        Object.keys(enabledSources).forEach(function(sk){
            ...
            if(slugs[sk]){
                ...
                var naBtn=$('<div class="kk-src-btn kk-src-btn--no selector" style="width:100%"><div class="kk-sb-main">'+E(sn)+' – Không tìm thấy</div><div class="kk-sb-sub">Bấm thử lại</div></div>');
                ...
                bE(naBtn,async function(){
                    ...
                    var baseTitle=getBaseName(t),baseOrig=getBaseName(o2);
                    var terms=[baseOrig,baseTitle,o2,t];
                    if(y){terms.push(baseOrig+' '+y);terms.push(baseTitle+' '+y);}
                    for(var i=0;i<terms.length;i++){
                        if(!terms[i]||terms[i].length<2)continue;
                        var items=await sSrc(source,terms[i],1);
                        var best=mBest(items,t,o2,y);
                        if(best&&best.slug){slugs[sk]=best.slug;Lampa.Noty.show('Tìm thấy!');bDet(tmdb,logos||(tmdb.images||{}),slugs,tmdbSeasons,keywords);return;}
                    }
        ...
        try{var rd2=await tRec(mt,tid,1);var ri=(rd2.results||[]).slice(0,20);if(ri.length){var rs2=$('<div class="kk-section kk-similar kk-section--last"></div>').append('<div class="kk-block-title">Gợi ý</div>');var rl2=$('<div class="kk-similar-list"></div>');ri.forEach(function(i){if(!i.media_type)i.media_type=mt;rl2.append(mkTC(i));});rs2.append(rl2);dw.append(rs2);}}catch(e){}
...
Lampa.Component.add('kkphim_settings',function(){
    ...
    this.create=function(){
        ...
        [{k:'hgrid',n:'🖼 Lưới ngang',d:'2 cột, backdrop ngang'},{k:'poster',n:'📋 Poster lớn',d:'1 card full'}].forEach(function(o){gCard.append(mo(o.n,o.d,cm===o.k,function(){ss({card_mode:o.k});comp.create();}));});
        w.append(gCard);
        var gRow=mg('📊 Số phim trong row');var rc=s.row_count||'20';
        [{k:'10',n:'10'},{k:'15',n:'15'},{k:'20',n:'20'},{k:'30',n:'30'},{k:'50',n:'50'}].forEach(function(o){gRow.append(mo(o.n+' phim','',rc===o.k,function(){ss({row_count:o.k});comp.create();}));});
        w.append(gRow);
        var gCat=mg('📋 Danh sách - Kiểu card');var ctm=s.cat_mode||'hgrid';
        [{k:'hgrid',n:'↔ Ngang (x2)'},{k:'vgrid',n:'↕ Dọc (tùy chỉnh)'}].forEach(function(o){gCat.append(mo(o.n,'',ctm===o.k,function(){ss({cat_mode:o.k});comp.create();}));});
        w.append(gCat);
        var g5=mg('📐 Số cột poster dọc');var cg=s.card_style||'3';
        [{k:'2',n:'2 cột'},{k:'3',n:'3 cột'},{k:'4',n:'4 cột'}].forEach(function(o){g5.append(mo(o.n,'',cg===o.k,function(){ss({card_style:o.k});comp.create();}));});
        w.append(g5);
        ...
        bE(tTS,async function(){var h=tsHost();if(!h){stTS.show().html('<span style="color:#f87171">❌ Chưa nhập</span>');return;}stTS.show().html('<span style="color:#fbbf24">⏳ ...</span>');try{var r=await fetch(tsU('/echo'),{method:'GET',headers:tsH()});stTS.html(r.ok?'<span style="color:#4ade80">✅ OK</span>':'<span style="color:#f87171">❌ '+r.status+'</span>');}catch(e){stTS.html('<span style="color:#f87171">❌ '+(e.message||'Lỗi')+'</span>');}});
        ...
        var gA=mg('🎬 AIOStreams');gA.append(mi('URL','Dán full URL',s.aio_url||'Chưa cài','AIO URL','aio_url',s));w.append(gA);
        var gHist=mg('📦 Dữ liệu');var clearHist=si2('Xóa lịch sử','','Xóa');bE(clearHist,function(){localStorage.removeItem(HIST_KEY);Lampa.Noty.show('OK');});gHist.append(clearHist);w.append(gHist);
        var g4=$('<div class="kk-stg-group"></div>');var cl=si2('🔄 Reset','Khôi phục mặc định','Xóa');cl.find('.kk-stg-value').css('color','#f87171');bE(cl,function(){localStorage.removeItem(STG_KEY);localStorage.removeItem(HIST_KEY);_gc={movie:null,tv:null};applyFontScale();Lampa.Activity.backward();});g4.append(cl);w.append(g4);
        w.append('<div class="kk-stg-ver">KKPhim v4.6.0</div>');
...
function startPlugin(){
    ...
    console.log('[KKPhim] v4.6.0 - Fixed Slug/Search ✅');
}
