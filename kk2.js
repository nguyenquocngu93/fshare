(function () {
    ...
    function parseStreamInfo(st) {
        ...
        for (var ri = 0; ri < sizePatterns.length; ri++) {
            var sm = raw.match(sizePatterns[ri]);
            if (sm) { size = (sm[1] || '').trim(); if (!size && sm[2]) size = sm[0].trim(); break; }
        }

        var seeds = '';
        var seedPatterns = [ /👤\s*(\d+)/, /(\d+)\s*seed/i, /Seeds?:?\s*(\d+)/i ];
        for (var si = 0; si < seedPatterns.length; si++) {
            var sem = raw.match(seedPatterns[si]);
        ...
        return { name: name, infoHash: st.infoHash || '', fileIdx: st.fileIdx, url: st.url || '', size: size, seeds: seeds, quality: quality, raw: raw };
    }
    ...
    async function getTmdbSeasons(id) {
        try {
            var r = await tmdbFetch('/tv/'+id+'?language='+tLang());
    ...
    async function tmdbRandom(type) { var rp = Math.floor(Math.random()*10)+1; return await tmdbFetch('/'+type+'/popular?language='+tLang()+'&page='+rp); }

    function tmdbNormCard(item) {
        if (!item) return null;
        var mt = item.media_type || (item.first_air_date ? 'tv' : 'movie');
        return { tmdb_id: item.id, media_type: mt, name: item.title||item.name||'', poster_url: item.poster_path ? TMDB_IMG_W500+item.poster_path : '', year: (item.release_date||item.first_air_date||'').slice(0,4), vote: item.vote_average ? Number(item.vote_average).toFixed(1) : '' };
    }
    ...
    function matchBest(items, title, origTitle, year) {
        ...
        for (var i = 0; i < items.length; i++) {
            var n1 = normStr(items[i].name||items[i].title||''), n2 = normStr(items[i].origin_name||items[i].original_name||'');
            if ((nT && (n1===nT||n2===nT)) || (nO && (n1===nO||n2===nO))) {
                if (!year || !items[i].year || String(items[i].year)===String(year)) return items[i];
            }
        }
        for (var j = 0; j < items.length; j++) {
            var m1 = normStr(items[j].name||items[j].title||''), m2 = normStr(items[j].origin_name||items[j].original_name||'');
    ...
    function extractSeasonNum(name, slug) {
        var m = name.match(/season\s*(\d+)/i) || name.match(/phần\s*(\d+)/i) || slug.match(/season-(\d+)/i) || slug.match(/phan-(\d+)/i) || name.match(/S(\d+)/);
    ...
    async function findAllSeasonSlugs(source, title, origTitle) {
        ...
        try {
            ...
            var nT = normStr(title), nO = normStr(origTitle);
            for (var i = 0; i < items.length; i++) {
                var it = items[i]; if (!it.slug) continue;
    ...
    async function playViaTS(stream, title, poster, fileIdx) {
        ...
        try {
            ...
            var info = null, rt = 0;
            while (rt < 3) {
                try { var r2 = await fetch(u, { method:'POST', headers:tsHdr(), body:JSON.stringify({ action:'get', hash:hash }) }); info = await r2.json(); if (info && info.file_stats && info.file_stats.length) break; } catch(e) {}
                rt++; await delay(1500);
            }
            var files = [];
            ...
            if (!files.length) Lampa.Player.play({ title:title, url:tsUrl('/stream/fname?link='+hash+'&index=0&play') });
                ...
                Lampa.Select.show({ title:'Chọn file', items: files.map(function(f){ return { title:(f.path||'').split('/').pop()+(f.length?' ('+(f.length/1048576).toFixed(0)+'MB)':''), value:f }; }),
                    onSelect: function(a){ Lampa.Player.play({ title:title, url:tsUrl('/stream/fname?link='+hash+'&index='+a.value.id+'&play') }); },
    ...
    function aioStreamUrl(type, imdbId, s, e) {
        var base = cleanAioBase(getAioUrl()); if (!base) return '';
        var t = type==='tv' ? 'series' : 'movie', id = imdbId;
        if (type==='tv' && s && e) id = imdbId+':'+s+':'+e;
    ...
    function showStreamResults(streams, title, poster) {
        ...
        Lampa.Select.show({
            ...
            items: streams.slice(0,40).map(function(s){ return { title: formatStreamLabel(s), value: s }; }),
            onSelect: function(a) {
                var s = a.value;
    ...
    async function openTorrentMovie(tmdbId, title, poster, imdbId) {
        ...
        try { var imdb = imdbId || await getImdbId('movie', tmdbId); if (!imdb) { Lampa.Noty.show('Không tìm thấy IMDB ID'); return; } var streams = await fetchStreams('movie', imdb); if (!streams.length) { Lampa.Noty.show('Không có torrent'); return; } showStreamResults(streams, title, poster); } catch(e) { Lampa.Noty.show('Lỗi: '+(e.message||'')); }
    }
    async function openTorrentTV(tmdbId, title, poster, imdbId) {
        Lampa.Noty.show('Tải season...');
        try {
            var imdb = imdbId || await getImdbId('tv', tmdbId); if (!imdb) { Lampa.Noty.show('Không tìm thấy IMDB ID'); return; }
    ...
    function pickTorrentEp(season, imdb, title, poster) {
        var items = [];
        for (var i = 1; i <= (season.episode_count||1); i++) items.push({ title:'S'+pad(season.season_number)+'E'+pad(i), value:{s:season.season_number,e:i} });
        Lampa.Select.show({ title:season.name, items:items, onSelect:async function(a){
            var label = title+' S'+pad(a.value.s)+'E'+pad(a.value.e); Lampa.Noty.show('Tìm '+label+'...');
            try { var streams = await fetchStreams('tv', imdb, a.value.s, a.value.e); if (!streams.length) { Lampa.Noty.show('Không có torrent'); return; } showStreamResults(streams, label, poster); } catch(e) { Lampa.Noty.show('Lỗi: '+(e.message||'')); }
        }, onBack:function(){ Lampa.Controller.toggle('content'); } });
    }

    /* ── Cards ── */
    function mkPeople(list, key, clickable) {
        return (list || []).map(function(p) {
            var av = p.profile_path ? '<img src="'+TMDB_IMG_W500+p.profile_path+'">' : '<div class="kk-cast-empty"></div>';
            var cls = (clickable && p.id) ? ' selector" data-person-id="'+p.id+'"' : '"';
            return '<div class="kk-cast-card'+cls+'><div class="kk-cast-img">'+av+'</div><div class="kk-cast-name">'+esc(p.name||'')+'</div><div class="kk-cast-role">'+esc(p[key]||'')+'</div></div>';
        }).join('');
    }
    function bindCastClicks(container) {
        container.find('.kk-cast-card[data-person-id]').each(function(){
            var card = $(this);
            bindEnter(card, function(){ var pid = card.attr('data-person-id'); if (pid) Lampa.Activity.push({ url:'', title:card.find('.kk-cast-name').text()||'Diễn viên', component:'kkphim_person', person_id:parseInt(pid), page:1 }); });
        });
    }
    ...
    function mkCard(item) {
        ...
        var c = $('<div class="kk-card selector">' +
            '<div class="kk-card-img"><img src="'+p+'">' +
            (n.quality ? '<div class="kk-card-q">'+esc(n.quality)+'</div>' : '') +
            (n.episode_current ? '<div class="kk-card-ep">'+esc(n.episode_current)+'</div>' : '') +
            '</div>' +
            '<div class="kk-card-name">'+esc(n.name)+'</div>' +
            '<div class="kk-card-year">'+esc(n.year)+'</div>' +
            '</div>');
    ...
    function mkTmdbCard(item) {
        ...
        var imgH = d.poster_url ? '<img src="'+d.poster_url+'">' : '<div style="width:100%;height:100%;background:#333"></div>';
        var c = $('<div class="kk-card selector">' +
            '<div class="kk-card-img">' + imgH +
            (d.vote ? '<div class="kk-card-q">⭐'+esc(d.vote)+'</div>' : '') +
            (d.media_type==='tv' ? '<div class="kk-card-ep">TV</div>' : '') +
            '</div>' +
            '<div class="kk-card-name">'+esc(d.name)+'</div>' +
            '<div class="kk-card-year">'+esc(d.year)+'</div>' +
            '</div>');
    ...
    function makeGridComp(name, fetchItems, titleFn) {
        Lampa.Component.add(name, function(obj) {
            ...
            this.create = function() {
                ...
                scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+esc(titleFn(obj))+'</div>').append(grid).append(lm));
                bindEnter(lm, function(){ if (!loading && hasMore) doLoad(); });
                doLoad();
            };
            function doLoad() {
                loading = true; lm.text('Đang tải...');
                fetchItems(obj, page).then(function(items) {
                    if (!items.length) { hasMore = false; lm.text(page<=1 ? 'Không có kết quả' : 'Hết'); }
                    else { items.forEach(function(i){ grid.append(mkTmdbCard(i).addClass('kk-card--grid')); }); page++; lm.text('Tải thêm'); }
    ...
    function mkSrcBtn(cssClass, line1, line2) {
        return $('<div class="kk-src-btn '+cssClass+' selector">' +
            '<div class="kk-sb-main">'+line1+' <span class="kk-arrow">▼</span></div>' +
            (line2 ? '<div class="kk-sb-sub">'+line2+'</div>' : '') +
            '</div>');
    }

    function buildMovieExpand(srcKey, srcName, slug, title, cssClass) {
        var wrap = $('<div style="width:100%"></div>');
        var btn  = mkSrcBtn(cssClass, '▶ '+esc(srcName), 'Bấm để xem');
        ...
        bindEnter(btn, function() {
            ...
            if (open && !loaded) {
                loaded = true; box.html('<div class="kk-ep-ld">⏳ Đang tải...</div>');
                fetchDetail(SOURCES[srcKey], slug).then(function(det) {
                    if (!det || !det.episodes || !det.episodes.length) { box.html('<div class="kk-ep-er">❌ Không có tập</div>'); return; }
                    box.empty(); fillEps(box, det.episodes, title);
    ...
    function buildTVExpand(srcKey, srcName, slug, title, origTitle, cssClass) {
        ...
        bindEnter(btn, function() {
            open = !open; btn.toggleClass('kk-open', open); box.toggleClass('kk-show', open);
            if (open && !loaded) {
                loaded = true; box.html('<div class="kk-ep-ld">⏳ Đang tìm seasons...</div>');
    ...
    function buildDetailExpand(episodes, title, srcName, cssClass) {
        ...
        var box = $('<div class="kk-ep-box"></div>');
        wrap.append(btn).append(box);
        if (!episodes || !episodes.length || total===0) {
            btn.removeClass(cssClass).addClass('kk-src-btn--no');
            btn.html('⚠️ '+esc(srcName)+' - Không có tập');
            return wrap;
        }
        if (total===1) {
            ...
            if (ep) {
                ...
                btn.find('.kk-sb-main').html('▶ '+esc(srcName));
                btn.find('.kk-sb-sub').text('Phát ngay');
                btn.find('.kk-arrow').remove();
                bindEnter(btn, function(){ if (link) Lampa.Player.play({ title:title, url:link }); else Lampa.Noty.show('Không có link'); });
                return wrap;
            }
        }
        ...
        for (var i = 0; i < entries.length; i++) {
            try {
                ...
                if (det && det.episodes && det.episodes.length) {
                    container.empty();
                    if (backFn) { var bk = $('<div class="kk-ep-bk selector">← Quay lại</div>'); bindEnter(bk, backFn); container.append(bk); }
                    fillEps(container, det.episodes, title+' S'+pad(sNum));
                    return;
                }
            } catch(e) {}
        }
    ...
    function fillEps(container, episodes, title) {
        episodes.forEach(function(sv) {
            var sName = sv.server_name||'Server', cnt = (sv.server_data||[]).length, icon = '📺', sn = sName.toLowerCase();
    ...
    function injectCSS() {
        ...
        var link = document.createElement('link'); link.id='kk-css'; link.rel='stylesheet'; link.href=CSS_URL;
    ...
    function startPlugin() {
        ...
        Lampa.Component.add('kkphim_settings', function() {
            ...
            this.create = function() {
                ...
                Object.keys(SOURCES).forEach(function(k){ var src=SOURCES[k]; g0.append(mkOpt(src.name,'API: '+src.api,cur===k,function(){ saveSettings({source:k}); Lampa.Noty.show(src.name); comp.create(); })); });
                w.append(g0);

                var g5 = mkGroup('🎨 Giao diện'); var curGrid = s.card_style||'3';
                [{k:'2',n:'2 cột'},{k:'3',n:'3 cột'},{k:'4',n:'4 cột'}].forEach(function(opt){ g5.append(mkOpt(opt.n,'Số cột',curGrid===opt.k,function(){ saveSettings({card_style:opt.k}); comp.create(); })); });
                w.append(g5);

                var g6 = mkGroup('🌐 Ngôn ngữ TMDB'); var curLang = s.tmdb_lang||'vi-VN';
                [{k:'vi-VN',n:'Tiếng Việt'},{k:'en-US',n:'English'},{k:'ja-JP',n:'日本語'},{k:'ko-KR',n:'한국어'},{k:'zh-CN',n:'中文'}].forEach(function(opt){ g6.append(mkOpt(opt.n,opt.k,curLang===opt.k,function(){ saveSettings({tmdb_lang:opt.k}); _genreCache={movie:null,tv:null}; comp.create(); })); });
                w.append(g6);
                ...
                bindEnter(tA, async function(){
                    ...
                    try {
                        ...
                        var d = await r.json();
                        if (d.streams && d.streams.length) console.log('[KKPhim] AIO raw[0]:', JSON.stringify(d.streams[0]));
                ...
                var cc = si('Xóa cache','Đặt lại genres','Xóa');
                bindEnter(cc, function(){ _genreCache={movie:null,tv:null}; Lampa.Noty.show('OK'); });
                g7.append(cc); w.append(g7);

                var g4 = $('<div class="kk-stg-group"></div>');
                var cl = si('🗑️ Xóa toàn bộ','Khôi phục mặc định','Xóa');
                cl.find('.kk-stg-value').css('color','#f87171');
                bindEnter(cl, function(){ localStorage.removeItem(SETTINGS_KEY); _genreCache={movie:null,tv:null}; Lampa.Activity.backward(); });
                g4.append(cl); w.append(g4);

                w.append('<div class="kk-stg-ver">KKPhim v2.9</div>');
            ...
            function mkInput(n,d,val,prompt,key,s){
                ...
                bindEnter(it, function(){
                    try { if (Lampa.Input && Lampa.Input.edit) { Lampa.Input.edit({title:prompt,value:s[key]||'',free:true,nosave:true},function(v){ v=(v||'').trim(); var o={}; o[key]=v; saveSettings(o); s[key]=v; it.find('.kk-stg-value').text(v||val); }); return; } } catch(e) {}
                    var v = window.prompt(prompt, s[key]||''); if (v!==null){ v=v.trim(); var o={}; o[key]=v; saveSettings(o); s[key]=v; it.find('.kk-stg-value').text(v||val); }
                });
                return it;
            }
            this.start = function(){ applyCtrl(scroll); enableScroll(scroll); };
        ...
        Lampa.Component.add('kkphim_person_films', function(obj) {
            ...
            this.create = function() {
                ...
                tmdbPersonCredits(personId).then(function(credits) {
                    if (credits && credits.cast) allFilms = credits.cast.filter(function(c){ return c.media_type===filmType; }).sort(function(a,b){ return (b.popularity||0)-(a.popularity||0); });
                    renderMore(); comp.activity.loader(false); comp.start();
        ...
        Lampa.Component.add('kkphim_main', function() {
            ...
            this.create = function() {
                ...
                Object.keys(SOURCES).forEach(function(k){
                    ...
                    bindEnter(btn,function(){ if(on)return; saveSettings({source:k}); comp.create(); });
                    sb.append(btn);
                });
                ...
                sb.append(tmdbBtn); scroll.append(sb);
                var loaded = 0;
        ...
        Lampa.Component.add('kkphim_tmdb_genre', function(obj) {
            ...
            this.create = function() {
                comp.activity.loader(true); clearScroll(scroll);
                grid.css('grid-template-columns','repeat('+getCardStyle()+',minmax(0,1fr))');
                var genreBar = $('<div class="kk-genre-bar"></div>');
                scroll.append(genreBar);
                scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title" id="kk-gtitle">'+esc(obj.title||'Thể loại')+'</div>').append(grid));
                ...
                Promise.all([loadGenres('movie'), loadGenres('tv')]).then(function(res) {
                    ...
                    var cur = merged.find(function(g){ return String(g.id)===curGid; });
                    if (cur) scroll.render().find('#kk-gtitle').text(cur.name);
                    doLoad();
                }).catch(function(){ doLoad(); });
            };
            function doLoad() {
                if (loading) return; loading = true; var promises = [];
        ...
        Lampa.Component.add('kkphim_tmdb_detail', function(obj) {
            ...
            this.create = function() {
                comp.activity.loader(true); clearScroll(scroll);
                if (!tmdbId) { comp.activity.loader(false); comp.start(); return; }
            ...
            async function buildDetail(tmdb, logos, slugs) {
                ...
                var hero = $('<div class="kk-hero">' +
                    ...
                    '<div class="kk-hero-info">'+logoH+tH+'<div class="kk-origin">'+esc(o)+'</div></div>' +
                    '</div></div></div>');
                var body = $('<div class="kk-body">' +
                    '<div class="kk-metas">' +
                    '<span class="kk-meta">⭐ '+esc(v)+'</span>' +
                ...
                if (simItems.length) {
                    var simSec = $('<div class="kk-section kk-similar"></div>').append('<div class="kk-block-title">Phim tương tự</div>');
                    ...
                    simSec.append(simList); dw.append(simSec);
                }
                try {
                    var recData  = await tmdbRecommendations(mediaType, tmdbId, 1);
                    var recItems = (recData.results||[]).slice(0,20);
                    if (recItems.length) {
                        var recSec  = $('<div class="kk-section kk-similar kk-section--last"></div>').append('<div class="kk-block-title">🎲 Gợi ý cho bạn</div>');
                        ...
                        recSec.append(recList); dw.append(recSec);
                    } else { dw.find('.kk-similar').last().addClass('kk-section--last'); }
                } catch(e) { dw.find('.kk-similar').last().addClass('kk-section--last'); }
        ...
        Lampa.Component.add('kkphim_category', function(obj) {
            ...
            function doLoad() {
                ...
                network.silent(url, hr, function(){ loading=false; lm.text('Lỗi'); comp.activity.loader(false); });
            }
            this.start=function(){ applyCtrl(scroll); enableScroll(scroll); };
            this.pause=function(){}; this.stop=function(){};
            this.render=function(){ return scroll.render(); };
        ...
        Lampa.Component.add('kkphim_detail', function(obj) {
            ...
            this.create = function() {
                ...
                network.silent(SRC_API()+'phim/'+movie.slug, function(res){ if(rendered)return; loadAll(norm(res.movie||res||{}), res.episodes||[]); }, function(){ comp.activity.loader(false); });
            };
            async function loadAll(data, episodes) {
                ...
                try {
                    ...
                    if (tid) { try { tmdb=await tmdbFetch('/'+tt+'/'+tid+'?language='+tLang()+'&append_to_response=credits,images'); } catch(e) {} try { logos=await tmdbFetch('/'+tt+'/'+tid+'/images'); } catch(e) {} }
            ...
            function build(data, episodes, tmdb, logos, ttype) {
                ...
                var tH = logoH ? '' : '<div class="kk-title">'+esc(t)+'</div>';
                var hero = $('<div class="kk-hero">' +
                    '<div class="kk-hero-bg"><img src="'+bk+'"><div class="kk-hero-mask"></div></div>' +
                    '<div class="kk-hero-bottom"><div class="kk-hero-flex">' +
                    '<div class="kk-hero-poster"><img src="'+ps+'"></div>' +
                    '<div class="kk-hero-info">'+logoH+tH+'<div class="kk-origin">'+esc(o2)+'</div></div>' +
                    '</div></div></div>');
                var body = $('<div class="kk-body">' +
                    '<div class="kk-metas">' +
                    '<span class="kk-meta">⭐ '+esc(v)+'</span>' +
                    (y     ? '<span class="kk-meta">📅 '+esc(y)+'</span>'    : '') +
                    (rt    ? '<span class="kk-meta">⏱ '+esc(rt)+'</span>'   : '') +
                    (epCur ? '<span class="kk-meta">🎬 '+esc(epCur)+'</span>': '') +
                    '</div>' +
                    '<div class="kk-genres">'+ghtml+'</div>' +
                    crewH +
                    '<div class="kk-desc">'+fmtTxt(d)+'</div>' +
                ...
                if (pCats && pCats.length && pCats[0] && pCats[0].slug) {
                    ...
                    var simList = $('<div class="kk-similar-list"><div class="kk-ep-ld">⏳</div></div>');
                    simSec.append(simList); dw.append(simSec);
                    fetch(SRC_API()+'v1/api/the-loai/'+pCats[0].slug+'?page=1').then(function(r){ return r.json(); }).then(function(res){
                        ...
                        simList.empty();
                        if (items.length) items.slice(0,20).forEach(function(i){ simList.append(mkCard(i)); });
                        else simList.append('<div class="kk-ep-ld">Không có</div>');
                    }).catch(function(){ simList.empty(); });
                }
                /* TMDB similar */
                if (hasTmdb) {
                    ...
                    var tSimList= $('<div class="kk-similar-list"><div class="kk-ep-ld">⏳</div></div>');
                    tSimSec.append(tSimList); dw.append(tSimSec);
                    tmdbSimilar(ttype, tid2, 1).then(function(res){
                        var items=(res.results||[]).slice(0,20); tSimList.empty();
                        if (items.length) items.forEach(function(i){ if(!i.media_type) i.media_type=ttype; tSimList.append(mkTmdbCard(i)); });
                        else tSimList.append('<div class="kk-ep-ld">Không có</div>');
                    }).catch(function(){ tSimList.empty(); });
                }
                scroll.append(dw);
            }
            this.start=function(){ applyCtrl(scroll); enableScroll(scroll); };
