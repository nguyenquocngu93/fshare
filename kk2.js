...
async function getBecauseWatchedItems(hist){
    ...
    try{
        if(hist.tmdb&&hist.tmdb.id){
            ...
            return (rr.results||[]).filter(function(i){return i&&i.id;}).slice(0,20).map(function(i){
    ...
    try{
        ...
        if(hist.category&&hist.category.length&&hist.category[0]&&hist.category[0].slug){
            ...
            if(r.ok){
                ...
                var items=((res&&res.data&&res.data.items)||(res&&res.items)||[]).map(nm).filter(function(i){
                    return i&&i.slug&&i.slug!==hist.slug;
                });
    ...
    try{
        ...
        if(r2.ok){
            ...
            var items2=((res2&&res2.data&&res2.data.items)||(res2&&res2.items)||[]).map(nm).filter(function(i){
                return i&&i.slug&&i.slug!==hist.slug;
            });
            return items2.slice(0,20);
        }
    }catch(e3){}
...
function mkCWCard(item){
    var n=nm(item);if(!n)return $('<div></div>');
    var p=fImg(item.thumb_url||item.poster_url||n.thumb_url||n.poster_url);
    var ep=item.ep_name||item.episode_current||'Tiếp tục xem';
    var percent=cwPercent(item);
    var remain='';
    if(item.duration&&item.progress&&item.duration>item.progress){
        remain=' · Còn '+fmtTime(item.duration-item.progress);
    }
    ...
    if(percent>0){
        h+='<div style="margin-top:8px;height:6px;background:rgba(255,255,255,.14);border-radius:999px;overflow:hidden;">'
          +'<div style="height:100%;width:'+percent+'%;background:#4ade80;border-radius:999px;"></div>'
          +'</div>';
    }
...
function bDetExp(eps,title,sn,css,movieData){
    var w=$('<div style="width:100%"></div>'),total=0;
    (eps||[]).forEach(function(sv){total+=(sv.server_data||[]).length;});
    ...
    if(total===1){
        ...
        if(ep){
            ...
            bE(btn,function(){
                if(link){
                    window.__kkphim_last_movie_for_progress={
                        item:movieData||{},
                        epName:ep.name||'Full',
                        link:link
                    };
        ...
        (sv.server_data||[]).forEach(function(ep){
            ...
            bE(chip,function(){
                if(link){
                    ...
                    saveContinue(movieData||{},ep.name||'Tập',link,0,0);
...
function startPlugin(){
    ...
    Lampa.Component.add('kkphim_settings',function(){
        ...
        this.create=function(){
            ...
            bE(cl,function(){localStorage.removeItem(STG_KEY);localStorage.removeItem(CW_KEY);localStorage.removeItem(HIST_KEY);_gc={movie:null,tv:null};applyFontScale();Lampa.Activity.backward();});
            g4.append(cl);w.append(g4);

            w.append('<div class="kk-stg-ver">KKPhim v3.2 Final</div>');
    ...
    Lampa.Component.add('kkphim_main',function(){
        ...
        this.create=function(){
            ...
            if(hist){
                getBecauseWatchedItems(hist).then(function(items){
                    if(items&&items.length){
                        ...
                        bE(moreRec,function(){
                            if(hist.tmdb&&hist.tmdb.id){
                                Lampa.Activity.push({
                                    url:'',
                                    title:'Gợi ý từ '+(hist.name||''),
                                    component:'kkphim_tmdb_list',
                                    listType:'trending',
                                    page_num:1
                                });
                            }else if(hist.category&&hist.category.length&&hist.category[0]&&hist.category[0].slug){
                                Lampa.Activity.push({
                                    url:'',
                                    title:'Gợi ý từ '+(hist.name||''),
                                    component:'kkphim_category',
                                    mode:'category',
                                    category_slug:hist.category[0].slug,
                                    page_num:1
                                });
                            }
                        });
                        ...
                        rowRec.append(
                            $('<div class="kk-row-head"></div>')
                                .append('<div class="kk-row-title">💡 Vì bạn đã xem "'+E(hist.name||'')+'" nên...</div>')
                                .append(moreRec)
                        );

                        if(hist.tmdb&&hist.tmdb.id)rowRec.append(mkRowList(items.slice(0,rowCount()),false));
                        else rowRec.append(mkRowList(items.slice(0,rowCount()),true));
        ...
        async function bDet(tmdb,logos,slugs){
            ...
            if(slugs.kkphim){
                if(mt==='movie')act.append(bMovExp('kkphim','KKPhim',slugs.kkphim,t,'kk-src-btn--kkphim',{
                    name:t,origin_name:o2,poster_url:ps,thumb_url:bk,tmdb:{id:tid,type:mt},category:tmdb.genres||[],type:mt==='tv'?'series':'single'
                }));
                else act.append(bTVExp('kkphim','KKPhim',slugs.kkphim,t,o2,'kk-src-btn--kkphim',{
                    name:t,origin_name:o2,poster_url:ps,thumb_url:bk,tmdb:{id:tid,type:mt},category:tmdb.genres||[],type:'series'
                }));
            ...
            if(slugs.ophim){
                ...
                else act.append(bTVExp('ophim','OPhim',slugs.ophim,t,o2,'kk-src-btn--ophim',{
                    name:t,origin_name:o2,poster_url:ps,thumb_url:bk,tmdb:{id:tid,type:mt},category:tmdb.genres||[],type:'series'
                }));
    ...
    Lampa.Component.add('kkphim_detail',function(obj){
        ...
        function bld(data,eps,tmdb,logos,tt){
            ...
            var dirsArr=[],castArr=[],logoH='',country='',tagline='',genreStr='';
            if(tmdb){
                ...
                tagline=tmdb.tagline||'';genreStr=(tmdb.genres||[]).slice(0,3).map(function(g){return g.name;}).join(' | ');
            ...
            var tH=logoH?'':'<div class="kk-title">'+E(t)+'</div>';
            var hero=mkHero(bk,ps,logoH,tH,o2,{tagline:tagline,vote:v,year:y,runtime:rt+(epCur?' · '+epCur:''),country:country,genres:genreStr,status:'',rated:''});
