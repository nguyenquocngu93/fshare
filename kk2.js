...
function exSn(name,slug){
    // Extract season number from name/slug
    var patterns=[
        /season\s*(\d+)/i,
        /phần\s*(\d+)/i,
        /mùa\s*(\d+)/i,
    ...
    // Check for trailing number that might be season
...
async function fSeasonSlugs(source,title,orig,targetSeason){
    ...
    try{
        ...
        for(var i=0;i<allItems.length;i++){
            ...
            // Also check slug similarity with existing results
        ...
        // If looking for specific season, filter
        if(targetSeason){
...
async function findTmdbId(item,type){
    ...
    try{
        ...
        // Fallback: return first result if close enough
        if(results.length){
...
// Card for KKPhim/OPhim items that redirect to TMDB
    ...
    bE(btn,function(){
        ...
        if(op&&!ld){
            ...
            fSeasonSlugs(source,title,orig,null).then(function(entries){
                ...
                // If TMDB has more seasons, add placeholders
...
// ══════════════════════════════════════════════════════════════
// MAIN COMPONENT - TMDB as single main tab with source rows
// ══════════════════════════════════════════════════════════════
Lampa.Component.add('kkphim_main',function(){
    ...
    this.create=function(){
        ...
        // Source rows (KKPhim/OPhim items → lead to TMDB detail)
...
// ══════════════════════════════════════════════════════════════
// SOURCE LIST - infinite scroll for source categories → TMDB card
// ══════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════
// TMDB LIST & SEARCH - infinite scroll
// ══════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════
// TMDB DETAIL - Improved source finding
// ══════════════════════════════════════════════════════════════
Lampa.Component.add('kkphim_tmdb_detail',function(obj){
    ...
    async function bDet(tmdb,logos,slugs,tmdbSeasons){
        ...
        // Source buttons - only enabled sources
        var enabledSources=getEnabledSources();
        ...
        Object.keys(enabledSources).forEach(function(sk){
            ...
            if(slugs[sk]){
                ...
                // Show N/A button with retry option
