(function () {
    ...
    async function loadGenres(type) {
        ...
        try {
            var r = await tmdbFetch('/genre/' + type + '/list?language=' + tLang());
    ...
    async function findAllSlugs(title, origTitle, year) {
        ...
        var terms = [title]; if (origTitle && origTitle !== title) terms.push(origTitle);
    ...
    function showEpList(sv, title) {
        Lampa.Select.show({
            title: (sv.server_name || 'Server') + ' (' + (sv.server_data || []).length + ' t\u1eadp)',
            items: (sv.server_data || []).map(function (ep) { return { title: ep.name || 'T\u1eadp', value: ep }; }),
            onSelect: function (a) {
    ...
    function showTioResults(streams, title, poster) {
        ...
        Lampa.Select.show({
            ...
            items: streams.slice(0, 40).map(function (s) {
                var l = s.name; if (s.size) l += ' | ' + s.size; if (s.seeds) l += ' | S:' + s.seeds; if (s.rawName) l += ' [' + s.rawName + ']';
    ...
    async function openTorrentTV(tmdbId, title, poster, imdbId) {
        ...
        try {
            ...
            if (seasons.length > 1) {
                Lampa.Select.show({
                    title: 'Ch\u1ecdn Season', items: seasons.map(function (s) { return { title: s.name + (s.episode_count ? ' (' + s.episode_count + ' t\u1eadp)' : ''), value: s }; }),
    ...
    function injectCSS() {
        ...
        css += '.kk-srcbtn{padding:.6em 1em;border-radius:.75em;font-size:.95em;font-weight:700;cursor:pointer;border:1px solid rgba(255,255,255,.1)}.kk-srcbtn--on{background:rgba(99,102,241,.25);border-color:rgba(99,102,241,.5);color:#c4b5fd}.kk-srcbtn--off{background:rgba(255,255,255,.06);color:rgba(255,255,255,.45)}.kk-srcbtn.focus{background:rgba(99,102,241,.4);color:#fff}';
        ...
        css += '.kk-row-list{display:flex;gap:.9em;overflow-x:auto;overflow-y:hidden;padding:0 1.5em .2em;-webkit-overflow-scrolling:touch}.kk-row-list::-webkit-scrollbar,.kk-cast-list::-webkit-scrollbar,.kk-similar-list::-webkit-scrollbar,.kk-genre-bar::-webkit-scrollbar{display:none}';
        ...
        css += '.kk-title{font-size:2.5em;line-height:1.05;font-weight:900;color:#fff;margin-bottom:.2em}.kk-origin{font-size:1.15em;line-height:1.45;color:rgba(255,255,255,.82)}';
        css += '.kk-body{position:relative;z-index:3;padding:1.4em 1.5em 0;background:#141414}';
        css += '.kk-metas{display:flex;flex-wrap:wrap;gap:.65em;margin:0 0 1.1em}.kk-meta{padding:.55em .95em;border-radius:.8em;background:rgba(255,255,255,.08);color:#fff;font-size:1.1em;font-weight:800}';
        ...
        // Genre bar
        css += '.kk-genre-bar{display:flex;gap:.5em;overflow-x:auto;padding:0 1.5em .8em;-webkit-overflow-scrolling:touch}';
        css += '.kk-genre-chip{flex:0 0 auto;padding:.55em 1em;border-radius:999px;font-size:.92em;font-weight:700;cursor:pointer;white-space:nowrap}';
        css += '.kk-genre-chip--on{background:rgba(99,102,241,.3);border:1px solid rgba(99,102,241,.5);color:#c4b5fd}';
        css += '.kk-genre-chip--off{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.55)}';
        css += '.kk-genre-chip.focus{background:rgba(99,102,241,.45);color:#fff}';
        // Type toggle
        css += '.kk-type-bar{display:flex;gap:.5em;padding:0 1.5em .5em}';
        css += '.kk-type-btn{padding:.5em .9em;border-radius:.7em;font-size:.9em;font-weight:700;cursor:pointer;border:1px solid rgba(255,255,255,.08)}';
        css += '.kk-type-btn--on{background:rgba(255,255,255,.12);color:#fff}.kk-type-btn--off{background:transparent;color:rgba(255,255,255,.4)}';
        css += '.kk-type-btn.focus{background:rgba(255,255,255,.2);color:#fff}';
        // Settings
        ...
        css += '.selector,.kk-act,.kk-row-more,.kk-loadmore,.kk-genre,.kk-card,.kk-btn,.kk-stg-item,.kk-srcbtn,.kk-genre-chip,.kk-type-btn{touch-action:manipulation;-webkit-tap-highlight-color:transparent}';
    ...
    function startPlugin() {
        ...
        Lampa.Component.add('kkphim_settings', function () {
            ...
            this.create = function () {
                ...
                Object.keys(SOURCES).forEach(function (k) {
                    var src = SOURCES[k], it = si(src.name, 'API: ' + src.api, cur === k ? '\u2705' : 'Ch\u1ecdn');
                ...
                [{ k: '2', n: '2 c\u1ed9t' }, { k: '3', n: '3 c\u1ed9t' }, { k: '4', n: '4 c\u1ed9t' }].forEach(function (opt) {
                    var it = si(opt.n, 'S\u1ed1 c\u1ed9t l\u01b0\u1edbi phim', curGrid === opt.k ? '\u2705' : 'Ch\u1ecdn');
                    if (curGrid === opt.k) it.find('.kk-stg-value').css('color', '#4ade80');
                    bindEnter(it, function () { saveSettings({ card_style: opt.k }); Lampa.Noty.show(opt.n); comp.create(); });
                    g5.append(it);
                });
                w.append(g5);
                ...
                [{ k: 'vi-VN', n: 'Ti\u1ebfng Vi\u1ec7t' }, { k: 'en-US', n: 'English' }, { k: 'ja-JP', n: '\u65e5\u672c\u8a9e' }, { k: 'ko-KR', n: '\ud55c\uad6d\uc5b4' }, { k: 'zh-CN', n: '\u4e2d\u6587' }].forEach(function (opt) {
                    var it = si(opt.n, opt.k, curLang === opt.k ? '\u2705' : 'Ch\u1ecdn');
                    if (curLang === opt.k) it.find('.kk-stg-value').css('color', '#4ade80');
                    bindEnter(it, function () { saveSettings({ tmdb_lang: opt.k }); _genreCache = { movie: null, tv: null }; Lampa.Noty.show(opt.n); comp.create(); });
                    g6.append(it);
                });
                w.append(g6);
                ...
                bindEnter(tii, function () { pi('Torrentio Config', s.torrentio_config || '', function (v) { v = (v || '').trim(); saveSettings({ torrentio_config: v }); s.torrentio_config = v; tii.find('.kk-stg-value').text(v ? 'C\u00f3' : 'M\u1eb7c \u0111\u1ecbnh'); }); });
                g2.append(tii);
                ...
                bindEnter(cc, function () { _genreCache = { movie: null, tv: null }; Lampa.Noty.show('\u0110\u00e3 x\u00f3a cache'); });
                g7.append(cc);
                w.append(g7);

                // Xóa
                var g4 = $('<div class="kk-stg-group"></div>');
                var cl = si('\ud83d\uddd1\ufe0f X\u00f3a to\u00e0n b\u1ed9 c\u00e0i \u0111\u1eb7t', 'Kh\u00f4i ph\u1ee5c m\u1eb7c \u0111\u1ecbnh', 'X\u00f3a');
                cl.find('.kk-stg-value').css('color', '#f87171');
                bindEnter(cl, function () { localStorage.removeItem(SETTINGS_KEY); _genreCache = { movie: null, tv: null }; Lampa.Noty.show('\u0110\u00e3 x\u00f3a'); Lampa.Activity.backward(); });
                g4.append(cl);
                w.append(g4);
        ...
        // ========== TMDB GENRE (with genre bar + type toggle) ==========
        Lampa.Component.add('kkphim_tmdb_genre', function (obj) {
            var scroll = new Lampa.Scroll({ mask: true, over: true }), comp = this;
            var page = obj.page_num || 1, curGenreId = obj.genre_id, curType = obj.genre_type || 'movie';
            var grid = $('<div class="kk-grid"></div>'), lm = $('<div class="kk-loadmore selector">T\u1ea3i th\u00eam</div>'), loading = false, hasMore = true;
            var genreBarEl = null, typeBarEl = null, gridWrap = null;
            ...
            this.create = function () {
                ...
                // Type toggle bar
                typeBarEl = $('<div class="kk-type-bar"></div>');
                ['movie', 'tv'].forEach(function (tp) {
                    var label = tp === 'movie' ? '\ud83c\udfac Phim l\u1ebb' : '\ud83d\udcfa Phim b\u1ed9';
                    var btn = $('<div class="kk-type-btn selector ' + (tp === curType ? 'kk-type-btn--on' : 'kk-type-btn--off') + '">' + label + '</div>');
                    bindEnter(btn, function () {
                        if (tp === curType) return;
                        Lampa.Activity.push({ url: '', title: obj.title || 'Th\u1ec3 lo\u1ea1i', component: 'kkphim_tmdb_genre', genre_id: curGenreId, genre_type: tp, page_num: 1 });
                    });
                    typeBarEl.append(btn);
                });
                scroll.append(typeBarEl);

                // Genre bar
                genreBarEl = $('<div class="kk-genre-bar"></div>');
                scroll.append(genreBarEl);

                gridWrap = $('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">' + esc(obj.title || 'Th\u1ec3 lo\u1ea1i') + '</div>').append(grid).append(lm);
                scroll.append(gridWrap);

                bindEnter(lm, function () { if (!loading && hasMore) doLoad(); });

                // Load genres then data
                loadGenres(curType).then(function (genres) {
                    genres.forEach(function (g) {
                        var on = String(g.id) === String(curGenreId);
                        var chip = $('<div class="kk-genre-chip selector ' + (on ? 'kk-genre-chip--on' : 'kk-genre-chip--off') + '">' + esc(g.name) + '</div>');
                        bindEnter(chip, function () {
                            Lampa.Activity.push({ url: '', title: g.name, component: 'kkphim_tmdb_genre', genre_id: g.id, genre_type: curType, page_num: 1 });
                        });
            ...
            function doLoad() {
                loading = true; lm.text('\u0110ang t\u1ea3i...');
                tmdbDiscover(curType, curGenreId, page).then(function (res) {
                    var items = res.results || [];
                    if (!items.length) { hasMore = false; lm.text('H\u1ebft'); comp.activity.loader(false); loading = false; comp.start(); return; }
                    items.forEach(function (i) { i.media_type = curType; grid.append(mkTmdbCard(i).addClass('kk-card--grid')); });
                    page++; loading = false; lm.text('T\u1ea3i th\u00eam'); comp.activity.loader(false); comp.start();
                }).catch(function () { loading = false; lm.text('L\u1ed7i'); comp.activity.loader(false); });
            }
        ...
        Lampa.Component.add('kkphim_tmdb_detail', function (obj) {
            ...
            async function buildTmdbDetail(tmdb, logos, slugs) {
                ...
                var ghtml = '';
                if (tmdb.genres && tmdb.genres.length) ghtml = tmdb.genres.map(function (g) { return '<span class="kk-genre selector" data-gid="' + g.id + '" data-gname="' + esc(g.name || '') + '" data-gtype="' + mediaType + '">' + esc(g.name || '') + '</span>'; }).join('');
                ...
                body.find('.kk-genre[data-gid]').each(function () {
                    var g = $(this);
                    bindEnter(g, function () { Lampa.Activity.push({ url: '', title: g.attr('data-gname') || 'Th\u1ec3 lo\u1ea1i', component: 'kkphim_tmdb_genre', genre_id: g.attr('data-gid'), genre_type: g.attr('data-gtype') || 'movie', page_num: 1 }); });
                });
                ...
                bindEnter(body.find('[data-action="play-movie"]'), async function () {
                    ...
                    try {
                        ...
                        if (!detail || !detail.episodes || !detail.episodes.length) { if (slugs.kkphim && slugs.ophim) { var ok = src.key === 'kkphim' ? 'ophim' : 'kkphim'; src = SOURCES[ok]; slug = slugs[ok]; detail = await fetchDetail(src, slug); } }
        ...
        Lampa.Component.add('kkphim_detail', function (obj) {
            ...
            async function loadAll(data, episodes) {
                ...
                try {
                    ...
                    if (tid) {
                        try { tmdb = await tmdbFetch('/' + tt + '/' + tid + '?language=' + tLang() + '&append_to_response=credits,images'); } catch (e) { try { tmdb = await tmdbFetch('/' + tt + '/' + tid + '?language=en-US&append_to_response=credits,images'); } catch (e2) {} }
