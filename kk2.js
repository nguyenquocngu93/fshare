/* KKPhim Plugin v4.5.4 - Fix labels + Search + Slug */
...
// FIX 1: Hàm xác định loại phim đúng từ item nguồn
function getItemType(item){
    var t=item.type||'';
    if(t==='series'||t==='tvshows'||t==='tv'||t==='hoathinh')return'tv';
    if(t==='single'||t==='movie')return'movie';
    // Fallback: kiểm tra episode_current và episode_total
...
// FIX 2: sSrc - search đúng API, hỗ trợ pagination
async function sSrc(source,kw,page){
    try{
        var pg=page||1;
        // Loại bỏ "page:X" syntax cũ nếu có
        var cleanKw=String(kw).replace(/\s+page:\d+$/,'').trim();
        var r=await fetch(source.api+'v1/api/tim-kiem?keyword='+encodeURIComponent(cleanKw)+'&page='+pg);
        if(!r.ok)return[];
        var d=await r.json();
...
async function fSlugs(title,orig,year){
    ...
    for(var i=0;i<terms.length;i++){
        for(var key in enabledSources){
            if(!r[key]){
                var items=await sSrc(enabledSources[key],terms[i],1);
...
function exSn(name,slug){
    ...
    var combined=(name+' '+slug).toLowerCase();
    for(var i=0;i<patterns.length;i++){
        var m=combined.match(patterns[i]);
        if(m)return parseInt(m[1]);
    }
    // Số cuối trong slug: chỉ lấy nếu slug kết thúc bằng -2, -3... (không phải năm)
...
// FIX 3: fSeasonSlugs - lọc chặt hơn, validate với TMDB seasons
async function fSeasonSlugs(source,title,orig,targetSeason,tmdbTotalSeasons){
    ...
    try{
        ...
        for(var t=0;t<searchTerms.length;t++){
            var items=await sSrc(source,searchTerms[t],1);
        ...
        // Tìm slug gốc (season 1) trước để làm base
        var baseSlug=null;
        var baseScore=0;
        for(var i=0;i<allItems.length;i++){
            var it=allItems[i];
            var n1=nStr(it.name||it.title||'');
            var n2=nStr(it.origin_name||it.original_name||'');
            var sc=mScore(it,title,orig,it.year);
            if(sc>baseScore){baseScore=sc;baseSlug=it.slug;}
        }

        // Lọc các item có tên khớp với title/orig
        for(var i=0;i<allItems.length;i++){
            var it=allItems[i];
            if(!it.slug)continue;
            var n1=nStr(it.name||it.title||'');
            ...
            // Kiểm tra tên khớp chặt
            var nameMatch=false;
            if(nT){
                // Exact match hoặc n1/n2 chứa nT (loại bỏ số season)
                var n1Base=n1.replace(/\s*(season|phan|mua|phần|mùa)\s*\d+/gi,'').trim();
                var n2Base=n2.replace(/\s*(season|phan|mua|phần|mùa)\s*\d+/gi,'').trim();
                if(n1===nT||n2===nT||n1Base===nT||n2Base===nT)nameMatch=true;
                // Cho phép n1 chứa nT (nhưng nT phải dài ít nhất 4 ký tự)
                if(!nameMatch&&nT.length>=4&&(n1.indexOf(nT)===0||n2.indexOf(nT)===0))nameMatch=true;
            }
            if(!nameMatch&&nO&&nO.length>=3){
                var n1Base2=n1.replace(/\s*(season|phan|mua|phần|mùa)\s*\d+/gi,'').trim();
                var n2Base2=n2.replace(/\s*(season|phan|mua|phần|mùa)\s*\d+/gi,'').trim();
                if(n1===nO||n2===nO||n1Base2===nO||n2Base2===nO)nameMatch=true;
                if(!nameMatch&&nO.length>=4&&(n1.indexOf(nO)===0||n2.indexOf(nO)===0))nameMatch=true;
            }

            // Kiểm tra slug liên quan (cùng base slug)
            if(!nameMatch&&baseSlug){
                var baseNorm=nStr(baseSlug).replace(/-\d+$/,'');
                var itNorm=nStr(it.slug).replace(/-\d+$/,'');
                // Slug của item phải bắt đầu bằng base slug (không tính số cuối)
                if(itNorm===baseNorm||it.slug.indexOf(baseSlug.replace(/-\d+$/,''))===0)nameMatch=true;
            }
            ...
            // FIX: Validate season number với TMDB
            // Nếu biết tổng số season của TMDB, không thêm season > tmdbTotalSeasons
            if(tmdbTotalSeasons&&sn>tmdbTotalSeasons)continue;

            results.push({slug:it.slug,name:it.name||it.title||'',season:sn,source:source,year:it.year||''});
        }

        // Sort by season number
        results.sort(function(a,b){return a.season-b.season;});

        // Deduplicate by season number - giữ item có score cao nhất
        var snSeen={};
        results=results.filter(function(r){
            if(snSeen[r.season])return false;
            snSeen[r.season]=true;
            return true;
        });
    ...
    }catch(e){console.error('fSeasonSlugs error:',e);}
    return results;
}
...
// FIX 1: mkTC, mkTCH, mkTCPF - label type đúng cho TMDB cards (đã đúng, media_type từ TMDB)
...
// FIX 1: mkSourceCard + mkSourceCardH - dùng getTypeLabel() để lấy nhãn đúng
function mkSourceCard(item,source){
    var p=fImg(item.poster_url||item.thumb_url,source);
    var typeLabel=getTypeLabel(item); // FIX: dùng hàm mới
...
// FIX 3: bTVExp - truyền tmdbTotalSeasons vào fSeasonSlugs
function bTVExp(sk,sn,slug,title,orig,css,tmdbSeasons){
    ...
    // Tính tổng số season từ TMDB để validate
    ...
    bE(btn,function(){
        ...
        if(op&&!ld){
            ...
            // FIX: truyền tmdbTotalSeasons để giới hạn season tìm được
            fSeasonSlugs(source,title,orig,null,tmdbTotalSeasons).then(function(entries){
                ...
                // Thêm seasons từ TMDB nếu chưa có (đánh dấu notFound)
...
// FIX 2: kkphim_source_search - search đúng API
Lampa.Component.add('kkphim_source_search',function(obj){
    ...
    function doL(){
        ...
        // FIX: sSrc(source, keyword, page) - không dùng "page:" syntax
        sSrc(source,kw,page).then(function(items){
            spinner.hide();
            if(!items||!items.length){
                done=true;
                endMsg.show().text(page<=1?'Không tìm thấy':'Đã hết');
    ...
    this.create=function(){
        ...
        bE($(tb.find('.kk-btn')[0]),function(){
            try{
                if(Lampa.Input&&Lampa.Input.edit){
                    Lampa.Input.edit({title:'Tìm kiếm',value:'',free:true},function(kw){if(kw)searchInCurrentTab(kw.trim());});
                    return;
                }
            }catch(e){}
        ...
        function searchInCurrentTab(kw){
            if(currentTab==='tmdb'){
                ...
                // FIX 2: search nguồn dùng component kkphim_source_search đúng
...
Lampa.Component.add('kkphim_tmdb_detail',function(obj){
    ...
    async function bDet(tmdb,logos,slugs,tmdbSeasons,keywords){
        ...
        // FIX 3: truyền tmdbSeasons vào bTVExp
        var tmdbTotalSeasons=tmdbSeasons&&tmdbSeasons.length?tmdbSeasons.length:null;
