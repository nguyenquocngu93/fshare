(function () {
    ...
    var SOURCES = {
        kkphim: { key: 'kkphim', name: 'KKPhim', api: 'https://phimapi.com/', img: 'https://phimimg.com/', color: '#a78bfa', icon: '🎬' },
        ophim: { key: 'ophim', name: 'OPhim', api: 'https://ophim1.com/', img: 'https://img.ophim.live/uploads/movies/', color: '#4ade80', icon: '🎥' }
    };
    ...
    function getSubdlKey(){return loadSettings().subdl_api_key||'';}
    function getPlayerMode(){return loadSettings().player_mode||'lampa';}
    ...
    async function playViaTS(stream,title,poster,fileIdx,movieData,ttype,season,episode){
        ...
        try{var td=await tsAdd(buildMag(stream.infoHash),title,poster);var hash=td.hash||stream.infoHash;await delay(2000);
        ...
        await playUrlWithSub(playUrl,title,movieData,ttype,season,episode);}catch(e){Lampa.Noty.show('Lỗi TS: '+(e.message||''));}
    }
    ...
    function stopSubOL(){var el=document.getElementById('kk-sub-overlay');if(el){el.innerHTML='';el.style.display='none';}}

    function playInternal(url,title,subUrl,subLabel){
        stopSubOL();Lampa.Player.play({title:title,url:url});if(!subUrl)return;
        var n=0,t=setInterval(function(){n++;if(n>40){clearInterval(t);return;}var v=document.querySelector('video');if(!v||!v.src)return;clearInterval(t);Array.from(v.querySelectorAll('track[data-kk]')).forEach(function(x){x.remove();});var tk=document.createElement('track');tk.kind='subtitles';tk.label=subLabel||'Sub';tk.srclang='vi';tk.src=subUrl;tk.setAttribute('data-kk','1');tk.default=true;v.appendChild(tk);setTimeout(function(){try{for(var i=0;i<v.textTracks.length;i++)if(v.textTracks[i].label===(subLabel||'Sub'))v.textTracks[i].mode='showing';}catch(e){}},800);Lampa.Noty.show('📝 '+(subLabel||'Sub'));},500);
    }

    function showCopyName(movieData,tmdbData){
        var enTitle='',year='';if(tmdbData){enTitle=tmdbData.original_title||tmdbData.original_name||tmdbData.title||tmdbData.name||'';if(tmdbData.release_date)year=tmdbData.release_date.slice(0,4);else if(tmdbData.first_air_date)year=tmdbData.first_air_date.slice(0,4);}
        if(!enTitle){enTitle=movieData.origin_name||movieData.name||'';year=movieData.year||'';}
        Lampa.Select.show({title:'🔍 Copy tên',items:[{title:'📋 '+enTitle+(year?' '+year:''),value:enTitle+(year?' '+year:'')},{title:'📋 '+enTitle+(year?' ('+year+')':''),value:enTitle+(year?' ('+year+')':'')},{title:'📋 '+enTitle,value:enTitle}],onSelect:function(a){copyToClipboard(a.value);Lampa.Noty.show('✅ '+a.value);},onBack:function(){Lampa.Controller.toggle('content');}});
    }
    ...
    async function showSubMenu(url,title,movieData,ttype,season,episode,tmdbData){
        ...
        var enTitle=tmdbData?(tmdbData.original_title||tmdbData.original_name||''):(movieData.origin_name||movieData.name||'');
    ...
    async function findOnSource(srcKey,tmdbId,nameVi,nameEn){
        ...
        for(var qi=0;qi<queries.length;qi++){try{var url=src.api+'v1/api/tim-kiem?keyword='+encodeURIComponent(queries[qi])+'&page=1';var r=await fetch(url);if(!r.ok)continue;var d=await r.json();var items=(d&&d.data&&d.data.items)||(d&&d.items)||[];
        for(var i=0;i<items.length;i++){var it=items[i];if(!it||!it.slug)continue;if(it.tmdb&&it.tmdb.id&&String(it.tmdb.id)===String(tmdbId))return{slug:it.slug,source:srcKey};var iN=(it.origin_name||'').toLowerCase().trim();var iN2=(it.name||'').toLowerCase().trim();var qL=queries[qi].toLowerCase().trim();if(iN===qL||iN2===qL)return{slug:it.slug,source:srcKey};}}catch(e){}}
        return null;
    }
    async function fetchEpisodesFromSource(srcKey,slug){var src=SOURCES[srcKey];if(!src||!slug)return null;try{var r=await fetch(src.api+'phim/'+slug);if(!r.ok)return null;var d=await r.json();return d.episodes||[];}catch(e){return null;}}
    async function findAllSources(tmdbId,ttype,nameVi,nameEn){
        var results=[];var keys=Object.keys(SOURCES);
        var promises=keys.map(function(k){return findOnSource(k,tmdbId,nameVi,nameEn).then(function(found){if(found)return fetchEpisodesFromSource(found.source,found.slug).then(function(eps){if(eps&&eps.length)results.push({source:found.source,sourceName:SOURCES[found.source].name,sourceIcon:SOURCES[found.source].icon,slug:found.slug,episodes:eps});});}).catch(function(){});});
    ...
    async function fetchStreams(type,imdbId,s,e){var url=buildStreamUrl(type,imdbId,s,e);var r=await fetch(url);if(!r.ok)throw new Error('HTTP '+r.status);var d=await r.json();var streams=(d.streams||[]).map(parseStreamItem);var qOrder={'2160P':0,'4K':0,'UHD':0,'1080P':1,'720P':2,'480P':3};streams.sort(function(a,b){if(a.hasUrl&&!b.hasUrl)return-1;if(!a.hasUrl&&b.hasUrl)return 1;var qa=qOrder[a.quality]!==undefined?qOrder[a.quality]:9;var qb=qOrder[b.quality]!==undefined?qOrder[b.quality]:9;if(qa!==qb)return qa-qb;return(b.seeds||0)-(a.seeds||0);});return streams;}
    ...
    async function openStreamSearch(tmdbId,ttype,data,episodes,poster,tmdbData){
        ...
        Lampa.Noty.show('Tìm stream...');try{var imdb=await getImdbId(ttype,tmdbId);if(!imdb){Lampa.Noty.show('Không tìm IMDB ID');return;}var streams=await fetchStreams(ttype,imdb);if(!streams.length){Lampa.Noty.show('Không có stream');return;}showStreamResults(streams,data.name||'',poster,data,ttype,null,null,tmdbData);}catch(e){Lampa.Noty.show('Lỗi: '+(e.message||''));}
    }
    async function tvStreamPicker(tmdbId,data,episodes,poster,tmdbData){Lampa.Noty.show('Tải seasons...');var seasons=await getTmdbSeasons(tmdbId);var imdb=await getImdbId('tv',tmdbId);if(!imdb){Lampa.Noty.show('Không tìm IMDB ID');return;}if(seasons.length>1)pickSeason(seasons,imdb,data,poster,tmdbData);else if(seasons.length===1)pickEpFrom(seasons[0],imdb,data,poster,tmdbData);else pickEpFB(imdb,data,episodes,poster,tmdbData);}
    ...
    function isMovieSingle(episodes){if(!episodes||!episodes.length)return false;var total=0;episodes.forEach(function(sv){total+=(sv.server_data||[]).length;});return total<=episodes.length;}

    function playEpisode(ep,title,movieData,ttype,tmdbData){
        var link=ep.link_m3u8||ep.link_embed||'';if(!link){Lampa.Noty.show('Không có link');return;}
        var epNum=parseInt((ep.name||'').replace(/[^\d]/g,''))||null;
        playUrlWithSub(link,title+' - '+(ep.name||''),movieData,ttype,ttype==='tv'?1:null,epNum,tmdbData);
    }
    ...
    function mkTmdbCard(item){
        ...
        bindEnter(c,function(){Lampa.Activity.push({url:'',title:title,component:'kkphim_detail_tmdb',tmdb_id:item.id,tmdb_type:ttype,page:1});});
        return c;
    }

    // Build source play buttons for episode lists
    function buildSourceButtons(allSources,movieTitle,movieData,ttype,tmdbData){
        var wrap=$('<div></div>');
        allSources.forEach(function(src){
            var srcObj=SOURCES[src.source];
            var color=srcObj?srcObj.color:'#fff';
            var icon=srcObj?srcObj.icon:'📺';
            src.episodes.forEach(function(sv){
                var serverName=sv.server_name||'Server';
                var epData=sv.server_data||[];
                if(!epData.length)return;
                // Single episode (movie) → play button
                if(epData.length===1||(epData.length<=2&&isMovieSingle([sv]))){
                    var ep=epData[0];
                    var btn=$('<div class="kk-act-wrap"><div class="kk-act kk-act--src selector" style="background:'+color+'22;border:1px solid '+color+'44;color:'+color+'">'+icon+' '+esc(src.sourceName)+' · '+esc(serverName)+'</div></div>');
                    bindEnter(btn.find('.kk-act--src'),function(){playEpisode(ep,movieTitle,movieData,ttype,tmdbData);});
                    wrap.append(btn);
                }else{
                    // Multiple episodes → expand button
                    var btn2=$('<div class="kk-act-wrap"><div class="kk-act kk-act--src selector" style="background:'+color+'22;border:1px solid '+color+'44;color:'+color+'">'+icon+' '+esc(src.sourceName)+' · '+esc(serverName)+' ('+epData.length+' tập)</div></div>');
                    bindEnter(btn2.find('.kk-act--src'),function(){
                        Lampa.Select.show({
                            title:icon+' '+src.sourceName+' - '+serverName,
                            items:epData.map(function(ep){return{title:ep.name||'Tập',value:ep};}),
                            onSelect:function(a){playEpisode(a.value,movieTitle,movieData,ttype,tmdbData);},
    ...
    // Build episode buttons for kkphim/ophim detail
    function buildEpisodeSection(episodes,movieTitle,movieData,ttype,tmdbData){
        var wrap=$('<div></div>');
        if(!episodes||!episodes.length)return wrap;
        var single=isMovieSingle(episodes);
        episodes.forEach(function(sv){
            ...
            if(single){
                var ep=epData[0];
                var btn=$('<div class="kk-act-wrap"><div class="kk-act kk-act--play selector">▶ '+esc(serverName)+'</div></div>');
                bindEnter(btn.find('.kk-act--play'),function(){playEpisode(ep,movieTitle,movieData,ttype,tmdbData);});
                wrap.append(btn);
            }else{
                var secEl=$('<div class="kk-section"></div>');
                secEl.append('<div class="kk-server">'+esc(serverName)+'</div>');
                var g=$('<div class="kk-eps"></div>');
                epData.forEach(function(ep){
                    var b=$('<div class="kk-ep selector">'+esc(ep.name||'')+'</div>');
                    bindEnter(b,function(){playEpisode(ep,movieTitle,movieData,ttype,tmdbData);});
                    g.append(b);
                });
                secEl.append(g);wrap.append(secEl);
            }
        });
        return wrap;
    }
...
'.kk-btn{display:inline-flex;align-items:center;padding:.8em 1.1em;border-radius:999px;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.1);color:#fff;font-size:.98em;font-weight:800;cursor:pointer}.kk-btn.focus{background:#fff;color:#000}'+
...
'.kk-srcbtn.focus{background:rgba(99,102,241,.4);color:#fff}.kk-srcbtn--tmdb.focus{background:rgba(1,180,228,.35);color:#fff}'+
...
'.kk-row-more{font-size:.98em;font-weight:800;padding:.7em 1.1em;border-radius:999px;background:rgba(255,255,255,.08);color:#fff;cursor:pointer}.kk-row-more.focus{background:#fff;color:#000}'+
...
'.kk-card{flex:0 0 auto;width:9.5em;cursor:pointer}.kk-card--grid{width:100%}'+
...
'.kk-grid-wrap{padding:0 1.5em}.kk-grid-title{font-size:1.9em;font-weight:900;color:#fff;margin:0 0 .85em}'+
'.kk-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.9em}'+
'.kk-loadmore{margin-top:1.1em;text-align:center;padding:.9em;border-radius:.9em;background:rgba(255,255,255,.08);color:#fff;font-size:1em;font-weight:800;cursor:pointer}.kk-loadmore.focus{background:#ff2332}'+
...
'.kk-hero-bg{position:relative;height:25em}.kk-hero-bg img{width:100%;height:100%;object-fit:cover}'+
...
'.kk-logo{max-width:34em;margin:0 0 1em}.kk-logo img{max-width:100%;max-height:10em;object-fit:contain;filter:drop-shadow(0 .4em 1.1em rgba(0,0,0,.45))}'+
...
'.kk-genre{padding:.55em .95em;border-radius:.8em;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.95);font-size:1.02em;font-weight:700;cursor:pointer}.kk-genre.focus{background:rgba(255,255,255,.18)}'+
...
'.kk-actions{display:flex;flex-wrap:wrap;gap:.7em;padding:.1em 0 .2em}.kk-act-wrap{width:100%}'+
...
'.kk-act--src{border-radius:.95em;font-weight:800}.kk-act--src.focus{filter:brightness(1.3)}'+
...
'.kk-cast-img{width:6.5em;height:6.5em;border-radius:50%;overflow:hidden;background:#2b2b2b;margin:0 auto .65em;border:2px solid rgba(255,255,255,.08)}.kk-cast-img img{width:100%;height:100%;object-fit:cover}.kk-cast-empty{width:100%;height:100%;background:#333;border-radius:50%}'+
'.kk-cast-name{font-size:1em;font-weight:800;color:#fff}.kk-cast-role{font-size:.88em;color:rgba(255,255,255,.6);margin-top:.12em}'+
'.kk-server{font-size:1.12em;font-weight:800;color:#63d471;margin:.95em 0 .65em}'+
'.kk-eps{display:flex;flex-wrap:wrap;gap:.65em}'+
'.kk-ep{min-width:4.2em;text-align:center;padding:.8em 1em;border-radius:.75em;background:rgba(255,255,255,.09);color:#fff;font-size:1em;font-weight:800;cursor:pointer}.kk-ep.focus{background:#ff2233}'+
'.kk-similar{padding-bottom:1.1em}.kk-similar-list{display:flex;gap:.9em;overflow-x:auto;-webkit-overflow-scrolling:touch}.kk-similar-list .kk-card{width:8.5em}'+
'.kk-stg-wrap{padding:1.4em}'+
'.kk-stg-title{font-size:2em;font-weight:900;color:#fff;margin:0 0 .3em}'+
'.kk-stg-ver{font-size:.9em;color:rgba(255,255,255,.35);margin:0 0 1.3em}'+
...
'.kk-stg-status--ok{background:rgba(74,222,128,.12);color:#4ade80}.kk-stg-status--err{background:rgba(248,113,113,.12);color:#f87171}.kk-stg-status--loading{background:rgba(255,255,255,.06);color:rgba(255,255,255,.5)}'+
...
'.kk-genre-tag{padding:.5em .9em;border-radius:.7em;font-size:.9em;font-weight:700;cursor:pointer;border:1px solid rgba(1,180,228,.2);background:rgba(1,180,228,.08);color:#90cef4}.kk-genre-tag.focus{background:rgba(1,180,228,.25);color:#fff}.kk-genre-tag--active{background:rgba(1,180,228,.25);border-color:rgba(1,180,228,.5);color:#fff}'+
...
'.kk-about{margin-top:1em;padding:1.2em;border-radius:1em;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06)}'+
'.kk-about-title{font-size:1.1em;font-weight:800;color:#fff;margin-bottom:.5em}'+
'.kk-about-text{font-size:.92em;color:rgba(255,255,255,.4);line-height:1.6}'+
    ...
    function startPlugin(){
        ...
        Lampa.Component.add('kkphim_settings',function(){
            ...
            this.create=function(){
                ...
                w.append('<div class="kk-stg-ver">Phiên bản '+PLUGIN_VERSION+'</div>');

                // Source
                var g0=$('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">📺 Nguồn phim mặc định</div>');
                var cur=s.source||'ophim';
                Object.keys(SOURCES).forEach(function(k){var src=SOURCES[k],it=si(src.icon+' '+src.name,src.api,cur===k?'✅ Đang dùng':'Chọn');if(cur===k)it.find('.kk-stg-value').css('color','#4ade80');bindEnter(it,function(){saveSettings({source:k});Lampa.Noty.show(src.name);comp.create();});g0.append(it);});
                w.append(g0);

                // TorrServer
                var g1=$('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">🖥️ TorrServer</div>');
                var hi=si('Địa chỉ','VD: 192.168.1.100:8090',s.torrserver_host||'Chưa cấu hình');bindEnter(hi,function(){pi('Địa chỉ TorrServer',s.torrserver_host||'',function(v){v=(v||'').trim();saveSettings({torrserver_host:v});hi.find('.kk-stg-value').text(v||'Chưa cấu hình');});});g1.append(hi);
                var pwi=si('Mật khẩu','Để trống nếu không có',s.torrserver_password?'••••••':'Không');bindEnter(pwi,function(){pi('Mật khẩu TS',s.torrserver_password||'',function(v){v=(v||'').trim();saveSettings({torrserver_password:v});pwi.find('.kk-stg-value').text(v?'••••••':'Không');});});g1.append(pwi);
                var tst=si('🔌 Kiểm tra kết nối','Ping TorrServer','Nhấn');var st1=$('<div class="kk-stg-status" style="display:none"></div>');
                bindEnter(tst,async function(){if(!getTSHost()){st1.show().attr('class','kk-stg-status kk-stg-status--err').text('❌ Chưa nhập địa chỉ');return;}st1.show().attr('class','kk-stg-status kk-stg-status--loading').text('⏳ Đang kiểm tra...');try{var r=await fetch(tsUrl('/echo'),{headers:tsHdr()});st1.attr('class','kk-stg-status '+(r.ok?'kk-stg-status--ok':'kk-stg-status--err')).text(r.ok?'✅ Kết nối thành công!':'❌ Lỗi: '+r.status);}catch(e){st1.attr('class','kk-stg-status kk-stg-status--err').text('❌ '+e.message);}});
                g1.append(tst).append(st1);w.append(g1);

                // Stream
                var g2=$('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">🧲 Torrent Stream</div>');
                var cfg=getStreamBase();
                g2.append($('<div style="padding:0 1.1em .6em;font-size:.88em;font-weight:700;color:'+(cfg.source==='aio'?'#818cf8':'#fbbf24')+'">Đang dùng: '+(cfg.source==='aio'?'🌊 Aiostreams':'🧲 Torrentio')+'</div>'));
                var tii=si('🧲 Torrentio URL','Dán manifest URL từ torrentio.strem.fun',s.torrentio_config?'✅ Đã cấu hình':'Mặc định');bindEnter(tii,function(){pi('Torrentio manifest URL',s.torrentio_config||'',function(v){v=(v||'').trim();saveSettings({torrentio_config:v});comp.create();});});g2.append(tii);
                var aii=si('🌊 Aiostreams URL','Dán manifest URL (ưu tiên cao hơn)',s.aio_config?'✅ Đã cấu hình':'Chưa có');bindEnter(aii,function(){pi('Aiostreams manifest URL',s.aio_config||'',function(v){v=(v||'').trim();saveSettings({aio_config:v});comp.create();});});g2.append(aii);
                var tti=si('🧪 Test stream','Thử tìm stream cho Inception','Nhấn');var st2=$('<div class="kk-stg-status" style="display:none"></div>');
                bindEnter(tti,async function(){st2.show().attr('class','kk-stg-status kk-stg-status--loading').text('⏳ Đang thử...');try{var url=buildStreamUrl('movie','tt1375666');var r=await fetch(url);if(!r.ok){st2.attr('class','kk-stg-status kk-stg-status--err').text('❌ HTTP '+r.status);return;}var d=await r.json();st2.attr('class','kk-stg-status kk-stg-status--ok').text('✅ '+(d.streams||[]).length+' stream tìm thấy');}catch(e){st2.attr('class','kk-stg-status kk-stg-status--err').text('❌ '+e.message);}});
                g2.append(tti).append(st2);w.append(g2);

                // Subtitle
                var g3=$('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">📝 Phụ đề (SubDL)</div>');
                var subdlItem=si('API Key','Lấy tại subdl.com/panel',s.subdl_api_key?'✅ Đã có':'❌ Chưa có');if(!s.subdl_api_key)subdlItem.find('.kk-stg-value').css('color','#f87171');
                bindEnter(subdlItem,function(){pi('SubDL API Key',s.subdl_api_key||'',function(v){v=(v||'').trim();saveSettings({subdl_api_key:v});comp.create();});});g3.append(subdlItem);
                var subM=s.sub_mode||'ask';
                [{k:'ask',n:'🤔 Hỏi mỗi lần',d:'Hiện danh sách sub để chọn'},{k:'auto',n:'⚡ Tự động',d:'Tự chọn sub Việt/Anh tốt nhất'},{k:'off',n:'🚫 Tắt',d:'Không dùng phụ đề'}].forEach(function(sm){var it=si(sm.n,sm.d,subM===sm.k?'✅':'');if(subM===sm.k)it.find('.kk-stg-value').css('color','#4ade80');bindEnter(it,function(){saveSettings({sub_mode:sm.k});Lampa.Noty.show(sm.n);comp.create();});g3.append(it);});
                var tsi=si('🧪 Test phụ đề','Thử tìm sub cho Inception','Nhấn');var st3=$('<div class="kk-stg-status" style="display:none"></div>');
                bindEnter(tsi,async function(){st3.show().attr('class','kk-stg-status kk-stg-status--loading').text('⏳ Đang tìm...');try{var subs=await searchSubs('tt1375666',null,'movie',null,null,'Inception');st3.attr('class','kk-stg-status '+(subs.length?'kk-stg-status--ok':'kk-stg-status--err')).text(subs.length?'✅ '+subs.length+' phụ đề tìm thấy':(getSubdlKey()?'❌ Không tìm thấy':'❌ Cần nhập API Key'));}catch(e){st3.attr('class','kk-stg-status kk-stg-status--err').text('❌ '+e.message);}});
                g3.append(tsi).append(st3);w.append(g3);

                // Data management
                var g5=$('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">🗄️ Dữ liệu</div>');
                var clearCache=si('🧹 Xóa cache','Xóa bộ nhớ tạm tìm kiếm','Nhấn');
                bindEnter(clearCache,function(){_tmdbCache={};_subCache={};Lampa.Noty.show('✅ Đã xóa cache');});g5.append(clearCache);
                var cl=si('🗑️ Xóa toàn bộ cài đặt','Reset về mặc định','Xóa');cl.find('.kk-stg-value').css('color','#f87171');
                bindEnter(cl,function(){Lampa.Select.show({title:'⚠️ Xác nhận',items:[{title:'✅ Xóa hết',value:true},{title:'❌ Hủy',value:false}],onSelect:function(a){if(a.value){localStorage.removeItem(SETTINGS_KEY);Lampa.Noty.show('Đã xóa');Lampa.Activity.backward();}},onBack:function(){Lampa.Controller.toggle('content');}});});
                g5.append(cl);w.append(g5);

                // About
                w.append($('<div class="kk-about"><div class="kk-about-title">ℹ️ Về KKPhim Plugin</div><div class="kk-about-text">Plugin xem phim cho Lampa với nguồn KKPhim, OPhim, TMDB.<br>Hỗ trợ Torrentio, Aiostreams, TorrServer.<br>Phụ đề tự động qua SubDL.<br><br>Version: '+PLUGIN_VERSION+'</div></div>'));
        ...
        Lampa.Component.add('kkphim_main',function(){
            ...
            this.create=function(){
                ...
                bindEnter($(tb.find('.kk-btn')[0]),openSearch);bindEnter($(tb.find('.kk-btn')[1]),function(){Lampa.Activity.push({url:'',title:'Cài đặt',component:'kkphim_settings'});});
                scroll.append(tb);
                ...
                var tmdbOn=_tab==='tmdb';var tmdbBtn=$('<div class="kk-srcbtn kk-srcbtn--tmdb selector '+(tmdbOn?'kk-srcbtn--on':'kk-srcbtn--off')+'">TMDB</div>');bindEnter(tmdbBtn,function(){if(tmdbOn)return;saveSettings({_main_tab:'tmdb'});comp.create();});sb.append(tmdbBtn);
                scroll.append(sb);
            ...
            function buildSourceHome(){var loaded=0;cats.forEach(function(cat){network.silent(SRC_API()+cat.api+'?page=1',function(res){var list=((res&&res.items)||(res&&res.data&&res.data.items)||[]).map(norm).filter(function(i){return i&&i.slug;});if(list.length){var row=$('<div class="kk-row"></div>');var more=$('<div class="kk-row-more selector">Xem thêm</div>');var rl=$('<div class="kk-row-list"></div>');bindEnter(more,function(){Lampa.Activity.push({url:'',title:cat.name,component:'kkphim_category',cat:cat,page_num:1});});list.slice(0,12).forEach(function(i){rl.append(mkCard(i));});row.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">'+esc(cat.name)+'</div>').append(more)).append(rl);scroll.append(row);}loaded++;if(loaded>=cats.length){comp.activity.loader(false);comp.start();}},function(){loaded++;if(loaded>=cats.length){comp.activity.loader(false);comp.start();}});});}
            function buildTmdbHome(){var tmdbRows=[{name:'🔥 Xu hướng phim',fn:function(){return tmdbTrending('movie',1);},type:'movie',lt:'trending'},{name:'🔥 Xu hướng TV',fn:function(){return tmdbTrending('tv',1);},type:'tv',lt:'trending'},{name:'⭐ Phim phổ biến',fn:function(){return tmdbPopular('movie',1);},type:'movie',lt:'popular'},{name:'⭐ TV phổ biến',fn:function(){return tmdbPopular('tv',1);},type:'tv',lt:'popular'},{name:'🏆 Đánh giá cao',fn:function(){return tmdbTopRated('movie',1);},type:'movie',lt:'top_rated'},{name:'💥 Hành Động',fn:function(){return tmdbDiscoverByGenre('movie',28,1);},type:'movie',lt:'genre',gid:28},{name:'👻 Kinh Dị',fn:function(){return tmdbDiscoverByGenre('movie',27,1);},type:'movie',lt:'genre',gid:27},{name:'🚀 Khoa Học VT',fn:function(){return tmdbDiscoverByGenre('movie',878,1);},type:'movie',lt:'genre',gid:878},{name:'😂 Hài',fn:function(){return tmdbDiscoverByGenre('movie',35,1);},type:'movie',lt:'genre',gid:35},{name:'💕 Lãng Mạn',fn:function(){return tmdbDiscoverByGenre('movie',10749,1);},type:'movie',lt:'genre',gid:10749}];
            var loaded=0;tmdbRows.forEach(function(row){row.fn().then(function(res){var list=(res.results||[]).filter(function(i){return i.poster_path;}).slice(0,12);if(list.length){var r=$('<div class="kk-row"></div>');var more=$('<div class="kk-row-more selector">Xem thêm</div>');var rl=$('<div class="kk-row-list"></div>');bindEnter(more,function(){Lampa.Activity.push({url:'',title:row.name,component:'kkphim_tmdb_genre',tmdb_genre_id:row.gid||0,tmdb_media_type:row.type,tmdb_list_type:row.lt,page_num:1});});list.forEach(function(item){item.media_type=item.media_type||row.type;rl.append(mkTmdbCard(item));});r.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">'+esc(row.name)+'</div>').append(more)).append(rl);scroll.append(r);}loaded++;if(loaded>=tmdbRows.length){comp.activity.loader(false);comp.start();}}).catch(function(){loaded++;if(loaded>=tmdbRows.length){comp.activity.loader(false);comp.start();}});});}
            this.start=function(){if(_src&&_tab!=='tmdb'&&_src!==getSourceKey()){comp.create();return;}applyCtrl(scroll);enableScroll(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){network.clear();scroll.destroy();};
        });
        ...
        Lampa.Component.add('kkphim_tmdb_genre',function(obj){
            ...
            this.create=function(){this.activity.loader(true);clearScroll(scroll);
            var allGenres=mediaType==='tv'?TMDB_TV_GENRES:TMDB_MOVIE_GENRES;var genreTags=$('<div class="kk-genre-tags"></div>');
            var movieTag=$('<div class="kk-genre-tag selector '+(mediaType==='movie'?'kk-genre-tag--active':'')+'">🎬 Phim</div>');var tvTag=$('<div class="kk-genre-tag selector '+(mediaType==='tv'?'kk-genre-tag--active':'')+'">📺 TV</div>');
            ...
            scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+esc(title)+'</div>').append(genreTags).append(grid).append(lm));bindEnter(lm,function(){if(!loading&&hasMore)doLoad();});doLoad();};
            function doLoad(){loading=true;lm.text('Đang tải...');var p;if(listType==='trending')p=tmdbTrending(mediaType,page);else if(listType==='top_rated')p=tmdbTopRated(mediaType,page);else if(listType==='popular')p=tmdbPopular(mediaType,page);else p=tmdbDiscoverByGenre(mediaType,genreId,page);p.then(function(res){var list=(res.results||[]).filter(function(i){return i.poster_path;});if(!list.length){hasMore=false;lm.text('Hết');comp.activity.loader(false);loading=false;comp.start();return;}if(page>=(res.total_pages||500))hasMore=false;list.forEach(function(item){item.media_type=item.media_type||mediaType;grid.append(mkTmdbCard(item).addClass('kk-card--grid'));});page++;loading=false;lm.text(hasMore?'Tải thêm':'Hết');comp.activity.loader(false);comp.start();}).catch(function(){loading=false;lm.text('Lỗi');comp.activity.loader(false);});}
        ...
        Lampa.Component.add('kkphim_category',function(obj){
            ...
            var page=obj.page_num||1,title=obj.title||(obj.cat&&obj.cat.name)||'',catSlug=obj.category_slug||'';var apiPath=obj.cat?obj.cat.api:null;
            var grid=$('<div class="kk-grid"></div>'),lm=$('<div class="kk-loadmore selector">Tải thêm</div>'),loading=false,hasMore=true;
        ...
        Lampa.Component.add('kkphim_search',function(obj){
            var network=new Lampa.Reguest(),scroll=new Lampa.Scroll({mask:true,over:true}),comp=this;
            var kw=obj.keyword||'',page=obj.page_num||1;var grid=$('<div class="kk-grid"></div>'),lm=$('<div class="kk-loadmore selector">Tải thêm</div>'),loading=false,hasMore=true,tmdbDone=false;
            ...
            function addTmdb(){if(tmdbDone)return;tmdbDone=true;tmdbSearchMulti(kw,1).then(function(res){var list=(res.results||[]).filter(function(i){return(i.media_type==='movie'||i.media_type==='tv')&&i.poster_path;});if(!list.length)return;var sec=$('<div style="padding:0 1.5em;margin-top:1.5em"></div>');sec.append($('<div class="kk-tmdb-notice">📡 Không tìm thấy trên '+esc(getSource().name)+'. Kết quả từ TMDB:</div>'));var g2=$('<div class="kk-grid"></div>');list.forEach(function(item){g2.append(mkTmdbCard(item).addClass('kk-card--grid'));});sec.append(g2);scroll.append(sec);comp.start();}).catch(function(){});}
        ...
        Lampa.Component.add('kkphim_detail',function(obj){
            var network=new Lampa.Reguest(),scroll=new Lampa.Scroll({mask:true,over:true});var movie=norm(obj.movie),comp=this,rendered=false,_tmdb=null;
            this.create=function(){this.activity.loader(true);clearScroll(scroll);rendered=false;if(!movie||!movie.slug){this.activity.loader(false);scroll.append($('<div class="empty__body"><div class="empty__title">Không có dữ liệu</div></div>'));comp.start();return;}network.silent(SRC_API()+'phim/'+movie.slug,function(res){if(rendered)return;loadAll(norm(res.movie||res||{}),res.episodes||[]);},function(){comp.activity.loader(false);Lampa.Noty.show('Lỗi');});};
            async function loadAll(data,episodes){if(!data||!data.slug)data=movie;try{var tid=getTmdbId(data),tt=detectType(data),tmdb=null,logos=null;if(!tid){var tr=await searchTmdbByName(data.name,data.origin_name,data.year,tt);if(tr){tid=tr.id;if(!data.tmdb)data.tmdb={};data.tmdb.id=tid;data.tmdb.type=tr.media_type||tt;}}if(tid){try{tmdb=await tmdbFetch('/'+tt+'/'+tid+'?language=vi-VN&append_to_response=credits,images');}catch(e){try{tmdb=await tmdbFetch('/'+tt+'/'+tid+'?language=en-US&append_to_response=credits,images');}catch(e2){}}try{logos=await tmdbFetch('/'+tt+'/'+tid+'/images');}catch(e3){}}_tmdb=tmdb;if(!rendered){build(data,episodes,tmdb,logos,tt);rendered=true;}}catch(e){if(!rendered){build(data,episodes,null,null,detectType(data));rendered=true;}}comp.activity.loader(false);comp.start();}
            function build(data,episodes,tmdb,logos,ttype){
                ...
                var body=$('<div class="kk-body"><div class="kk-metas"><span class="kk-meta">⭐ '+esc(v)+'</span>'+(y?'<span class="kk-meta">📅 '+esc(y)+'</span>':'')+(rt?'<span class="kk-meta">⏱ '+esc(rt)+'</span>':'')+(epCur?'<span class="kk-meta">🎬 '+esc(epCur)+'</span>':'')+'</div><div class="kk-genres">'+ghtml+'</div>'+crewH+'<div class="kk-desc">'+fmtTxt(d)+'</div><div class="kk-actions" id="kk-actions"></div></div>');

                // Build play buttons
                var actionsEl=body.find('#kk-actions');
                // Play buttons for episodes
                var epButtons=buildEpisodeSection(episodes,t,data,ttype,_tmdb);
                actionsEl.append(epButtons.children());

                // Stream button
                if(hasTmdb){var streamBtn=$('<div class="kk-act-wrap"><div class="kk-act kk-act--stream selector">'+esc(streamLbl)+'</div></div>');bindEnter(streamBtn.find('.kk-act--stream'),function(){openStreamSearch(getTmdbId(data),ttype,data,episodes,ps,_tmdb);});actionsEl.append(streamBtn);}
                // Sub
                var subBtn=$('<div class="kk-act-wrap"><div class="kk-act kk-act--sub selector">📝 Phụ đề</div></div>');
                bindEnter(subBtn.find('.kk-act--sub'),async function(){var tmdbId=getTmdbId(data),imdb=null;if(tmdbId)try{imdb=await getImdbId(ttype,tmdbId);}catch(e){}var en=_tmdb?(_tmdb.original_title||_tmdb.original_name||''):(data.origin_name||data.name||'');Lampa.Noty.show('Tìm sub...');var subs=await searchSubs(imdb,tmdbId,ttype,null,null,en);if(!subs.length){Lampa.Noty.show(getSubdlKey()?'Không có':'Cần key');return;}Lampa.Select.show({title:'📝 ('+subs.length+')',items:subs.slice(0,40).map(function(s){return{title:s.label,value:s};}),onSelect:function(a){Lampa.Noty.show('✅ '+a.value.label);Lampa.Controller.toggle('content');},onBack:function(){Lampa.Controller.toggle('content');}});});
                actionsEl.append(subBtn);
                // Copy
                var copyBtn=$('<div class="kk-act-wrap"><div class="kk-act kk-act--copy selector">🔍 Copy tên</div></div>');bindEnter(copyBtn.find('.kk-act--copy'),function(){showCopyName(data,_tmdb);});actionsEl.append(copyBtn);
                ...
                // Episode list for series (non-single)
                if(!single&&episodes&&episodes.length){
                    ...
                    episodes.forEach(function(sv){
                        ...
                        var g=$('<div class="kk-eps"></div>');
                        (sv.server_data||[]).forEach(function(ep){var b=$('<div class="kk-ep selector">'+esc(ep.name||'')+'</div>');bindEnter(b,function(){playEpisode(ep,t,data,ttype,_tmdb);});g.append(b);});
                        ew.append(g);
                    });
                    dw.append(ew);
                }
        ...
        Lampa.Component.add('kkphim_detail_tmdb',function(obj){
            ...
            this.create=function(){this.activity.loader(true);clearScroll(scroll);if(!tmdbId){this.activity.loader(false);scroll.append($('<div class="empty__body"><div class="empty__title">Không có dữ liệu</div></div>'));comp.start();return;}doLoad();};
            ...
            async function doLoad(){
                ...
                _tmdb=tmdb;
                var nameVi=tmdb?(tmdb.title||tmdb.name||''):'';var nameEn=tmdb?(tmdb.original_title||tmdb.original_name||''):'';if(!nameEn)nameEn=nameVi;
                var allSources=await findAllSources(tmdbId,ttype,nameVi,nameEn);
                buildPage(tmdb,logos,allSources);comp.activity.loader(false);comp.start();
            }

            function buildPage(tmdb,logos,allSources){
                clearScroll(scroll);var bk='',ps='',t='',o='',d='Không có mô tả',v='N/A',y='',rt='';
                var ghtml='',castH='',dirH='',crewH='',logoH='',dir='';
                if(tmdb){if(tmdb.backdrop_path)bk=TMDB_IMG+tmdb.backdrop_path;if(tmdb.poster_path)ps=TMDB_IMG+tmdb.poster_path;t=tmdb.title||tmdb.name||'';o=tmdb.original_title||tmdb.original_name||'';if(tmdb.overview)d=tmdb.overview;if(tmdb.vote_average)v=Number(tmdb.vote_average).toFixed(1);if(tmdb.release_date)y=tmdb.release_date.slice(0,4);if(!y&&tmdb.first_air_date)y=tmdb.first_air_date.slice(0,4);if(tmdb.runtime)rt=tmdb.runtime+' phút';if(tmdb.number_of_seasons)rt=(rt?rt+' · ':'')+tmdb.number_of_seasons+' mùa';var logo=pickLogo(logos||tmdb.images);if(logo&&logo.file_path)logoH='<div class="kk-logo"><img src="'+TMDB_IMG_W500+logo.file_path+'"></div>';if(tmdb.genres&&tmdb.genres.length)ghtml=tmdb.genres.map(function(g){return '<span class="kk-genre selector" data-tmdb-gid="'+g.id+'">'+esc(g.name||'')+'</span>';}).join('');if(tmdb.credits){castH=mkPeople((tmdb.credits.cast||[]).slice(0,12),'character');var dirs=(tmdb.credits.crew||[]).filter(function(c){return c.job==='Director'||c.job==='Creator';}).filter(function(p,i,a){return a.findIndex(function(x){return x.name===p.name;})===i;}).slice(0,6);if(dirs.length){dir=dirs.map(function(c){return c.name;}).join(', ');dirH=mkPeople(dirs.map(function(c){return{name:c.name,profile_path:c.profile_path,job:c.job||'Đạo diễn'};}), 'job');}}}
                if(dir&&!dirH)crewH='<div class="kk-crew"><b>Đạo diễn</b><span>'+esc(dir)+'</span></div>';
                ...
                var body=$('<div class="kk-body"><div class="kk-metas"><span class="kk-meta">⭐ '+esc(v)+'</span>'+(y?'<span class="kk-meta">📅 '+esc(y)+'</span>':'')+(rt?'<span class="kk-meta">⏱ '+esc(rt)+'</span>':'')+'</div><div class="kk-genres">'+ghtml+'</div>'+crewH+'<div class="kk-desc">'+fmtTxt(d)+'</div>'+noticeHtml+'<div class="kk-actions" id="kk-actions"></div></div>');

                var actionsEl=body.find('#kk-actions');
                // Stream button first
                var streamBtn=$('<div class="kk-act-wrap"><div class="kk-act kk-act--stream selector">'+esc(streamLbl)+'</div></div>');bindEnter(streamBtn.find('.kk-act--stream'),function(){openStreamSearch(tmdbId,ttype,movieData,null,ps,_tmdb);});actionsEl.append(streamBtn);
                // Source play buttons
                if(hasSource){var srcBtns=buildSourceButtons(allSources,t,movieData,ttype,_tmdb);actionsEl.append(srcBtns.children());}
                // Sub + Copy
                var subBtn=$('<div class="kk-act-wrap"><div class="kk-act kk-act--sub selector">📝 Phụ đề</div></div>');bindEnter(subBtn.find('.kk-act--sub'),async function(){var imdb=null;try{imdb=await getImdbId(ttype,tmdbId);}catch(e){}var en=_tmdb?(_tmdb.original_title||_tmdb.original_name||''):o;Lampa.Noty.show('Tìm sub...');var subs=await searchSubs(imdb,tmdbId,ttype,null,null,en);if(!subs.length){Lampa.Noty.show(getSubdlKey()?'Không có':'Cần key');return;}Lampa.Select.show({title:'📝 ('+subs.length+')',items:subs.slice(0,40).map(function(s){return{title:s.label,value:s};}),onSelect:function(a){Lampa.Noty.show('✅ '+a.value.label);Lampa.Controller.toggle('content');},onBack:function(){Lampa.Controller.toggle('content');}});});actionsEl.append(subBtn);
                var copyBtn=$('<div class="kk-act-wrap"><div class="kk-act kk-act--copy selector">🔍 Copy tên</div></div>');bindEnter(copyBtn.find('.kk-act--copy'),function(){showCopyName(movieData,_tmdb);});actionsEl.append(copyBtn);
                ...
                var similar=[];if(tmdb&&tmdb.recommendations&&tmdb.recommendations.results)similar=tmdb.recommendations.results;if(!similar.length&&tmdb&&tmdb.similar&&tmdb.similar.results)similar=tmdb.similar.results;similar=similar.filter(function(i){return i.poster_path;}).slice(0,12);
                if(similar.length){var simSection=$('<div class="kk-section kk-section--last kk-similar"></div>').append('<div class="kk-block-title">Phim tương tự</div>');var simList=$('<div class="kk-similar-list"></div>');similar.forEach(function(item){item.media_type=item.media_type||ttype;simList.append(mkTmdbCard(item));});simSection.append(simList);dw.append(simSection);}else dw.append($('<div class="kk-section kk-section--last"></div>'));
                scroll.append(dw);
            }
            this.start=function(){applyCtrl(scroll);enableScroll(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){scroll.destroy();};
        });
