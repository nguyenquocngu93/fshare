(function () {
    ...
    function fullImg(u) { if (!u) return ''; if (u.indexOf('http') === 0) return u; var base = SRC_IMG(); return base ? base + u : u; }
    ...
    async function tsAdd(mag, title, poster) {
        var u = tsUrl('/torrents'); if (!u) throw new Error('Ch\u01b0a c\u1ea5u h\u00ecnh TS');
        ...
        if (!r.ok) throw new Error('TS:' + r.status); return await r.json();
    }
    ...
    async function playViaTS(stream, title, poster, fileIdx) {
        ...
        try {
            var td = await tsAdd(buildMag(stream.infoHash), title, poster); var hash = td.hash || stream.infoHash;
            await delay(2000); var info = null, rt = 0;
    ...
    async function getTmdbSeasons(id) {
        try { var r = await tmdbFetch('/tv/' + id + '?language=' + tLang()); if (r && r.seasons) return r.seasons.filter(function (s) { return s.season_number > 0; }).map(function (s) { return { season_number: s.season_number, name: s.name || ('Season ' + s.season_number), episode_count: s.episode_count || 0 }; }); } catch (e) {} return [];
    }

    async function tmdbTrending(page) { return await tmdbFetch('/trending/all/week?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbTrendingDay(page) { return await tmdbFetch('/trending/all/day?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbPopularMovies(page) { return await tmdbFetch('/movie/popular?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbPopularTV(page) { return await tmdbFetch('/tv/popular?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbTopRatedMovies(page) { return await tmdbFetch('/movie/top_rated?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbTopRatedTV(page) { return await tmdbFetch('/tv/top_rated?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbNowPlaying(page) { return await tmdbFetch('/movie/now_playing?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbUpcoming(page) { return await tmdbFetch('/movie/upcoming?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbAiringToday(page) { return await tmdbFetch('/tv/airing_today?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbOnTheAir(page) { return await tmdbFetch('/tv/on_the_air?language=' + tLang() + '&page=' + (page || 1)); }
    async function tmdbSearchMulti(query, page) { return await tmdbFetch('/search/multi?language=' + tLang() + '&query=' + encodeURIComponent(query) + '&page=' + (page || 1)); }
    ...
    async function tmdbDiscover(type, genreId, page) { return await tmdbFetch('/discover/' + type + '?language=' + tLang() + '&sort_by=popularity.desc&with_genres=' + genreId + '&page=' + (page || 1)); }

    async function loadGenres(type) {
        if (_genreCache[type]) return _genreCache[type];
        try { var r = await tmdbFetch('/genre/' + type + '/list?language=' + tLang()); _genreCache[type] = r.genres || []; return _genreCache[type]; } catch (e) { return []; }
    }
    ...
    function showSourceEpisodePicker(data, episodes, title) {
        ...
        if (totalEps === 1) { var ep = getFirstEp(episodes); if (!ep) { Lampa.Noty.show('Kh\u00f4ng c\u00f3 link'); return; } var link = ep.link_m3u8 || ep.link_embed || ''; if (!link) { Lampa.Noty.show('Kh\u00f4ng c\u00f3 link'); return; } Lampa.Player.play({ title: title, url: link }); return; }
    ...
    function pickTorrentEp(season, imdb, title, poster) {
        ...
        Lampa.Select.show({ title: season.name, items: items,
            onSelect: async function (a) { var label = title + ' S' + pad(a.value.s) + 'E' + pad(a.value.e); Lampa.Noty.show('T\u00ecm ' + label + '...'); try { var streams = await fetchTio('tv', imdb, a.value.s, a.value.e); if (!streams.length) { Lampa.Noty.show('Kh\u00f4ng c\u00f3 torrent'); return; } showTioResults(streams, label, poster); } catch (e) { Lampa.Noty.show('L\u1ed7i: ' + (e.message || '')); } },
            onBack: function () { Lampa.Controller.toggle('content'); } });
    }
    ...
    function promptTorrentEp(ds, imdb, title, poster) {
        ...
        var v = window.prompt('Season:T\u1eadp', ds + ':1'), p = String(v || ds + ':1').split(':'); var s = parseInt(p[0]) || ds, e2 = parseInt(p[1]) || 1, label = title + ' S' + pad(s) + 'E' + pad(e2);
    ...
    function applyCtrl(scroll) {
        Lampa.Controller.add('content', { toggle: function () { Lampa.Controller.collectionSet(scroll.render()); Lampa.Controller.collectionFocus(false, scroll.render()); }, left: function () { if (Navigator.canmove('left')) Navigator.move('left'); else Lampa.Controller.toggle('menu'); }, right: function () { Navigator.move('right'); }, up: function () { if (Navigator.canmove('up')) Navigator.move('up'); else Lampa.Controller.toggle('head'); }, down: function () { Navigator.move('down'); }, back: function () { Lampa.Activity.backward(); } });
    ...
    function injectCSS() {
        ...
        var css = '';
        css += '.kk-topbar{display:flex;justify-content:space-between;align-items:center;padding:0 1.5em;margin-bottom:.8em;gap:1em;z-index:5}.kk-topbar-title{font-size:2em;font-weight:900;color:#fff;flex:1}.kk-topbar-btns{display:flex;gap:.5em}';
        css += '.kk-btn{display:inline-flex;align-items:center;gap:.45em;padding:.8em 1.1em;border-radius:999px;background:linear-gradient(180deg,rgba(255,255,255,.14),rgba(255,255,255,.08));border:1px solid rgba(255,255,255,.10);color:#fff;font-size:.98em;font-weight:800;cursor:pointer}.kk-btn.focus{background:#fff;color:#000}';
        css += '.kk-srcbar{display:flex;gap:.5em;padding:0 1.5em .7em;flex-wrap:wrap}.kk-srcbtn{padding:.6em 1em;border-radius:.75em;font-size:.95em;font-weight:700;cursor:pointer;border:1px solid rgba(255,255,255,.1)}.kk-srcbtn--on{background:rgba(99,102,241,.25);border-color:rgba(99,102,241,.5);color:#c4b5fd}.kk-srcbtn--off{background:rgba(255,255,255,.06);color:rgba(255,255,255,.45)}.kk-srcbtn.focus{background:rgba(99,102,241,.4);color:#fff}';
        css += '.kk-tsbar{padding:0 1.5em .5em}.kk-tsbadge{display:inline-flex;align-items:center;gap:.4em;padding:.45em .85em;border-radius:.65em;background:rgba(74,222,128,.1);border:1px solid rgba(74,222,128,.15);font-size:.88em;color:#4ade80;font-weight:700}';
        css += '.kk-row{margin-bottom:1.9em}.kk-row-head{display:flex;justify-content:space-between;align-items:center;padding:0 1.5em;margin-bottom:.85em}.kk-row-title{font-size:1.55em;font-weight:900;color:#fff}.kk-row-more{font-size:.98em;font-weight:800;padding:.7em 1.1em;border-radius:999px;background:rgba(255,255,255,.08);color:#fff;cursor:pointer}.kk-row-more.focus{background:#fff;color:#000}';
        css += '.kk-row-list{display:flex;gap:.9em;overflow-x:auto;overflow-y:hidden;padding:0 1.5em .2em;-webkit-overflow-scrolling:touch}.kk-row-list::-webkit-scrollbar,.kk-cast-list::-webkit-scrollbar,.kk-similar-list::-webkit-scrollbar{display:none}';
        css += '.kk-card{flex:0 0 auto;width:9.5em;cursor:pointer}.kk-card--grid{width:100%}.kk-card-img{position:relative;width:100%;aspect-ratio:2/3;border-radius:.9em;overflow:hidden;background:#242424}.kk-card-img img{width:100%;height:100%;object-fit:cover}';
        css += '.kk-card-q{position:absolute;top:.5em;left:.5em;padding:.2em .5em;border-radius:.4em;font-size:.7em;font-weight:800;background:#f6c344;color:#000}.kk-card-ep{position:absolute;top:.5em;right:.5em;padding:.2em .5em;border-radius:.4em;font-size:.7em;font-weight:800;background:#e53935;color:#fff}';
        css += '.kk-card-name{margin-top:.6em;font-size:1em;line-height:1.3;font-weight:700;color:#fff;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.kk-card-year{margin-top:.18em;font-size:.88em;color:rgba(255,255,255,.5)}';
        css += '.kk-grid-wrap{padding:0 1.5em}.kk-grid-title{font-size:1.9em;font-weight:900;color:#fff;margin:0 0 .85em}.kk-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.9em}';
        ...
        css += '.kk-hero{position:relative;overflow:hidden;background:#111}.kk-hero-bg{position:relative;height:25em}.kk-hero-bg img{width:100%;height:100%;object-fit:cover}.kk-hero-mask{position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,.08),rgba(0,0,0,.16) 24%,rgba(0,0,0,.36) 52%,rgba(14,14,14,.78) 78%,rgba(14,14,14,1))}';
        css += '.kk-hero-bottom{position:absolute;left:0;right:0;bottom:0;z-index:2;padding:1.4em 1.5em 1.2em}.kk-hero-flex{display:block}.kk-hero-poster{display:none}.kk-hero-info{min-width:0}';
        css += '.kk-logo{max-width:34em;margin:0 0 1em}.kk-logo img{max-width:100%;max-height:10em;object-fit:contain;filter:drop-shadow(0 .4em 1.1em rgba(0,0,0,.45))}.kk-title{font-size:2.5em;line-height:1.05;font-weight:900;color:#fff;margin-bottom:.2em}.kk-origin{font-size:1.15em;line-height:1.45;color:rgba(255,255,255,.82)}';
        css += '.kk-body{position:relative;z-index:3;padding:1.4em 1.5em 0;background:#141414}.kk-metas{display:flex;flex-wrap:wrap;gap:.65em;margin:0 0 1.1em}.kk-meta{padding:.55em .95em;border-radius:.8em;background:rgba(255,255,255,.08);color:#fff;font-size:1.1em;font-weight:800}';
        css += '.kk-genres{display:flex;flex-wrap:wrap;gap:.65em;margin:0 0 1.1em}.kk-genre{padding:.55em .95em;border-radius:.8em;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.95);font-size:1.02em;font-weight:700;cursor:pointer}.kk-genre.focus{background:rgba(255,255,255,.18)}';
        ...
        css += '.kk-actions{display:flex;flex-wrap:wrap;gap:.7em;padding:.1em 0 .2em}.kk-act-wrap{width:100%}.kk-act{display:inline-flex;align-items:center;justify-content:center;width:100%;padding:.95em;border-radius:.95em;font-size:1.15em;font-weight:900;cursor:pointer}';
        css += '.kk-act--play{background:#ff1730;color:#fff}.kk-act--play.focus{background:#ff3047}.kk-act--torrent{background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff}.kk-act--torrent.focus{background:linear-gradient(135deg,#818cf8,#a78bfa)}.kk-act--disabled{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);color:rgba(255,255,255,.35);cursor:default}';
        css += '.kk-section{padding:1.25em 1.5em 0;background:#141414}.kk-section+.kk-section{padding-top:1.15em;border-top:1px solid rgba(255,255,255,.04)}.kk-body+.kk-section{border-top:1px solid rgba(255,255,255,.04)}.kk-section--last{padding-bottom:1.4em}';
        css += '.kk-block-title{font-size:1.75em;font-weight:900;color:#fff;margin:0 0 .8em}';
        css += '.kk-cast-list{display:flex;gap:1em;overflow-x:auto;-webkit-overflow-scrolling:touch;touch-action:pan-x}.kk-cast-card{flex:0 0 auto;width:7.2em;text-align:center}.kk-cast-img{width:6.5em;height:6.5em;border-radius:50%;overflow:hidden;background:#2b2b2b;margin:0 auto .65em;border:2px solid rgba(255,255,255,.08)}.kk-cast-img img{width:100%;height:100%;object-fit:cover}.kk-cast-empty{width:100%;height:100%;background:#333;border-radius:50%}.kk-cast-name{font-size:1em;font-weight:800;color:#fff}.kk-cast-role{font-size:.88em;color:rgba(255,255,255,.6);margin-top:.12em}';
        ...
        css += '.kk-slug-info{margin:0 0 1.1em;padding:.85em 1.1em;border-radius:.85em;font-size:1em;font-weight:700}.kk-slug-found{background:rgba(74,222,128,.1);border:1px solid rgba(74,222,128,.15);color:#4ade80}.kk-slug-notfound{background:rgba(251,191,36,.1);border:1px solid rgba(251,191,36,.15);color:#fbbf24}';
        css += '.kk-genre-bar{display:flex;flex-wrap:wrap;gap:.45em;padding:0 1.5em .8em}';
        css += '.kk-genre-chip{flex:0 0 auto;padding:.5em .9em;border-radius:999px;font-size:.88em;font-weight:700;cursor:pointer;white-space:nowrap}';
        css += '.kk-genre-chip--on{background:linear-gradient(135deg,#6366f1,#8b5cf6);border:1px solid rgba(139,92,246,.6);color:#fff;box-shadow:0 0 12px rgba(99,102,241,.3)}';
        css += '.kk-genre-chip--off{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.55)}';
        css += '.kk-genre-chip.focus{background:linear-gradient(135deg,#818cf8,#a78bfa);color:#fff;border-color:rgba(167,139,250,.6)}';
        css += '.kk-stg-wrap{padding:1.4em}.kk-stg-title{font-size:2em;font-weight:900;color:#fff;margin:0 0 1.3em}.kk-stg-group{margin-bottom:1.7em}.kk-stg-group-title{font-size:1.3em;font-weight:900;color:#fff;margin:0 0 .8em;display:flex;align-items:center;gap:.5em}';
        ...
        css += '.kk-stg-status{margin-top:.7em;padding:.85em 1.1em;border-radius:.85em;font-size:.98em;font-weight:700}.kk-stg-status--ok{background:rgba(74,222,128,.12);color:#4ade80}.kk-stg-status--err{background:rgba(248,113,113,.12);color:#f87171}.kk-stg-status--loading{background:rgba(255,255,255,.06);color:rgba(255,255,255,.5)}';
        css += '.kk-stg-ver{margin-top:1.5em;text-align:center;font-size:.85em;color:rgba(255,255,255,.25)}';
        css += '.selector,.kk-act,.kk-row-more,.kk-loadmore,.kk-genre,.kk-card,.kk-btn,.kk-stg-item,.kk-srcbtn,.kk-genre-chip{touch-action:manipulation;-webkit-tap-highlight-color:transparent}';
    ...
    function startPlugin() {
        ...
        Lampa.Component.add('kkphim_settings', function () {
            ...
            this.create = function () {
                ...
                Object.keys(SOURCES).forEach(function (k) { var src = SOURCES[k], it = si(src.name, 'API: ' + src.api, cur === k ? '\u2705' : 'Ch\u1ecdn'); if (cur === k) it.find('.kk-stg-value').css('color', '#4ade80'); bindEnter(it, function () { saveSettings({ source: k }); Lampa.Noty.show(src.name); comp.create(); }); g0.append(it); });
                w.append(g0);
                var g5 = $('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">\ud83c\udfa8 Giao di\u1ec7n</div>');
                var curGrid = s.card_style || '3';
                [{ k: '2', n: '2 c\u1ed9t' }, { k: '3', n: '3 c\u1ed9t' }, { k: '4', n: '4 c\u1ed9t' }].forEach(function (opt) { var it = si(opt.n, 'S\u1ed1 c\u1ed9t l\u01b0\u1edbi phim', curGrid === opt.k ? '\u2705' : 'Ch\u1ecdn'); if (curGrid === opt.k) it.find('.kk-stg-value').css('color', '#4ade80'); bindEnter(it, function () { saveSettings({ card_style: opt.k }); Lampa.Noty.show(opt.n); comp.create(); }); g5.append(it); });
                w.append(g5);
                var g6 = $('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">\ud83c\udf10 Ng\u00f4n ng\u1eef TMDB</div>');
                var curLang = s.tmdb_lang || 'vi-VN';
                [{ k: 'vi-VN', n: 'Ti\u1ebfng Vi\u1ec7t' }, { k: 'en-US', n: 'English' }, { k: 'ja-JP', n: '\u65e5\u672c\u8a9e' }, { k: 'ko-KR', n: '\ud55c\uad6d\uc5b4' }, { k: 'zh-CN', n: '\u4e2d\u6587' }].forEach(function (opt) { var it = si(opt.n, opt.k, curLang === opt.k ? '\u2705' : 'Ch\u1ecdn'); if (curLang === opt.k) it.find('.kk-stg-value').css('color', '#4ade80'); bindEnter(it, function () { saveSettings({ tmdb_lang: opt.k }); _genreCache = { movie: null, tv: null }; Lampa.Noty.show(opt.n); comp.create(); }); g6.append(it); });
                w.append(g6);
                var g1 = $('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">\ud83d\udda5\ufe0f TorrServer</div>');
                var hi = si('\u0110\u1ecba ch\u1ec9', 'V\u00ed d\u1ee5: 192.168.1.100:8090', s.torrserver_host || 'Ch\u01b0a c\u00e0i');
                bindEnter(hi, function () { pi('\u0110\u1ecba ch\u1ec9 TorrServer', s.torrserver_host || '', function (v) { v = (v || '').trim(); saveSettings({ torrserver_host: v }); s.torrserver_host = v; hi.find('.kk-stg-value').text(v || 'Ch\u01b0a c\u00e0i'); }); }); g1.append(hi);
                ...
                var ti = si('\ud83d\udd0c Ki\u1ec3m tra k\u1ebft n\u1ed1i', 'Ping TorrServer', 'Nh\u1ea5n'), st1 = $('<div class="kk-stg-status" style="display:none"></div>');
                ...
                var tii = si('Config URL', 'D\u00e1n link manifest ho\u1eb7c config', s.torrentio_config ? 'C\u00f3' : 'M\u1eb7c \u0111\u1ecbnh');
                bindEnter(tii, function () { pi('Torrentio Config', s.torrentio_config || '', function (v) { v = (v || '').trim(); saveSettings({ torrentio_config: v }); s.torrentio_config = v; tii.find('.kk-stg-value').text(v ? 'C\u00f3' : 'M\u1eb7c \u0111\u1ecbnh'); }); }); g2.append(tii);
                var tti = si('\ud83e\uddea Ki\u1ec3m tra Torrentio', 'Th\u1eed v\u1edbi phim Inception', 'Nh\u1ea5n'), st2 = $('<div class="kk-stg-status" style="display:none"></div>');
                bindEnter(tti, function () { testTIO(st2); }); g2.append(tti).append(st2); w.append(g2);
                var g7 = $('<div class="kk-stg-group"></div>').append('<div class="kk-stg-group-title">\ud83d\uddc3\ufe0f B\u1ed9 nh\u1edb \u0111\u1ec7m</div>');
                var cc = si('X\u00f3a cache th\u1ec3 lo\u1ea1i', '\u0110\u1eb7t l\u1ea1i danh s\u00e1ch th\u1ec3 lo\u1ea1i TMDB', 'X\u00f3a');
                bindEnter(cc, function () { _genreCache = { movie: null, tv: null }; Lampa.Noty.show('\u0110\u00e3 x\u00f3a cache'); }); g7.append(cc); w.append(g7);
                var g4 = $('<div class="kk-stg-group"></div>');
                var cl = si('\ud83d\uddd1\ufe0f X\u00f3a to\u00e0n b\u1ed9 c\u00e0i \u0111\u1eb7t', 'Kh\u00f4i ph\u1ee5c m\u1eb7c \u0111\u1ecbnh', 'X\u00f3a'); cl.find('.kk-stg-value').css('color', '#f87171');
                ...
                w.append('<div class="kk-stg-ver">KKPhim Plugin v2.0</div>');
            ...
            async function testTS(el) { if (!getTSHost()) { el.show().attr('class', 'kk-stg-status kk-stg-status--err').text('\u274c Ch\u01b0a nh\u1eadp \u0111\u1ecba ch\u1ec9'); return; } el.show().attr('class', 'kk-stg-status kk-stg-status--loading').text('\u23f3 \u0110ang ki\u1ec3m tra...'); try { var r = await fetch(tsUrl('/echo'), { headers: tsHdr() }); el.attr('class', 'kk-stg-status ' + (r.ok ? 'kk-stg-status--ok' : 'kk-stg-status--err')).text(r.ok ? '\u2705 K\u1ebft n\u1ed1i th\u00e0nh c\u00f4ng!' : '\u274c L\u1ed7i: ' + r.status); } catch (e) { el.attr('class', 'kk-stg-status kk-stg-status--err').text('\u274c ' + (e.message || '')); } }
            async function testTIO(el) { el.show().attr('class', 'kk-stg-status kk-stg-status--loading').text('\u23f3 \u0110ang ki\u1ec3m tra...'); var c = cleanTioConfig(getTioConfig()); var u = TORRENTIO_BASE + (c ? '/' + c : '') + '/stream/movie/tt1375666.json'; try { var r = await fetch(u); if (!r.ok) { el.attr('class', 'kk-stg-status kk-stg-status--err').text('\u274c L\u1ed7i: ' + r.status); return; } var d = await r.json(); el.attr('class', 'kk-stg-status kk-stg-status--ok').text('\u2705 T\u00ecm th\u1ea5y ' + (d.streams || []).length + ' torrent'); } catch (e) { el.attr('class', 'kk-stg-status kk-stg-status--err').text('\u274c ' + (e.message || '')); } }
        ...
        Lampa.Component.add('kkphim_tmdb_main', function () {
            ...
            this.create = function () {
                ...
                var sb = $('<div class="kk-srcbar"></div>');
                Object.keys(SOURCES).forEach(function (k) { var s = SOURCES[k], btn = $('<div class="kk-srcbtn selector kk-srcbtn--off">' + esc(s.name) + '</div>'); bindEnter(btn, function () { saveSettings({ source: k }); Lampa.Activity.push({ url: '', title: 'KKPhim', component: 'kkphim_main', page: 1 }); }); sb.append(btn); });
                ...
                var sections = [
                    ...
                    { name: '\ud83d\udcc5 S\u1eafp chi\u1ebfu', fn: tmdbUpcoming, lt: 'upcoming' },
        ...
        Lampa.Component.add('kkphim_tmdb_list', function (obj) {
            ...
            this.create = function () { comp.activity.loader(true); clearScroll(scroll); grid.css('grid-template-columns', 'repeat(' + getCardStyle() + ',minmax(0,1fr))'); scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">' + esc(obj.title || 'TMDB') + '</div>').append(grid).append(lm)); bindEnter(lm, function () { if (!loading && hasMore) doLoad(); }); doLoad(); };
            function doLoad() { loading = true; lm.text('\u0110ang t\u1ea3i...'); fetchFn(page).then(function (res) { var items = (res.results || []).filter(function (i) { return i.media_type !== 'person'; }); if (!items.length) { hasMore = false; lm.text('H\u1ebft'); comp.activity.loader(false); loading = false; comp.start(); return; } items.forEach(function (i) { grid.append(mkTmdbCard(i).addClass('kk-card--grid')); }); page++; loading = false; lm.text('T\u1ea3i th\u00eam'); comp.activity.loader(false); comp.start(); }).catch(function () { loading = false; lm.text('L\u1ed7i'); comp.activity.loader(false); }); }
            this.start = function () { applyCtrl(scroll); enableScroll(scroll); }; this.pause = function () {}; this.stop = function () {}; this.render = function () { return scroll.render(); }; this.destroy = function () { scroll.destroy(); };
        });

        // ========== TMDB GENRE (wrap, mixed movie+tv) ==========
        Lampa.Component.add('kkphim_tmdb_genre', function (obj) {
            var scroll = new Lampa.Scroll({ mask: true, over: true }), comp = this;
            var curGenreId = String(obj.genre_id || '');
            var grid = $('<div class="kk-grid"></div>'), lm = $('<div class="kk-loadmore selector">T\u1ea3i th\u00eam</div>');
            var loading = false, hasMore = true;
            var movieDone = false, tvDone = false, moviePage = 1, tvPage = 1;
            var allItems = [], renderedSet = {};

            this.create = function () {
                comp.activity.loader(true); clearScroll(scroll);
                grid.css('grid-template-columns', 'repeat(' + getCardStyle() + ',minmax(0,1fr))');
                var genreBarEl = $('<div class="kk-genre-bar"></div>');
                scroll.append(genreBarEl);
                var gridWrap = $('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title" id="kk-genre-title">' + esc(obj.title || 'Th\u1ec3 lo\u1ea1i') + '</div>').append(grid).append(lm);
                scroll.append(gridWrap);
                bindEnter(lm, function () { if (!loading && hasMore) doLoad(); });

                Promise.all([loadGenres('movie'), loadGenres('tv')]).then(function (results) {
                    var merged = [], seen = {};
                    (results[0] || []).forEach(function (g) { if (!seen[g.id]) { seen[g.id] = true; merged.push(g); } });
                    (results[1] || []).forEach(function (g) { if (!seen[g.id]) { seen[g.id] = true; merged.push(g); } });
                    merged.sort(function (a, b) { return (a.name || '').localeCompare(b.name || ''); });
                    merged.forEach(function (g) {
                        var on = String(g.id) === curGenreId;
                        var chip = $('<div class="kk-genre-chip selector ' + (on ? 'kk-genre-chip--on' : 'kk-genre-chip--off') + '">' + esc(g.name) + '</div>');
                        bindEnter(chip, function () { Lampa.Activity.push({ url: '', title: g.name, component: 'kkphim_tmdb_genre', genre_id: g.id, page_num: 1 }); });
                        genreBarEl.append(chip);
                    });
                    var curGenre = merged.find(function (g) { return String(g.id) === curGenreId; });
                    if (curGenre) scroll.render().find('#kk-genre-title').text(curGenre.name);
                    doLoad();
            ...
            function doLoad() {
                ...
                var promises = [];
                if (!movieDone) promises.push(tmdbDiscover('movie', curGenreId, moviePage).then(function (res) { var items = res.results || []; if (!items.length) { movieDone = true; return; } items.forEach(function (i) { i.media_type = 'movie'; }); allItems = allItems.concat(items); moviePage++; }).catch(function () { movieDone = true; }));
                if (!tvDone) promises.push(tmdbDiscover('tv', curGenreId, tvPage).then(function (res) { var items = res.results || []; if (!items.length) { tvDone = true; return; } items.forEach(function (i) { i.media_type = 'tv'; }); allItems = allItems.concat(items); tvPage++; }).catch(function () { tvDone = true; }));
                Promise.all(promises).then(function () {
                    allItems.sort(function (a, b) { return (b.popularity || 0) - (a.popularity || 0); });
                    var added = 0;
                    for (var i = 0; i < allItems.length; i++) { var item = allItems[i]; var key = item.media_type + '_' + item.id; if (!renderedSet[key]) { renderedSet[key] = true; grid.append(mkTmdbCard(item).addClass('kk-card--grid')); added++; } }
                    if (movieDone && tvDone && added === 0) { hasMore = false; lm.text(Object.keys(renderedSet).length === 0 ? 'Kh\u00f4ng c\u00f3 k\u1ebft qu\u1ea3' : 'H\u1ebft'); }
                    else if (movieDone && tvDone) { hasMore = false; lm.text('H\u1ebft'); }
                    else { lm.text('T\u1ea3i th\u00eam'); }
        ...
        Lampa.Component.add('kkphim_tmdb_search', function (obj) {
            ...
            var grid = $('<div class="kk-grid"></div>'), lm = $('<div class="kk-loadmore selector">T\u1ea3i th\u00eam</div>'), loading = false, hasMore = true;
            this.create = function () { comp.activity.loader(true); clearScroll(scroll); grid.css('grid-template-columns', 'repeat(' + getCardStyle() + ',minmax(0,1fr))'); scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">\ud83d\udd0d TMDB: ' + esc(kw) + '</div>').append(grid).append(lm)); bindEnter(lm, function () { if (!loading && hasMore) doLoad(); }); doLoad(); };
            function doLoad() { loading = true; lm.text('\u0110ang t\u1ea3i...'); tmdbSearchMulti(kw, page).then(function (res) { var items = (res.results || []).filter(function (i) { return i.media_type !== 'person'; }); if (!items.length) { hasMore = false; lm.text(page === 1 ? 'Kh\u00f4ng c\u00f3 k\u1ebft qu\u1ea3' : 'H\u1ebft'); comp.activity.loader(false); loading = false; comp.start(); return; } items.forEach(function (i) { grid.append(mkTmdbCard(i).addClass('kk-card--grid')); }); page++; loading = false; lm.text('T\u1ea3i th\u00eam'); comp.activity.loader(false); comp.start(); }).catch(function () { loading = false; lm.text('L\u1ed7i'); comp.activity.loader(false); }); }
            this.start = function () { applyCtrl(scroll); enableScroll(scroll); }; this.pause = function () {}; this.stop = function () {}; this.render = function () { return scroll.render(); }; this.destroy = function () { scroll.destroy(); };
        });
        ...
        Lampa.Component.add('kkphim_tmdb_detail', function (obj) {
            ...
            async function buildDetail(tmdb, logos, slugs) {
                ...
                if (tmdb.credits) { castH = mkPeople((tmdb.credits.cast || []).slice(0, 12), 'character'); var dirs = (tmdb.credits.crew || []).filter(function (c) { return c.job === 'Director' || c.job === 'Creator' || c.job === 'Series Director'; }); dirs = dirs.filter(function (p, i, a) { return a.findIndex(function (x) { return x.name === p.name; }) === i; }).slice(0, 10); if (dirs.length) { dir = dirs.map(function (c) { return c.name; }).join(', '); dirH = mkPeople(dirs.map(function (c) { return { name: c.name, profile_path: c.profile_path, job: c.job || '\u0110\u1ea1o di\u1ec5n' }; }), 'job'); } }
                ...
                var slugInfoHtml = hasSlug ? '<div class="kk-slug-info kk-slug-found">\u2705 Ngu\u1ed3n: ' + (slugs.kkphim ? 'KKPhim' : '') + (slugs.kkphim && slugs.ophim ? ', ' : '') + (slugs.ophim ? 'OPhim' : '') + '</div>' : '<div class="kk-slug-info kk-slug-notfound">\u26a0\ufe0f Kh\u00f4ng t\u00ecm th\u1ea5y ngu\u1ed3n ph\u00e1t \u2014 Ch\u1ec9 c\u00f3 Torrent</div>';
                ...
                var actHtml = '';
                if (mediaType === 'movie') { actHtml += hasSlug ? '<div class="kk-act-wrap"><div class="kk-act kk-act--play selector" data-action="play-movie">\u25b6 Xem phim</div></div>' : '<div class="kk-act-wrap"><div class="kk-act kk-act--disabled">\u25b6 Kh\u00f4ng c\u00f3 ngu\u1ed3n ph\u00e1t</div></div>'; actHtml += '<div class="kk-act-wrap"><div class="kk-act kk-act--torrent selector" data-action="torrent-movie">\ud83e\uddf2 Torrent' + (getTSHost() ? ' \u2192 TS' : '') + '</div></div>'; }
                else { actHtml += hasSlug ? '<div class="kk-act-wrap"><div class="kk-act kk-act--play selector" data-action="play-tv">\u25b6 Xem phim</div></div>' : '<div class="kk-act-wrap"><div class="kk-act kk-act--disabled">\u25b6 Kh\u00f4ng c\u00f3 ngu\u1ed3n ph\u00e1t</div></div>'; actHtml += '<div class="kk-act-wrap"><div class="kk-act kk-act--torrent selector" data-action="torrent-tv">\ud83e\uddf2 Torrent' + (getTSHost() ? ' \u2192 TS' : '') + '</div></div>'; }
                ...
                bindEnter(body.find('[data-action="play-movie"]'), async function () { var slug = slugs.kkphim || slugs.ophim, src = slugs.kkphim ? SOURCES.kkphim : SOURCES.ophim; Lampa.Noty.show('T\u1ea3i t\u1eeb ' + src.name + '...'); try { var detail = await fetchDetail(src, slug); if (!detail || !detail.episodes || !detail.episodes.length) { if (slugs.kkphim && slugs.ophim) { var ok = src.key === 'kkphim' ? 'ophim' : 'kkphim'; src = SOURCES[ok]; slug = slugs[ok]; detail = await fetchDetail(src, slug); } } if (!detail || !detail.episodes || !detail.episodes.length) { Lampa.Noty.show('Kh\u00f4ng c\u00f3 t\u1eadp phim'); return; } var ep = getFirstEp(detail.episodes); if (!ep) { Lampa.Noty.show('Kh\u00f4ng c\u00f3 link'); return; } var link = ep.link_m3u8 || ep.link_embed || ''; if (!link) { Lampa.Noty.show('Kh\u00f4ng c\u00f3 link'); return; } Lampa.Player.play({ title: t, url: link }); } catch (e) { Lampa.Noty.show('L\u1ed7i: ' + (e.message || '')); } });
                bindEnter(body.find('[data-action="play-tv"]'), async function () { Lampa.Noty.show('\u0110ang t\u00ecm t\u1ea5t c\u1ea3 seasons...'); try { var seasonMap = await gatherAllSeasons(t, o, slugs); var seasonNums = Object.keys(seasonMap).map(Number).sort(function (a, b) { return a - b; }); if (!seasonNums.length) { Lampa.Noty.show('Kh\u00f4ng t\u00ecm th\u1ea5y season'); return; } if (seasonNums.length === 1) loadAndShowEps(seasonMap[seasonNums[0]], t, seasonNums[0]); else { Lampa.Select.show({ title: 'Ch\u1ecdn Season (' + seasonNums.length + ')', items: seasonNums.map(function (sn) { var entries = seasonMap[sn]; var srcs = entries.map(function (e) { return e.source.name; }).filter(function (v, i, a) { return a.indexOf(v) === i; }).join(', '); return { title: 'Season ' + sn + ' (' + srcs + ')', value: { entries: entries, season: sn } }; }), onSelect: function (a) { loadAndShowEps(a.value.entries, t, a.value.season); }, onBack: function () { Lampa.Controller.toggle('content'); } }); } } catch (e) { Lampa.Noty.show('L\u1ed7i: ' + (e.message || '')); } });
            ...
            async function loadAndShowEps(entries, title, seasonNum) { Lampa.Noty.show('T\u1ea3i t\u1eadp Season ' + seasonNum + '...'); var allEps = []; for (var i = 0; i < entries.length; i++) { var eps = await loadSeasonEpisodes(entries[i]); if (eps.length > allEps.length) allEps = eps; } if (!allEps.length) { Lampa.Noty.show('Kh\u00f4ng c\u00f3 t\u1eadp n\u00e0o'); return; } Lampa.Select.show({ title: 'Season ' + seasonNum + ' (' + allEps.length + ' t\u1eadp)', items: allEps.map(function (ep) { return { title: ep.name + ' [' + ep.source + ']', value: ep }; }), onSelect: function (a) { if (!a.value.link) { Lampa.Noty.show('Kh\u00f4ng c\u00f3 link'); return; } Lampa.Player.play({ title: title + ' S' + pad(seasonNum) + ' - ' + a.value.name, url: a.value.link }); }, onBack: function () { Lampa.Controller.toggle('content'); } }); }
            this.start = function () { applyCtrl(scroll); enableScroll(scroll); }; this.pause = function () {}; this.stop = function () {}; this.render = function () { return scroll.render(); }; this.destroy = function () { scroll.destroy(); };
        });
        ...
        Lampa.Component.add('kkphim_detail', function (obj) {
            ...
            function build(data, episodes, tmdb, logos, ttype) {
                ...
                if (tmdb) { if (tmdb.backdrop_path) bk = TMDB_IMG + tmdb.backdrop_path; if (tmdb.poster_path) ps = TMDB_IMG + tmdb.poster_path; if (tmdb.title || tmdb.name) t = tmdb.title || tmdb.name; if (tmdb.original_title || tmdb.original_name) o2 = tmdb.original_title || tmdb.original_name; if (tmdb.overview) d = tmdb.overview; if (tmdb.vote_average) v = Number(tmdb.vote_average).toFixed(1); if (tmdb.release_date) y = tmdb.release_date.slice(0, 4); if (!y && tmdb.first_air_date) y = tmdb.first_air_date.slice(0, 4); if (tmdb.runtime) rt = tmdb.runtime + ' ph\u00fat'; var logo = pickLogo(logos || tmdb.images); if (logo && logo.file_path) logoH = '<div class="kk-logo"><img src="' + TMDB_IMG_W500 + logo.file_path + '"></div>'; if (tmdb.credits) { castH = mkPeople((tmdb.credits.cast || []).slice(0, 12), 'character'); var dirs = (tmdb.credits.crew || []).filter(function (c) { return c.job === 'Director' || c.job === 'Creator' || c.job === 'Series Director'; }); dirs = dirs.filter(function (p, i, a) { return a.findIndex(function (x) { return x.name === p.name; }) === i; }).slice(0, 10); if (dirs.length) { dir = dirs.map(function (c) { return c.name; }).join(', '); dirH = mkPeople(dirs.map(function (c) { return { name: c.name, profile_path: c.profile_path, job: c.job || '\u0110\u1ea1o di\u1ec5n' }; }), 'job'); } } }
