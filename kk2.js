(function () {
    ...
    function cleanAioBase(raw){ if(!raw)return''; raw=String(raw).trim().replace(/\/manifest\.json\s*$/i,'').replace(/\/+$/,''); return raw; }
    ...
    /* ══════════════════════════════════════════════
       STREAM PARSER  –  hỗ trợ Torrentio & AIOStreams
    ══════════════════════════════════════════════ */
    function parseStreamInfo(st) {
        /*  AIOStreams trả về:
              st.name  = "Aio 1080p"          ← tên ngắn
              st.title = "Aio 1080p\n...\n💾 14.0 GB\n👤 12 seeds"
            Torrentio trả về:
              st.name  = "[YIFY] ..."
              st.title = "[YIFY]...\n🎥 1080p / ...\n💾 2.3 GB\n👤 55 seeds"
        */
        ...
        var size = '';
        var sizeRe = [
            /💾\s*([\d.,]+\s*(?:TB|GB|GiB|GBytes?|MB|MiB|MBytes?))/i,
            /\b([\d.,]+)\s*(TB)\b/i,
            /\b([\d.,]+)\s*(GB|GiB|GBytes?)\b/i,
            /\b([\d.,]+)\s*(MB|MiB|MBytes?)\b/i
        ];
        for (var ri=0;ri<sizeRe.length;ri++){
            var sm = raw.match(sizeRe[ri]);
            if (sm){ size = sm[1] ? sm[1].trim() : (sm[1]+' '+sm[2]).trim(); break; }
        }
        ...
        for (var se=0;se<seedRe.length;se++){
            var sem = raw.match(seedRe[se]);
            if (sem){ seeds = sem[1]; break; }
        }

        /* --- quality --- */
        var quality = '';
        var qm = raw.match(/\b(2160p|4K|UHD|1080p|720p|480p|360p|HDR10?|BluRay|WEB-?DL|HDTV|CAM|DVDRip)\b/i);
        if (qm) quality = qm[1].toUpperCase();

        /* --- source label (inside []) --- */
        var srcLabel = '';
        var slm = name.match(/^\[([^\]]+)\]/);
        if (slm) srcLabel = slm[1];
    ...
    function formatStreamLabel(s) {
        /* dòng 1: tên, dòng 2: meta */
        var line1 = s.name;
        var meta  = [];
        if (s.quality)  meta.push(s.quality);
        if (s.size)     meta.push('💾 '+s.size);
        if (s.seeds)    meta.push('👤 '+s.seeds);
        return meta.length ? line1+'\n'+meta.join('  ') : line1;
    }
    ...
    async function tmdbSimilar(type,id,p){ return await tmdbFetch('/'+type+'/'+id+'/similar?language='+tLang()+'&page='+(p||1)); }
    async function tmdbRecommendations(type,id,p){ return await tmdbFetch('/'+type+'/'+id+'/recommendations?language='+tLang()+'&page='+(p||1)); }
    async function tmdbRandom(type){ var rPage=Math.floor(Math.random()*10)+1; return await tmdbFetch('/'+type+'/popular?language='+tLang()+'&page='+rPage); }
    ...
    async function playViaTS(stream,title,poster,fileIdx){ if(!getTSHost()){ Lampa.Noty.show('Chưa cấu hình TorrServer!'); return; } Lampa.Noty.show('Đang gửi TorrServer...'); try{ var u=tsUrl('/torrents'); var r=await fetch(u,{method:'POST',headers:tsHdr(),body:JSON.stringify({action:'add',link:buildMag(stream.infoHash),title:title||'',poster:poster||'',save_to_db:false})}); if(!r.ok)throw new Error('TS:'+r.status); var td=await r.json(); var hash=td.hash||stream.infoHash; await delay(2000); var info=null,rt=0; while(rt<3){ try{ var r2=await fetch(u,{method:'POST',headers:tsHdr(),body:JSON.stringify({action:'get',hash:hash})}); info=await r2.json(); if(info&&info.file_stats&&info.file_stats.length)break; }catch(e){} rt++; await delay(1500); } var files=[]; if(info&&info.file_stats)files=info.file_stats.filter(function(f){ return(f.path||'').toLowerCase().match(/\.(mp4|mkv|avi|mov|flv|webm|ts|m2ts)$/); }).sort(function(a,b){ return(a.id||0)-(b.id||0); }); if(!files.length)Lampa.Player.play({title:title,url:tsUrl('/stream/fname?link='+hash+'&index=0&play')}); else if(files.length===1)Lampa.Player.play({title:title,url:tsUrl('/stream/fname?link='+hash+'&index='+(files[0].id||0)+'&play')}); else if(fileIdx!==undefined&&fileIdx!==null&&fileIdx>=0)Lampa.Player.play({title:title,url:tsUrl('/stream/fname?link='+hash+'&index='+(files[fileIdx].id||fileIdx)+'&play')}); else{ Lampa.Select.show({title:'Chọn file',items:files.map(function(f){ return{title:(f.path||'').split('/').pop()+(f.length?' ('+(f.length/1048576).toFixed(0)+'MB)':''),value:f}; }),onSelect:function(a){ Lampa.Player.play({title:title,url:tsUrl('/stream/fname?link='+hash+'&index='+a.value.id+'&play')}); },onBack:function(){ Lampa.Controller.toggle('content'); }}); } }catch(e){ Lampa.Noty.show('Lỗi TS: '+(e.message||'')); } }
    ...
    function aioStreamUrl(type,imdbId,s,e){ var base=cleanAioBase(getAioUrl()); if(!base)return''; var t=type==='tv'?'series':'movie',id=imdbId; if(type==='tv'&&s&&e)id=imdbId+':'+s+':'+e; return base+'/stream/'+t+'/'+id+'.json'; }

    async function fetchStreams(type,imdbId,s,e){
        var engine=getTorrentEngine(),url;
        if(engine==='aio'){ url=aioStreamUrl(type,imdbId,s,e); if(!url)throw new Error('Chưa cấu hình AIOStreams'); }
        else{ url=tioUrl(type,imdbId,s,e); }
        var r=await fetch(url);
        if(!r.ok)throw new Error(engine+' HTTP '+r.status);
        var d=await r.json();
        /* Debug: log raw streams vào console */
        if(d.streams&&d.streams.length){ console.log('[KKPhim] Raw stream[0]:', JSON.stringify(d.streams[0])); }
        return(d.streams||[]).map(parseStreamInfo);
    }

    function showStreamResults(streams,title,poster){
        var ts=!!getTSHost();
        var eng=getTorrentEngine()==='aio'?'AIO':'Torrent';
        Lampa.Select.show({
            title: eng+': '+title+' ('+streams.length+')'+(ts?' → TS':''),
            items: streams.slice(0,40).map(function(s){
                return{ title: formatStreamLabel(s), value: s };
            }),
            onSelect:function(a){
                var s=a.value;
    ...
    async function openTorrentTV(tmdbId,title,poster,imdbId){ Lampa.Noty.show('Tải season...'); try{ var imdb=imdbId||await getImdbId('tv',tmdbId); if(!imdb){ Lampa.Noty.show('Không tìm thấy IMDB ID'); return; } var seasons=await getTmdbSeasons(tmdbId); if(seasons.length>1){ Lampa.Select.show({title:'Chọn Season',items:seasons.map(function(s){ return{title:s.name+(s.episode_count?' ('+s.episode_count+' tập)':''),value:s}; }),onSelect:function(a){ pickTorrentEp(a.value,imdb,title,poster); },onBack:function(){ Lampa.Controller.toggle('content'); }}); }else if(seasons.length===1)pickTorrentEp(seasons[0],imdb,title,poster); }catch(e){ Lampa.Noty.show('Lỗi: '+(e.message||'')); } }
    ...
    /* ── expand builders (buttons dọc, full-width) ── */
    ...
    /* Mỗi expand wrapper chiếm toàn bộ chiều rộng */
    function buildMovieExpand(srcKey,srcName,slug,title,cssClass){
        var wrap=$('<div style="width:100%"></div>'),btn=mkSrcBtn(cssClass,'▶ '+esc(srcName),'Bấm để xem'),box=$('<div class="kk-ep-box"></div>');
        wrap.append(btn).append(box); var loaded=false,open=false;
        bindEnter(btn,function(){ open=!open; btn.toggleClass('kk-open',open); box.toggleClass('kk-show',open); if(open&&!loaded){ loaded=true; box.html('<div class="kk-ep-ld">⏳ Đang tải...</div>'); fetchDetail(SOURCES[srcKey],slug).then(function(det){ if(!det||!det.episodes||!det.episodes.length){ box.html('<div class="kk-ep-er">❌ Không có tập</div>'); return; } box.empty(); fillEps(box,det.episodes,title); }).catch(function(e){ box.html('<div class="kk-ep-er">❌ '+esc(e.message||'Lỗi')+'</div>'); }); } });
        return wrap;
    }
    function buildTVExpand(srcKey,srcName,slug,title,origTitle,cssClass){
        var wrap=$('<div style="width:100%"></div>'),btn=mkSrcBtn(cssClass,'▶ '+esc(srcName),'Bấm để chọn season/tập'),box=$('<div class="kk-ep-box"></div>');
        wrap.append(btn).append(box); var loaded=false,open=false;
        bindEnter(btn,function(){ open=!open; btn.toggleClass('kk-open',open); box.toggleClass('kk-show',open); if(open&&!loaded){ loaded=true; box.html('<div class="kk-ep-ld">⏳ Đang tìm seasons...</div>'); var source=SOURCES[srcKey]; findAllSeasonSlugs(source,title,origTitle).then(function(entries){ if(!entries.length&&slug)entries=[{slug:slug,name:title,season:1,source:source}]; if(!entries.length){ box.html('<div class="kk-ep-er">❌ Không tìm thấy</div>'); return; } var sMap={}; entries.forEach(function(e){ if(!sMap[e.season])sMap[e.season]=[]; sMap[e.season].push(e); }); var sNums=Object.keys(sMap).map(Number).sort(function(a,b){ return a-b; }); if(sNums.length===1)loadSeason(box,sMap[sNums[0]],title,sNums[0],null); else showSeasons(box,sMap,sNums,title); }).catch(function(e){ box.html('<div class="kk-ep-er">❌ '+esc(e.message||'Lỗi')+'</div>'); }); } });
        return wrap;
    }
    function buildDetailExpand(episodes,title,srcName,cssClass){
        var wrap=$('<div style="width:100%"></div>'),total=0; (episodes||[]).forEach(function(sv){ total+=(sv.server_data||[]).length; });
        var btn=mkSrcBtn(cssClass,'▶ '+esc(srcName),total+' tập • Bấm để mở'),box=$('<div class="kk-ep-box"></div>');
        ...
        fillEps(box,episodes,title); var open=false;
        bindEnter(btn,function(){ open=!open; btn.toggleClass('kk-open',open); box.toggleClass('kk-show',open); });
        return wrap;
    }
    /* Torrent button – full width, dọc */
    function buildTorrentBtn(mediaType,tmdbId,title,poster,imdbId){
        var eng=getTorrentEngine();
        var label=eng==='aio'?'🌊 AIOStreams':'🧲 Torrent';
        if(getTSHost())label+=' → TS';
        ...
        if(mediaType==='movie') bindEnter(btn,function(){ openTorrentMovie(tmdbId,title,poster,imdbId); });
        else                    bindEnter(btn,function(){ openTorrentTV(tmdbId,title,poster,imdbId); });
        return $('<div style="width:100%"></div>').append(btn);
    }

    function showSeasons(container,sMap,sNums,title){ container.empty(); sNums.forEach(function(sn){ var item=$('<div class="kk-sn-it selector"><span class="kk-sn-nm">📺 Season '+sn+'</span><span class="kk-sn-bd">'+sMap[sn].length+' nguồn</span></div>'); bindEnter(item,function(){ loadSeason(container,sMap[sn],title,sn,function(){ showSeasons(container,sMap,sNums,title); }); }); container.append(item); }); }
    async function loadSeason(container,entries,title,sNum,backFn){ container.html('<div class="kk-ep-ld">⏳ Tải Season '+sNum+'...</div>'); for(var i=0;i<entries.length;i++){ try{ var det=await fetchDetail(entries[i].source,entries[i].slug); if(det&&det.episodes&&det.episodes.length){ container.empty(); if(backFn){ var bk=$('<div class="kk-ep-bk selector">← Quay lại</div>'); bindEnter(bk,backFn); container.append(bk); } fillEps(container,det.episodes,title+' S'+pad(sNum)); return; } }catch(e){} } container.html('<div class="kk-ep-er">❌ Không có tập</div>'); }
    function fillEps(container,episodes,title){ episodes.forEach(function(sv){ var sName=sv.server_name||'Server',cnt=(sv.server_data||[]).length,icon='📺',sn=sName.toLowerCase(); if(sn.indexOf('thuyết minh')>-1||sn.indexOf('thuyet minh')>-1)icon='🇻🇳'; else if(sn.indexOf('vietsub')>-1||sn.indexOf('sub')>-1)icon='📝'; else if(sn.indexOf('lồng')>-1||sn.indexOf('long')>-1)icon='🎤'; container.append('<div class="kk-sv-hd">'+icon+' '+esc(sName)+' ('+cnt+')</div>'); var grid=$('<div class="kk-ep-chips"></div>'); (sv.server_data||[]).forEach(function(ep){ var link=ep.link_m3u8||ep.link_embed||''; var chip=$('<div class="kk-ep-c selector'+(link?'':' off')+'">'+esc(ep.name||'Tập')+'</div>'); bindEnter(chip,function(){ if(link)Lampa.Player.play({title:title+' - '+(ep.name||''),url:link}); else Lampa.Noty.show('Không có link'); }); grid.append(chip); }); container.append(grid); }); }
    ...
    function startPlugin(){
        ...
        Lampa.Component.add('kkphim_settings',function(){
            ...
            this.create=function(){
                ...
                [{k:'2',n:'2 cột'},{k:'3',n:'3 cột'},{k:'4',n:'4 cột'}].forEach(function(opt){ g5.append(mkOpt(opt.n,'Số cột',curGrid===opt.k,function(){ saveSettings({card_style:opt.k}); comp.create(); })); }); w.append(g5);
                var g6=mkGroup('🌐 Ngôn ngữ TMDB'); var curLang=s.tmdb_lang||'vi-VN';
                [{k:'vi-VN',n:'Tiếng Việt'},{k:'en-US',n:'English'},{k:'ja-JP',n:'日本語'},{k:'ko-KR',n:'한국어'},{k:'zh-CN',n:'中文'}].forEach(function(opt){ g6.append(mkOpt(opt.n,opt.k,curLang===opt.k,function(){ saveSettings({tmdb_lang:opt.k}); _genreCache={movie:null,tv:null}; comp.create(); })); }); w.append(g6);
                var gE=mkGroup('🎯 Nguồn Torrent'); var curEng=s.torrent_engine||'torrentio';
                gE.append(mkOpt('Torrentio','torrentio.strem.fun',curEng==='torrentio',function(){ saveSettings({torrent_engine:'torrentio'}); comp.create(); }));
                gE.append(mkOpt('AIOStreams','Tự host / public AIO',curEng==='aio',function(){ saveSettings({torrent_engine:'aio'}); comp.create(); }));
                w.append(gE);
                ...
                gA.append(mkInput('Link manifest.json','Dán full URL',s.aio_url||'Chưa cài','AIO URL','aio_url',s));
                var stA=$('<div class="kk-stg-status" style="display:none"></div>');
                var tA=si('🧪 Kiểm tra AIO','Test Inception (tt1375666)','Nhấn');
                bindEnter(tA,async function(){
                    var base=cleanAioBase(getAioUrl());
                    if(!base){ stA.show().attr('class','kk-stg-status kk-stg-status--err').text('❌ Chưa nhập URL'); return; }
                    stA.show().attr('class','kk-stg-status kk-stg-status--loading').text('⏳ Đang test...');
                    try{
                        var url=base+'/stream/movie/tt1375666.json';
                        var r=await fetch(url);
                        if(!r.ok){ stA.attr('class','kk-stg-status kk-stg-status--err').text('❌ HTTP '+r.status); return; }
                        var d=await r.json();
                        var raw0=d.streams&&d.streams[0]?JSON.stringify(d.streams[0]):'';
                        console.log('[KKPhim] AIO test raw[0]:',raw0);
                        var parsed=(d.streams||[]).map(parseStreamInfo);
                        var withSize=parsed.filter(function(s){ return s.size; }).length;
                        var withSeeds=parsed.filter(function(s){ return s.seeds; }).length;
                        stA.attr('class','kk-stg-status kk-stg-status--ok').text('✅ '+parsed.length+' streams | size: '+withSize+' | seeds: '+withSeeds);
                    }catch(e){ stA.attr('class','kk-stg-status kk-stg-status--err').text('❌ '+(e.message||'')); }
                });
                gA.append(tA).append(stA); w.append(gA);
                var g7=mkGroup('🗃️ Cache'); var cc=si('Xóa cache','Đặt lại genres','Xóa'); bindEnter(cc,function(){ _genreCache={movie:null,tv:null}; Lampa.Noty.show('OK'); }); g7.append(cc); w.append(g7);
                var g4=$('<div class="kk-stg-group"></div>'); var cl=si('🗑️ Xóa toàn bộ','Khôi phục mặc định','Xóa'); cl.find('.kk-stg-value').css('color','#f87171'); bindEnter(cl,function(){ localStorage.removeItem(SETTINGS_KEY); _genreCache={movie:null,tv:null}; Lampa.Activity.backward(); }); g4.append(cl); w.append(g4);
                w.append('<div class="kk-stg-ver">KKPhim v2.9</div>'); scroll.append(w); comp.start();
            };
        ...
        Lampa.Component.add('kkphim_person',function(obj){
            ...
            this.create=function(){ comp.activity.loader(true); clearScroll(scroll); if(!personId){ comp.activity.loader(false); comp.start(); return; }
                Promise.all([tmdbFetch('/person/'+personId+'?language='+tLang()),tmdbPersonCredits(personId)]).then(function(res){
                    var person=res[0],credits=res[1],w=$('<div class="kk-detail-wrap"></div>');
                    var av=person.profile_path?TMDB_IMG_W500+person.profile_path:'';
                    w.append('<div class="kk-person-header"><div class="kk-person-avatar">'+(av?'<img src="'+av+'">' :'')+' </div><div class="kk-person-info"><div class="kk-person-name">'+esc(person.name||'')+'</div>'+(person.birthday?'<div class="kk-person-meta">🎂 '+esc(person.birthday)+'</div>':'')+(person.known_for_department?'<div class="kk-person-meta">🎬 '+esc(person.known_for_department)+'</div>':'')+'</div></div>');
                    if(person.biography)w.append('<div class="kk-person-bio">'+fmtTxt((person.biography||'').substring(0,600))+(person.biography.length>600?'...':'')+'</div>');
                    if(credits&&credits.cast){
                        ...
                        if(movies.length){ var r1=$('<div class="kk-row"></div>'),rl1=$('<div class="kk-row-list"></div>'),m1=$('<div class="kk-row-more selector">Xem thêm ('+movies.length+')</div>'); bindEnter(m1,function(){ Lampa.Activity.push({url:'',title:(person.name||'')+' - Phim lẻ',component:'kkphim_person_films',person_id:personId,film_type:'movie',page:1}); }); movies.slice(0,10).forEach(function(c){ rl1.append(mkTmdbCard(c)); }); r1.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">🎬 Phim lẻ ('+movies.length+')</div>').append(m1)).append(rl1); w.append(r1); }
                        if(tvs.length){ var r2=$('<div class="kk-row"></div>'),rl2=$('<div class="kk-row-list"></div>'),m2=$('<div class="kk-row-more selector">Xem thêm ('+tvs.length+')</div>'); bindEnter(m2,function(){ Lampa.Activity.push({url:'',title:(person.name||'')+' - Phim bộ',component:'kkphim_person_films',person_id:personId,film_type:'tv',page:1}); }); tvs.slice(0,10).forEach(function(c){ rl2.append(mkTmdbCard(c)); }); r2.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">📺 Phim bộ ('+tvs.length+')</div>').append(m2)).append(rl2); w.append(r2); }
                    }
        ...
        Lampa.Component.add('kkphim_person_films',function(obj){
            var scroll=new Lampa.Scroll({mask:true,over:true}),comp=this,personId=obj.person_id,filmType=obj.film_type||'movie';
            var allFilms=[],rendered=0,perPage=20,grid=$('<div class="kk-grid"></div>'),lm=$('<div class="kk-loadmore selector">Tải thêm</div>');
            this.create=function(){ comp.activity.loader(true); clearScroll(scroll); grid.css('grid-template-columns','repeat('+getCardStyle()+',minmax(0,1fr))'); scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+esc(obj.title||'Phim')+'</div>').append(grid).append(lm)); bindEnter(lm,function(){ renderMore(); });
                tmdbPersonCredits(personId).then(function(credits){ if(credits&&credits.cast)allFilms=credits.cast.filter(function(c){ return c.media_type===filmType; }).sort(function(a,b){ return(b.popularity||0)-(a.popularity||0); }); renderMore(); comp.activity.loader(false); comp.start(); }).catch(function(){ comp.activity.loader(false); lm.text('Lỗi'); });
            };
            function renderMore(){ var batch=allFilms.slice(rendered,rendered+perPage); if(!batch.length){ lm.text('Hết'); return; } batch.forEach(function(c){ grid.append(mkTmdbCard(c).addClass('kk-card--grid')); }); rendered+=batch.length; lm.text(rendered>=allFilms.length?'Hết ('+allFilms.length+')':'Tải thêm ('+rendered+'/'+allFilms.length+')'); }
            this.start=function(){ applyCtrl(scroll); enableScroll(scroll); }; this.pause=function(){}; this.stop=function(){}; this.render=function(){ return scroll.render(); }; this.destroy=function(){ scroll.destroy(); };
        });
        ...
        Lampa.Component.add('kkphim_tmdb_main',function(){
            ...
            this.create=function(){
                ...
                var sb=$('<div class="kk-srcbar"></div>'); Object.keys(SOURCES).forEach(function(k){ var s=SOURCES[k]; var btn=$('<div class="kk-srcbtn selector kk-srcbtn--off">'+esc(s.name)+'</div>'); bindEnter(btn,function(){ saveSettings({source:k}); Lampa.Activity.push({url:'',title:'KKPhim',component:'kkphim_main',page:1}); }); sb.append(btn); }); sb.append('<div class="kk-srcbtn kk-srcbtn--on" style="background:rgba(1,180,228,.25);border-color:rgba(1,180,228,.5);color:#01b4e4">TMDB</div>'); scroll.append(sb);
                /* Random row */
                var rndRow=$('<div class="kk-row"></div>'),rndMore=$('<div class="kk-row-more selector">Xem thêm</div>'),rndList=$('<div class="kk-row-list"></div>');
                bindEnter(rndMore,function(){ Lampa.Activity.push({url:'',title:'🎲 Ngẫu nhiên',component:'kkphim_tmdb_list',listType:'trending',page_num:Math.floor(Math.random()*5)+2}); });
                rndRow.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">🎲 Khám phá ngẫu nhiên</div>').append(rndMore)).append(rndList); scroll.append(rndRow);
                Promise.all([tmdbRandom('movie'),tmdbRandom('tv')]).then(function(res){ var items=[]; (res[0].results||[]).forEach(function(i){ i.media_type='movie'; items.push(i); }); (res[1].results||[]).forEach(function(i){ i.media_type='tv'; items.push(i); }); for(var si=items.length-1;si>0;si--){ var sj=Math.floor(Math.random()*(si+1)); var st=items[si]; items[si]=items[sj]; items[sj]=st; } items.slice(0,12).forEach(function(i){ rndList.append(mkTmdbCard(i)); }); }).catch(function(){});
                var loaded=0; sections.forEach(function(sec){ var fn=TMDB_FN[sec.lt]; if(!fn){ loaded++; return; } fn(1).then(function(res){ var items=(res.results||[]).filter(function(i){ return i.media_type!=='person'; }); if(items.length){ var row=$('<div class="kk-row"></div>'),more=$('<div class="kk-row-more selector">Xem thêm</div>'),rl=$('<div class="kk-row-list"></div>'); bindEnter(more,function(){ Lampa.Activity.push({url:'',title:sec.name,component:'kkphim_tmdb_list',listType:sec.lt,page_num:1}); }); items.slice(0,12).forEach(function(i){ rl.append(mkTmdbCard(i)); }); row.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">'+esc(sec.name)+'</div>').append(more)).append(rl); scroll.append(row); } loaded++; if(loaded>=sections.length){ comp.activity.loader(false); comp.start(); } }).catch(function(){ loaded++; if(loaded>=sections.length){ comp.activity.loader(false); comp.start(); } }); });
            };
            this.start=function(){ applyCtrl(scroll); enableScroll(scroll); }; this.pause=function(){}; this.stop=function(){}; this.render=function(){ return scroll.render(); }; this.destroy=function(){ scroll.destroy(); };
        });
        ...
        Lampa.Component.add('kkphim_tmdb_genre',function(obj){
            ...
            var grid=$('<div class="kk-grid"></div>'),loading=false,movieDone=false,tvDone=false,moviePage=1,tvPage=1,allItems=[],renderedSet={};
            this.create=function(){ comp.activity.loader(true); clearScroll(scroll); grid.css('grid-template-columns','repeat('+getCardStyle()+',minmax(0,1fr))'); var genreBar=$('<div class="kk-genre-bar"></div>'); scroll.append(genreBar); scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title" id="kk-gtitle">'+esc(obj.title||'Thể loại')+'</div>').append(grid));
                var scrollBody=scroll.render().find('.scroll__body');
                scrollBody.on('scroll',function(){ if(loading||(movieDone&&tvDone))return; var el=scrollBody[0]; if(el.scrollTop+el.clientHeight>=el.scrollHeight-400)doLoad(); });
                Promise.all([loadGenres('movie'),loadGenres('tv')]).then(function(res){ var merged=[],seen={}; (res[0]||[]).concat(res[1]||[]).forEach(function(g){ if(!seen[g.id]){ seen[g.id]=true; merged.push(g); } }); merged.sort(function(a,b){ return(a.name||'').localeCompare(b.name||''); }); merged.forEach(function(g){ var on=String(g.id)===curGid; var chip=$('<div class="kk-genre-chip selector '+(on?'kk-genre-chip--on':'kk-genre-chip--off')+'">'+esc(g.name)+'</div>'); bindEnter(chip,function(){ Lampa.Activity.push({url:'',title:g.name,component:'kkphim_tmdb_genre',genre_id:g.id,page_num:1}); }); genreBar.append(chip); }); var cur=merged.find(function(g){ return String(g.id)===curGid; }); if(cur)scroll.render().find('#kk-gtitle').text(cur.name); doLoad(); }).catch(function(){ doLoad(); });
            };
            function doLoad(){ if(loading)return; loading=true; var promises=[]; if(!movieDone)promises.push(tmdbDiscover('movie',curGid,moviePage).then(function(r){ var items=r.results||[]; if(!items.length)movieDone=true; else{ items.forEach(function(i){ i.media_type='movie'; }); allItems=allItems.concat(items); moviePage++; } }).catch(function(){ movieDone=true; })); if(!tvDone)promises.push(tmdbDiscover('tv',curGid,tvPage).then(function(r){ var items=r.results||[]; if(!items.length)tvDone=true; else{ items.forEach(function(i){ i.media_type='tv'; }); allItems=allItems.concat(items); tvPage++; } }).catch(function(){ tvDone=true; })); Promise.all(promises).then(function(){ allItems.sort(function(a,b){ return(b.popularity||0)-(a.popularity||0); }); for(var i=0;i<allItems.length;i++){ var key=allItems[i].media_type+'_'+allItems[i].id; if(!renderedSet[key]){ renderedSet[key]=true; grid.append(mkTmdbCard(allItems[i]).addClass('kk-card--grid')); } } loading=false; comp.activity.loader(false); comp.start(); }).catch(function(){ loading=false; comp.activity.loader(false); }); }
            this.start=function(){ applyCtrl(scroll); enableScroll(scroll); }; this.pause=function(){}; this.stop=function(){}; this.render=function(){ return scroll.render(); }; this.destroy=function(){ scroll.destroy(); };
        });
        ...
        Lampa.Component.add('kkphim_tmdb_detail',function(obj){
            ...
            this.create=function(){ comp.activity.loader(true); clearScroll(scroll); if(!tmdbId){ comp.activity.loader(false); comp.start(); return; }
            ...
            async function buildDetail(tmdb,logos,slugs){
                ...
                var epTotal=tmdb.number_of_episodes?tmdb.number_of_episodes+' tập':'';
                ...
                /* Nút 1: KKPhim */
                if(slugs.kkphim){
                    if(mediaType==='movie') actions.append(buildMovieExpand('kkphim','KKPhim',slugs.kkphim,t,'kk-src-btn--kkphim'));
                    else                    actions.append(buildTVExpand('kkphim','KKPhim',slugs.kkphim,t,o,'kk-src-btn--kkphim'));
                }else{
                    actions.append('<div class="kk-src-btn kk-src-btn--no">KKPhim – Không tìm thấy</div>');
                }
                /* Nút 2: OPhim */
                if(slugs.ophim){
                    if(mediaType==='movie') actions.append(buildMovieExpand('ophim','OPhim',slugs.ophim,t,'kk-src-btn--ophim'));
                    else                    actions.append(buildTVExpand('ophim','OPhim',slugs.ophim,t,o,'kk-src-btn--ophim'));
                }else{
                    actions.append('<div class="kk-src-btn kk-src-btn--no">OPhim – Không tìm thấy</div>');
                }
                /* Nút 3: Torrent / AIO */
                actions.append(buildTorrentBtn(mediaType,tmdbId,t,ps,imdbId));
                ...
                /* Similar 20 */
                var simItems=(tmdb.similar&&tmdb.similar.results)?tmdb.similar.results.slice(0,20):[];
                if(simItems.length){ var simSec=$('<div class="kk-section kk-similar"></div>').append('<div class="kk-block-title">Phim tương tự</div>'); var simList=$('<div class="kk-similar-list"></div>'); simItems.forEach(function(i){ if(!i.media_type)i.media_type=mediaType; simList.append(mkTmdbCard(i)); }); simSec.append(simList); dw.append(simSec); }
                /* Recommendations */
                try{ var recData=await tmdbRecommendations(mediaType,tmdbId,1); var recItems=(recData.results||[]).slice(0,20); if(recItems.length){ var recSec=$('<div class="kk-section kk-similar kk-section--last"></div>').append('<div class="kk-block-title">🎲 Gợi ý cho bạn</div>'); var recList=$('<div class="kk-similar-list"></div>'); recItems.forEach(function(i){ if(!i.media_type)i.media_type=mediaType; recList.append(mkTmdbCard(i)); }); recSec.append(recList); dw.append(recSec); }else{ dw.find('.kk-similar').last().addClass('kk-section--last'); } }catch(e){ dw.find('.kk-similar').last().addClass('kk-section--last'); }
        ...
        Lampa.Component.add('kkphim_detail',function(obj){
            ...
            function build(data,episodes,tmdb,logos,ttype){
                ...
                var pCats=data.category||[]; if(pCats.length)ghtml=pCats.map(function(g){ return g?'<span class="kk-genre selector" data-slug="'+esc(g.slug||'')+'" data-title="'+esc(g.name||'')+'">'+esc(g.name||'')+'</span>':''; }).join('');
                ...
                /* Actions – dọc */
                var actions=$('<div class="kk-actions"></div>');
                var curSrc=getSource(),srcCss=curSrc.key==='kkphim'?'kk-src-btn--kkphim':'kk-src-btn--ophim';
                ...
                if(hasTmdb) actions.append(buildTorrentBtn(ttype,getTmdbId(data),data.name||t,ps,null));
                body.append(actions);
                var dw=$('<div class="kk-detail-wrap"></div>').append(hero).append(body);
                if(dirH){ var ds=$('<div class="kk-section"><div class="kk-block-title">Đạo diễn</div><div class="kk-cast-list">'+dirH+'</div></div>'); bindCastClicks(ds); dw.append(ds); }
                if(castH){ var cs=$('<div class="kk-section"><div class="kk-block-title">Diễn viên</div><div class="kk-cast-list">'+castH+'</div></div>'); bindCastClicks(cs); dw.append(cs); }
                /* Phim liên quan từ cùng category */
                if(pCats&&pCats.length&&pCats[0]&&pCats[0].slug){
                    var simSec=$('<div class="kk-section kk-similar"></div>').append('<div class="kk-block-title">Phim liên quan</div>');
                    var simList=$('<div class="kk-similar-list"><div class="kk-ep-ld">⏳</div></div>'); simSec.append(simList); dw.append(simSec);
                    fetch(SRC_API()+'v1/api/the-loai/'+pCats[0].slug+'?page=1').then(function(r){ return r.json(); }).then(function(res){ var items=((res&&res.data&&res.data.items)||(res&&res.items)||[]).map(norm).filter(function(i){ return i&&i.slug&&i.slug!==data.slug; }); simList.empty(); if(items.length)items.slice(0,20).forEach(function(i){ simList.append(mkCard(i)); }); else simList.append('<div class="kk-ep-ld">Không có</div>'); }).catch(function(){ simList.empty(); });
                }
                /* TMDB similar */
                if(hasTmdb){
                    var tid2=getTmdbId(data);
                    var tSimSec=$('<div class="kk-section kk-similar kk-section--last"></div>').append('<div class="kk-block-title">🎬 TMDB liên quan</div>');
                    var tSimList=$('<div class="kk-similar-list"><div class="kk-ep-ld">⏳</div></div>'); tSimSec.append(tSimList); dw.append(tSimSec);
                    tmdbSimilar(ttype,tid2,1).then(function(res){ var items=(res.results||[]).slice(0,20); tSimList.empty(); if(items.length)items.forEach(function(i){ if(!i.media_type)i.media_type=ttype; tSimList.append(mkTmdbCard(i)); }); else tSimList.append('<div class="kk-ep-ld">Không có</div>'); }).catch(function(){ tSimList.empty(); });
                }
                scroll.append(dw);
            }
