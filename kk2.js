...
function oTSearch(){function go(kw){kw=String(kw||'').trim();if(kw)Lampa.Activity.push({url:'',title:'TMDB: '+kw,component:'kkphim_tmdb_search',keyword:kw,page_num:1});}try{if(Lampa.Input&&Lampa.Input.edit){Lampa.Input.edit({title:'Tìm phim TMDB',value:'',free:true},go);return;}}catch(e){}go(window.prompt('Tìm phim TMDB:'));}

/* ── stream parser ── */
function pStream(st){
    var raw=st.title||'';
    var name=(st.name||raw.split('\n')[0]||'?').trim();
    var size='',seeds='',qual='';
    var sr=[/💾\s*([\d.,]+\s*(?:TB|GB|GiB|MB|MiB))/i,/\b([\d.,]+\s*(?:TB|GB|GiB))\b/i,/\b([\d.,]+\s*(?:MB|MiB))\b/i];
    for(var i=0;i<sr.length;i++){var sm=raw.match(sr[i]);if(sm){size=sm[1].trim();break;}}
    var sp=[/👤\s*(\d+)/,/(\d+)\s*seed/i,/Seeds?:?\s*(\d+)/i];
    for(var j=0;j<sp.length;j++){var se=raw.match(sp[j]);if(se){seeds=se[1];break;}}
    var qm=raw.match(/\b(2160p|4K|UHD|1080p|720p|480p|HDR|BluRay|WEB-?DL|HDTV|CAM)\b/i);
    if(qm)qual=qm[1].toUpperCase();
    return{name:name,infoHash:st.infoHash||'',fileIdx:st.fileIdx,url:st.url||'',size:size,seeds:seeds,quality:qual,raw:raw};
}
function fmtStream(s){
    var m=[];
    if(s.quality)m.push(s.quality);
    if(s.size)m.push('💾 '+s.size);
    if(s.seeds)m.push('👤 '+s.seeds);
    return m.length?s.name+'\n'+m.join('  '):s.name;
}
...
function mBest(items,title,orig,year){
    ...
    for(var i=0;i<items.length;i++){var n1=nStr(items[i].name||items[i].title||''),n2=nStr(items[i].origin_name||items[i].original_name||'');if((nT&&(n1===nT||n2===nT))||(nO&&(n1===nO||n2===nO))){if(!year||!items[i].year||String(items[i].year)===String(year))return items[i];}}
    for(var j=0;j<items.length;j++){var m1=nStr(items[j].name||items[j].title||''),m2=nStr(items[j].origin_name||items[j].original_name||'');if((nT&&(m1.indexOf(nT)>-1||nT.indexOf(m1)>-1))||(nO&&(m2.indexOf(nO)>-1||nO.indexOf(m2)>-1))){if(!year||!items[j].year||String(items[j].year)===String(year))return items[j];}}
    return null;
}
...
async function fSeasonSlugs(source,title,orig){
    var results=[];try{var items=await sSrc(source,title);if(!items.length&&orig)items=await sSrc(source,orig);var nT=nStr(title),nO=nStr(orig);
    for(var i=0;i<items.length;i++){var it=items[i];if(!it.slug)continue;var n1=nStr(it.name||it.title||''),n2=nStr(it.origin_name||it.original_name||'');var match=false;
    ...
    if(match)results.push({slug:it.slug,name:it.name||it.title||'',season:exSn(it.name||it.title||'',it.slug||''),source:source});}}catch(e){}return results;
}
...
async function playTS(stream,title,poster,fi){
    ...
    try{var u=tsU('/torrents');var r=await fetch(u,{method:'POST',headers:tsH(),body:JSON.stringify({action:'add',link:bMag(stream.infoHash),title:title||'',poster:poster||'',save_to_db:false})});if(!r.ok)throw new Error('TS:'+r.status);var td=await r.json();var hash=td.hash||stream.infoHash;await dly(2000);
    var info=null,rt=0;while(rt<3){try{var r2=await fetch(u,{method:'POST',headers:tsH(),body:JSON.stringify({action:'get',hash:hash})});info=await r2.json();if(info&&info.file_stats&&info.file_stats.length)break;}catch(e){}rt++;await dly(1500);}
...
async function fStreams(type,imdb,s,e){
    ...
    var r=await fetch(url);if(!r.ok)throw new Error(eng+' '+r.status);var d=await r.json();
    if(d.streams&&d.streams.length)console.log('[KK] raw[0]:',JSON.stringify(d.streams[0]));
    return(d.streams||[]).map(pStream);
}
function showStr(streams,title,poster){
    var ts=!!tsHost(),eng=tEngine()==='aio'?'AIO':'Torrent';
    Lampa.Select.show({title:eng+': '+title+' ('+streams.length+')'+(ts?' → TS':''),
        items:streams.slice(0,40).map(function(s){return{title:fmtStream(s),value:s};}),
        onSelect:function(a){var s=a.value;if(ts&&s.infoHash)playTS(s,title,poster,s.fileIdx);else if(s.url)Lampa.Player.play({title:title,url:s.url});else Lampa.Noty.show(s.infoHash?'Chưa cấu hình TS!':'Không có link');},
        onBack:function(){Lampa.Controller.toggle('content');}});
}
async function oTorMov(tid,title,poster,imdb){Lampa.Noty.show('Tìm torrent...');try{var id=imdb||await gImdb('movie',tid);if(!id){Lampa.Noty.show('Không có IMDB');return;}var st=await fStreams('movie',id);if(!st.length){Lampa.Noty.show('Không có');return;}showStr(st,title,poster);}catch(e){Lampa.Noty.show('Lỗi: '+(e.message||''));}}
...
/* ── cards ── */
function mkPpl(list,key,click){
    return(list||[]).map(function(p){
        var av=p.profile_path?'<img src="'+TMDB_W500+p.profile_path+'">':'<div class="kk-cast-empty"></div>';
        var cls=(click&&p.id)?' selector" data-person-id="'+p.id+'"':'"';
        return'<div class="kk-cast-card'+cls+'><div class="kk-cast-img">'+av+'</div><div class="kk-cast-name">'+E(p.name||'')+'</div><div class="kk-cast-role">'+E(p[key]||'')+'</div></div>';
    }).join('');
}
function bCast(c){c.find('.kk-cast-card[data-person-id]').each(function(){var card=$(this);bE(card,function(){var pid=card.attr('data-person-id');if(pid)Lampa.Activity.push({url:'',title:card.find('.kk-cast-name').text()||'',component:'kkphim_person',person_id:parseInt(pid),page:1});});});}

function mkC(item){
    var n=nm(item);if(!n)return $('<div></div>');var p=fImg(n.poster_url||n.thumb_url);
    var h='<div class="kk-card selector"><div class="kk-card-img"><img src="'+p+'">';
    if(n.quality)h+='<div class="kk-card-q">'+E(n.quality)+'</div>';
    if(n.episode_current)h+='<div class="kk-card-ep">'+E(n.episode_current)+'</div>';
    h+='</div><div class="kk-card-name">'+E(n.name)+'</div><div class="kk-card-year">'+E(n.year)+'</div></div>';
    var c=$(h);bE(c,function(){if(n.slug)Lampa.Activity.push({url:'',title:n.name||'',component:'kkphim_detail',movie:n,page:1});});return c;
}
function mkTC(item){
    var d=tNorm(item);if(!d||!d.tmdb_id)return $('<div></div>');
    var img=d.poster_url?'<img src="'+d.poster_url+'">':'<div style="width:100%;height:100%;background:#333"></div>';
    var h='<div class="kk-card selector"><div class="kk-card-img">'+img;
    if(d.vote)h+='<div class="kk-card-q">⭐'+E(d.vote)+'</div>';
    if(d.media_type==='tv')h+='<div class="kk-card-ep">TV</div>';
    h+='</div><div class="kk-card-name">'+E(d.name)+'</div><div class="kk-card-year">'+E(d.year)+'</div></div>';
    var c=$(h);bE(c,function(){Lampa.Activity.push({url:'',title:d.name||'',component:'kkphim_tmdb_detail',tmdb_id:d.tmdb_id,media_type:d.media_type,page:1});});return c;
}
...
function bDetExp(eps,title,sn,css){
    ...
    if(total===1){var ep=gEp1(eps);if(ep){var link=ep.link_m3u8||ep.link_embed||'';btn.find('.kk-sb-main').html('▶ '+E(sn));btn.find('.kk-sb-sub').text('Phát ngay');btn.find('.kk-arrow').remove();bE(btn,function(){if(link)Lampa.Player.play({title:title,url:link});else Lampa.Noty.show('Không có link');});return w;}}
    fillE(box,eps,title);var op=false;bE(btn,function(){op=!op;btn.toggleClass('kk-open',op);box.toggleClass('kk-show',op);});return w;
}
function bTorBtn(mt,tid,title,poster,imdb){
    var eng=tEngine(),label=eng==='aio'?'🌊 AIOStreams':'🧲 Torrent';if(tsHost())label+=' → TS';
    var css=eng==='aio'?'kk-src-btn--aio':'kk-src-btn--torrent';
    var btn=$('<div class="kk-src-btn '+css+' selector" style="width:100%"><div class="kk-sb-main">'+label+'</div><div class="kk-sb-sub">Phát qua torrent</div></div>');
...
function gHtml(genres,isTmdb){
    ...
    return genres.map(function(g){
        if(!g)return'';
        if(isTmdb){
            return'<span class="kk-genre selector" data-gid="'+g.id+'" data-gname="'+E(g.name||'')+'">'+E(g.name||'')+'</span>';
        }else{
            return'<span class="kk-genre selector" data-slug="'+E(g.slug||'')+'" data-title="'+E(g.name||'')+'">'+E(g.name||'')+'</span>';
        }
    }).join('');
}

/* ── detail builder helpers ── */
function mkHero(bk,ps,logoH,titleH,origin){
    return $('<div class="kk-hero">'+
        '<div class="kk-hero-bg">'+(bk?'<img src="'+bk+'">':'')+'<div class="kk-hero-mask"></div></div>'+
        '<div class="kk-hero-bottom"><div class="kk-hero-flex">'+
        '<div class="kk-hero-poster">'+(ps?'<img src="'+ps+'">':'')+'</div>'+
        '<div class="kk-hero-info">'+logoH+titleH+'<div class="kk-origin">'+E(origin)+'</div></div>'+
        '</div></div></div>');
}
function mkBody(v,y,rt,extra,genres,crewH,desc){
    var metas='<span class="kk-meta">⭐ '+E(v)+'</span>';
    if(y)metas+='<span class="kk-meta">📅 '+E(y)+'</span>';
    if(rt)metas+='<span class="kk-meta">⏱ '+E(rt)+'</span>';
    if(extra)metas+='<span class="kk-meta">🎬 '+E(extra)+'</span>';
    return $('<div class="kk-body"><div class="kk-metas">'+metas+'</div><div class="kk-genres">'+genres+'</div>'+crewH+'<div class="kk-desc">'+fTxt(desc)+'</div></div>');
}
...
function startPlugin(){
    ...
    Lampa.Component.add('kkphim_settings',function(){
        ...
        this.create=function(){
            cScr(scroll);var s=ls(),w=$('<div class="kk-stg-wrap"></div>');w.append('<div class="kk-stg-title">⚙️ Cài đặt</div>');
            var g0=mg('📺 Nguồn phim');var cur=s.source||'ophim';Object.keys(SOURCES).forEach(function(k){var sc=SOURCES[k];g0.append(mo(sc.name,'API: '+sc.api,cur===k,function(){ss({source:k});Lampa.Noty.show(sc.name);comp.create();}));});w.append(g0);
            var g5=mg('🎨 Giao diện');var cg=s.card_style||'3';[{k:'2',n:'2 cột'},{k:'3',n:'3 cột'},{k:'4',n:'4 cột'}].forEach(function(o){g5.append(mo(o.n,'Số cột',cg===o.k,function(){ss({card_style:o.k});comp.create();}));});w.append(g5);
            var g6=mg('🌐 Ngôn ngữ TMDB');var cl2=s.tmdb_lang||'vi-VN';[{k:'vi-VN',n:'Tiếng Việt'},{k:'en-US',n:'English'},{k:'ja-JP',n:'日本語'},{k:'ko-KR',n:'한국어'},{k:'zh-CN',n:'中文'}].forEach(function(o){g6.append(mo(o.n,o.k,cl2===o.k,function(){ss({tmdb_lang:o.k});_gc={movie:null,tv:null};comp.create();}));});w.append(g6);
            var gE=mg('🎯 Nguồn Torrent');var ce=s.torrent_engine||'torrentio';gE.append(mo('Torrentio','torrentio.strem.fun',ce==='torrentio',function(){ss({torrent_engine:'torrentio'});comp.create();}));gE.append(mo('AIOStreams','Tự host',ce==='aio',function(){ss({torrent_engine:'aio'});comp.create();}));w.append(gE);
            var g1=mg('🖥️ TorrServer');g1.append(mi('Địa chỉ','192.168.1.100:8090',s.torrserver_host||'Chưa cài','Địa chỉ TS','torrserver_host',s));g1.append(mi('Mật khẩu','Để trống nếu không',s.torrserver_password?'••••':'Không','Mật khẩu','torrserver_password',s));w.append(g1);
            var g2=mg('🧲 Torrentio');g2.append(mi('Config','Dán link',s.torrentio_config?'Có':'Mặc định','Config','torrentio_config',s));w.append(g2);
            var gA=mg('🌊 AIOStreams');gA.append(mi('URL','Dán full URL',s.aio_url||'Chưa cài','AIO URL','aio_url',s));
            var stA=$('<div class="kk-stg-status" style="display:none"></div>');
            var tA=si2('🧪 Test AIO','tt1375666','Nhấn');
            bE(tA,async function(){var base=cAio(aioUrl());if(!base){stA.show().attr('class','kk-stg-status kk-stg-status--err').text('❌ Chưa nhập');return;}stA.show().attr('class','kk-stg-status kk-stg-status--loading').text('⏳...');try{var r=await fetch(base+'/stream/movie/tt1375666.json');if(!r.ok){stA.attr('class','kk-stg-status kk-stg-status--err').text('❌ '+r.status);return;}var d=await r.json();if(d.streams&&d.streams.length)console.log('[KK] AIO raw[0]:',JSON.stringify(d.streams[0]));var ps2=(d.streams||[]).map(pStream),ws=ps2.filter(function(s){return s.size;}).length;stA.attr('class','kk-stg-status kk-stg-status--ok').text('✅ '+ps2.length+' | size:'+ws);}catch(e){stA.attr('class','kk-stg-status kk-stg-status--err').text('❌ '+(e.message||''));}});
            gA.append(tA).append(stA);w.append(gA);
            var g7=mg('🗃️ Cache');var cc=si2('Xóa cache','','Xóa');bE(cc,function(){_gc={movie:null,tv:null};Lampa.Noty.show('OK');});g7.append(cc);w.append(g7);
            var g4=$('<div class="kk-stg-group"></div>');var cl=si2('🗑️ Xóa toàn bộ','','Xóa');cl.find('.kk-stg-value').css('color','#f87171');bE(cl,function(){localStorage.removeItem(STG_KEY);_gc={movie:null,tv:null};Lampa.Activity.backward();});g4.append(cl);w.append(g4);
            w.append('<div class="kk-stg-ver">KKPhim v2.9.1</div>');scroll.append(w);comp.start();
        };
    ...
    Lampa.Component.add('kkphim_person',function(obj){
        ...
        this.create=function(){comp.activity.loader(true);cScr(scroll);if(!pid){comp.activity.loader(false);comp.start();return;}
        Promise.all([tFetch('/person/'+pid+'?language='+tmLang()),tPersonC(pid)]).then(function(res){
            var p=res[0],cr=res[1],w=$('<div class="kk-detail-wrap"></div>');var av=p.profile_path?TMDB_W500+p.profile_path:'';
            ...
            if(cr&&cr.cast){var mv=cr.cast.filter(function(c){return c.media_type==='movie';}).sort(function(a,b){return(b.popularity||0)-(a.popularity||0);});var tv=cr.cast.filter(function(c){return c.media_type==='tv';}).sort(function(a,b){return(b.popularity||0)-(a.popularity||0);});
            if(mv.length){var r1=$('<div class="kk-row"></div>'),rl=$('<div class="kk-row-list"></div>'),mr=$('<div class="kk-row-more selector">Thêm ('+mv.length+')</div>');bE(mr,function(){Lampa.Activity.push({url:'',title:p.name+' - Movie',component:'kkphim_person_films',person_id:pid,film_type:'movie',page:1});});mv.slice(0,10).forEach(function(c){rl.append(mkTC(c));});r1.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">🎬 Phim lẻ</div>').append(mr)).append(rl);w.append(r1);}
            if(tv.length){var r2=$('<div class="kk-row"></div>'),rl2=$('<div class="kk-row-list"></div>'),mr2=$('<div class="kk-row-more selector">Thêm ('+tv.length+')</div>');bE(mr2,function(){Lampa.Activity.push({url:'',title:p.name+' - TV',component:'kkphim_person_films',person_id:pid,film_type:'tv',page:1});});tv.slice(0,10).forEach(function(c){rl2.append(mkTC(c));});r2.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">📺 Phim bộ</div>').append(mr2)).append(rl2);w.append(r2);}}
            scroll.append(w);comp.activity.loader(false);comp.start();}).catch(function(){comp.activity.loader(false);});};
        this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){scroll.destroy();};
    });
    ...
    Lampa.Component.add('kkphim_person_films',function(obj){
        ...
        this.create=function(){comp.activity.loader(true);cScr(scroll);grid.css('grid-template-columns','repeat('+cardSt()+',minmax(0,1fr))');scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+E(obj.title||'')+'</div>').append(grid).append(lm));bE(lm,function(){rMore();});
        tPersonC(pid).then(function(cr){if(cr&&cr.cast)all=cr.cast.filter(function(c){return c.media_type===ft;}).sort(function(a,b){return(b.popularity||0)-(a.popularity||0);});rMore();comp.activity.loader(false);comp.start();}).catch(function(){comp.activity.loader(false);});};
    ...
    Lampa.Component.add('kkphim_main',function(){
        ...
        this.create=function(){net.clear();this.activity.loader(true);cScr(scroll);var sc=src();_s=sc.key;
        var tb=$('<div class="kk-topbar"><div class="kk-topbar-title">'+E(sc.name)+'</div><div class="kk-topbar-btns"><div class="kk-btn selector">🔍</div><div class="kk-btn selector">⚙️</div></div></div>');bE($(tb.find('.kk-btn')[0]),oSearch);bE($(tb.find('.kk-btn')[1]),function(){Lampa.Activity.push({url:'',title:'Cài đặt',component:'kkphim_settings'});});scroll.append(tb);
        var sb=$('<div class="kk-srcbar"></div>');Object.keys(SOURCES).forEach(function(k){var s2=SOURCES[k],on=k===sc.key;var btn=$('<div class="kk-srcbtn selector '+(on?'kk-srcbtn--on':'kk-srcbtn--off')+'">'+E(s2.name)+'</div>');bE(btn,function(){if(on)return;ss({source:k});comp.create();});sb.append(btn);});
        var tb2=$('<div class="kk-srcbtn selector kk-srcbtn--off" style="background:rgba(1,180,228,.15);border-color:rgba(1,180,228,.4);color:#01b4e4">TMDB</div>');bE(tb2,function(){Lampa.Activity.push({url:'',title:'TMDB',component:'kkphim_tmdb_main',page:1});});sb.append(tb2);scroll.append(sb);
        var ld2=0;cats.forEach(function(cat){net.silent(sApi()+cat.api+'?page=1',function(res){var list=((res&&res.items)||(res&&res.data&&res.data.items)||[]).map(nm).filter(function(i){return i&&i.slug;});if(list.length){var row=$('<div class="kk-row"></div>'),mr=$('<div class="kk-row-more selector">Xem thêm</div>'),rl=$('<div class="kk-row-list"></div>');bE(mr,function(){Lampa.Activity.push({url:'',title:cat.name,component:'kkphim_category',cat:cat,page_num:1,mode:'api'});});list.slice(0,12).forEach(function(i){rl.append(mkC(i));});row.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">'+E(cat.name)+'</div>').append(mr)).append(rl);scroll.append(row);}ld2++;if(ld2>=cats.length){comp.activity.loader(false);comp.start();}},function(){ld2++;if(ld2>=cats.length){comp.activity.loader(false);comp.start();}});});};
    ...
    Lampa.Component.add('kkphim_tmdb_main',function(){
        var scroll=new Lampa.Scroll({mask:true,over:true}),comp=this;
        var secs=[{name:'🔥 Xu hướng',lt:'trending_day'},{name:'🌟 Tuần',lt:'trending'},{name:'🎬 Chiếu rạp',lt:'now_playing'},{name:'📅 Sắp chiếu',lt:'upcoming'},{name:'🌟 Phim lẻ',lt:'popular_movies'},{name:'📺 Phim bộ',lt:'popular_tv'},{name:'📺 Phát sóng',lt:'on_the_air'},{name:'⭐ Top lẻ',lt:'top_movies'},{name:'⭐ Top bộ',lt:'top_tv'}];
        this.create=function(){comp.activity.loader(true);cScr(scroll);
        var tb=$('<div class="kk-topbar"><div class="kk-topbar-title" style="color:#01b4e4">TMDB</div><div class="kk-topbar-btns"><div class="kk-btn selector">🔍</div><div class="kk-btn selector">⚙️</div></div></div>');bE($(tb.find('.kk-btn')[0]),oTSearch);bE($(tb.find('.kk-btn')[1]),function(){Lampa.Activity.push({url:'',title:'Cài đặt',component:'kkphim_settings'});});scroll.append(tb);
        var sb=$('<div class="kk-srcbar"></div>');Object.keys(SOURCES).forEach(function(k){var s2=SOURCES[k];var btn=$('<div class="kk-srcbtn selector kk-srcbtn--off">'+E(s2.name)+'</div>');bE(btn,function(){ss({source:k});Lampa.Activity.push({url:'',title:'KKPhim',component:'kkphim_main',page:1});});sb.append(btn);});sb.append('<div class="kk-srcbtn kk-srcbtn--on" style="background:rgba(1,180,228,.25);border-color:rgba(1,180,228,.5);color:#01b4e4">TMDB</div>');scroll.append(sb);
        var rr=$('<div class="kk-row"></div>'),rm=$('<div class="kk-row-more selector">Thêm</div>'),rl=$('<div class="kk-row-list"></div>');bE(rm,function(){Lampa.Activity.push({url:'',title:'🎲 Ngẫu nhiên',component:'kkphim_tmdb_list',listType:'trending',page_num:Math.floor(Math.random()*5)+2});});rr.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">🎲 Ngẫu nhiên</div>').append(rm)).append(rl);scroll.append(rr);
        Promise.all([tRand('movie'),tRand('tv')]).then(function(res){var items=[];(res[0].results||[]).forEach(function(i){i.media_type='movie';items.push(i);});(res[1].results||[]).forEach(function(i){i.media_type='tv';items.push(i);});for(var si2=items.length-1;si2>0;si2--){var sj=Math.floor(Math.random()*(si2+1));var st=items[si2];items[si2]=items[sj];items[sj]=st;}items.slice(0,12).forEach(function(i){rl.append(mkTC(i));});}).catch(function(){});
        var ld2=0;secs.forEach(function(sec){var fn=TFN[sec.lt];if(!fn){ld2++;return;}fn(1).then(function(res){var items=(res.results||[]).filter(function(i){return i.media_type!=='person';});if(items.length){var row=$('<div class="kk-row"></div>'),mr=$('<div class="kk-row-more selector">Thêm</div>'),rl2=$('<div class="kk-row-list"></div>');bE(mr,function(){Lampa.Activity.push({url:'',title:sec.name,component:'kkphim_tmdb_list',listType:sec.lt,page_num:1});});items.slice(0,12).forEach(function(i){rl2.append(mkTC(i));});row.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">'+E(sec.name)+'</div>').append(mr)).append(rl2);scroll.append(row);}ld2++;if(ld2>=secs.length){comp.activity.loader(false);comp.start();}}).catch(function(){ld2++;if(ld2>=secs.length){comp.activity.loader(false);comp.start();}});});};
        this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){scroll.destroy();};
    });
    ...
    Lampa.Component.add('kkphim_tmdb_genre',function(obj){
        ...
        this.create=function(){comp.activity.loader(true);cScr(scroll);grid.css('grid-template-columns','repeat('+cardSt()+',minmax(0,1fr))');
        var gb=$('<div class="kk-genre-bar"></div>');scroll.append(gb);scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title" id="kk-gtitle">'+E(obj.title||'')+'</div>').append(grid));
        var sb=scroll.render().find('.scroll__body');sb.on('scroll',function(){if(ld||(md&&td))return;var el=sb[0];if(el.scrollTop+el.clientHeight>=el.scrollHeight-400)doL();});
        Promise.all([lGenres('movie'),lGenres('tv')]).then(function(res){var mg2=[],sn={};(res[0]||[]).concat(res[1]||[]).forEach(function(g){if(!sn[g.id]){sn[g.id]=true;mg2.push(g);}});mg2.sort(function(a,b){return(a.name||'').localeCompare(b.name||'');});mg2.forEach(function(g){var on=String(g.id)===cg;var ch=$('<div class="kk-genre-chip selector '+(on?'kk-genre-chip--on':'kk-genre-chip--off')+'">'+E(g.name)+'</div>');bE(ch,function(){Lampa.Activity.push({url:'',title:g.name,component:'kkphim_tmdb_genre',genre_id:g.id,page_num:1});});gb.append(ch);});var cur=mg2.find(function(g){return String(g.id)===cg;});if(cur)scroll.render().find('#kk-gtitle').text(cur.name);doL();}).catch(function(){doL();});};
        function doL(){if(ld)return;ld=true;var ps=[];if(!md)ps.push(tDiscover('movie',cg,mp).then(function(r){var items=r.results||[];if(!items.length)md=true;else{items.forEach(function(i){i.media_type='movie';});all=all.concat(items);mp++;}}).catch(function(){md=true;}));if(!td)ps.push(tDiscover('tv',cg,tp).then(function(r){var items=r.results||[];if(!items.length)td=true;else{items.forEach(function(i){i.media_type='tv';});all=all.concat(items);tp++;}}).catch(function(){td=true;}));Promise.all(ps).then(function(){all.sort(function(a,b){return(b.popularity||0)-(a.popularity||0);});for(var i=0;i<all.length;i++){var key=all[i].media_type+'_'+all[i].id;if(!rs[key]){rs[key]=true;grid.append(mkTC(all[i]).addClass('kk-card--grid'));}}ld=false;comp.activity.loader(false);comp.start();}).catch(function(){ld=false;comp.activity.loader(false);});}
        this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){scroll.destroy();};
    });
    ...
    Lampa.Component.add('kkphim_tmdb_detail',function(obj){
        ...
        this.create=function(){comp.activity.loader(true);cScr(scroll);if(!tid){comp.activity.loader(false);comp.start();return;}
        ...
        async function bDet(tmdb,logos,slugs){
            cScr(scroll);
            var bk=tmdb.backdrop_path?TMDB_IMG+tmdb.backdrop_path:'',ps=tmdb.poster_path?TMDB_W500+tmdb.poster_path:'';
            ...
            var extra='';if(tmdb.number_of_episodes)extra=tmdb.number_of_episodes+' tập';if(tmdb.number_of_seasons)extra+=(extra?' | ':'')+tmdb.number_of_seasons+' season';
            var logo=pLogo(logos||(tmdb.images||{})),logoH='';if(logo&&logo.file_path)logoH='<div class="kk-logo"><img src="'+TMDB_W500+logo.file_path+'"></div>';
            var gh=gHtml(tmdb.genres,true);
            var castH='',dirH='',crewH='',dir='';
            if(tmdb.credits){castH=mkPpl((tmdb.credits.cast||[]).slice(0,12),'character',true);var dirs=(tmdb.credits.crew||[]).filter(function(c){return c.job==='Director'||c.job==='Creator'||c.job==='Series Director';}).filter(function(p,i,a){return a.findIndex(function(x){return x.name===p.name;})===i;}).slice(0,10);if(dirs.length){dir=dirs.map(function(c){return c.name;}).join(', ');dirH=mkPpl(dirs.map(function(c){return{id:c.id,name:c.name,profile_path:c.profile_path,job:c.job||'Đạo diễn'};}),'job',true);}}
            if(dir)crewH='<div class="kk-crew"><b>Đạo diễn</b><span>'+E(dir)+'</span></div>';
            var imdb=(tmdb.external_ids&&tmdb.external_ids.imdb_id)||null;
            var tH=logoH?'':'<div class="kk-title">'+E(t)+'</div>';
            var hero=mkHero(bk,ps,logoH,tH,o);
            var body=mkBody(v,y,rt,extra,gh,crewH,d);
            ...
            if(slugs.kkphim){if(mt==='movie')act.append(bMovExp('kkphim','KKPhim',slugs.kkphim,t,'kk-src-btn--kkphim'));else act.append(bTVExp('kkphim','KKPhim',slugs.kkphim,t,o,'kk-src-btn--kkphim'));}else{act.append('<div class="kk-src-btn kk-src-btn--no">KKPhim – N/A</div>');}
            if(slugs.ophim){if(mt==='movie')act.append(bMovExp('ophim','OPhim',slugs.ophim,t,'kk-src-btn--ophim'));else act.append(bTVExp('ophim','OPhim',slugs.ophim,t,o,'kk-src-btn--ophim'));}else{act.append('<div class="kk-src-btn kk-src-btn--no">OPhim – N/A</div>');}
            act.append(bTorBtn(mt,tid,t,ps,imdb));body.append(act);
            var dw=$('<div class="kk-detail-wrap"></div>').append(hero).append(body);
            if(dirH){var ds=$('<div class="kk-section"><div class="kk-block-title">Đạo diễn</div><div class="kk-cast-list">'+dirH+'</div></div>');bCast(ds);dw.append(ds);}
            if(castH){var cs=$('<div class="kk-section"><div class="kk-block-title">Diễn viên</div><div class="kk-cast-list">'+castH+'</div></div>');bCast(cs);dw.append(cs);}
            ...
            try{var rd=await tRec(mt,tid,1);var ri=(rd.results||[]).slice(0,20);if(ri.length){var rs2=$('<div class="kk-section kk-similar kk-section--last"></div>').append('<div class="kk-block-title">🎲 Gợi ý</div>');var rl2=$('<div class="kk-similar-list"></div>');ri.forEach(function(i){if(!i.media_type)i.media_type=mt;rl2.append(mkTC(i));});rs2.append(rl2);dw.append(rs2);}else{dw.find('.kk-similar').last().addClass('kk-section--last');}}catch(e){dw.find('.kk-similar').last().addClass('kk-section--last');}
    ...
    Lampa.Component.add('kkphim_search',function(obj){
        var net=new Lampa.Reguest(),scroll=new Lampa.Scroll({mask:true,over:true}),comp=this;
        var kw=obj.keyword||'',page=obj.page_num||1;var grid=$('<div class="kk-grid"></div>'),lm=$('<div class="kk-loadmore selector">Tải thêm</div>'),ld=false,hm=true;
    ...
    Lampa.Component.add('kkphim_detail',function(obj){
        ...
        function bld(data,eps,tmdb,logos,tt){
            ...
            var castH='',dirH='',crewH='',logoH='',dir='';
            if(tmdb){if(tmdb.backdrop_path)bk=TMDB_IMG+tmdb.backdrop_path;if(tmdb.poster_path)ps=TMDB_IMG+tmdb.poster_path;if(tmdb.title||tmdb.name)t=tmdb.title||tmdb.name;if(tmdb.original_title||tmdb.original_name)o2=tmdb.original_title||tmdb.original_name;if(tmdb.overview)d=tmdb.overview;if(tmdb.vote_average)v=Number(tmdb.vote_average).toFixed(1);if(tmdb.release_date)y=tmdb.release_date.slice(0,4);if(tmdb.runtime)rt=tmdb.runtime+' phút';var logo=pLogo(logos||tmdb.images);if(logo&&logo.file_path)logoH='<div class="kk-logo"><img src="'+TMDB_W500+logo.file_path+'"></div>';if(tmdb.credits){castH=mkPpl((tmdb.credits.cast||[]).slice(0,12),'character',true);var dirs=(tmdb.credits.crew||[]).filter(function(c){return c.job==='Director'||c.job==='Creator';}).filter(function(p,i,a){return a.findIndex(function(x){return x.name===p.name;})===i;}).slice(0,10);if(dirs.length){dir=dirs.map(function(c){return c.name;}).join(', ');dirH=mkPpl(dirs.map(function(c){return{id:c.id,name:c.name,profile_path:c.profile_path,job:c.job||'Đạo diễn'};}),'job',true);}}}
            ...
            if(dir&&!dirH)crewH='<div class="kk-crew"><b>Đạo diễn</b><span>'+E(dir)+'</span></div>';
            var hasTmdb=!!gTid(data);var tH=logoH?'':'<div class="kk-title">'+E(t)+'</div>';
            var hero=mkHero(bk,ps,logoH,tH,o2);
            ...
            var act=$('<div class="kk-actions"></div>');var cSrc=src(),sCss=cSrc.key==='kkphim'?'kk-src-btn--kkphim':'kk-src-btn--ophim';
            if(eps&&eps.length)act.append(bDetExp(eps,data.name||t,cSrc.name,sCss));else act.append('<div class="kk-src-btn kk-src-btn--no">⚠️ Không có tập</div>');
            ...
            if(pCats.length&&pCats[0]&&pCats[0].slug){var ss2=$('<div class="kk-section kk-similar"></div>').append('<div class="kk-block-title">Phim liên quan</div>');var sl=$('<div class="kk-similar-list"><div class="kk-ep-ld">⏳</div></div>');ss2.append(sl);dw.append(ss2);fetch(sApi()+'v1/api/the-loai/'+pCats[0].slug+'?page=1').then(function(r){return r.json();}).then(function(res){var items=((res&&res.data&&res.data.items)||(res&&res.items)||[]).map(nm).filter(function(i){return i&&i.slug&&i.slug!==data.slug;});sl.empty();if(items.length)items.slice(0,20).forEach(function(i){sl.append(mkC(i));});else sl.html('<div class="kk-ep-ld">Không có</div>');}).catch(function(){sl.empty();});}
            if(hasTmdb){var tid2=gTid(data);var ts2=$('<div class="kk-section kk-similar kk-section--last"></div>').append('<div class="kk-block-title">🎬 TMDB liên quan</div>');var tl=$('<div class="kk-similar-list"><div class="kk-ep-ld">⏳</div></div>');ts2.append(tl);dw.append(ts2);tSimilar(tt,tid2,1).then(function(res){var items=(res.results||[]).slice(0,20);tl.empty();if(items.length)items.forEach(function(i){if(!i.media_type)i.media_type=tt;tl.append(mkTC(i));});else tl.html('<div class="kk-ep-ld">Không có</div>');}).catch(function(){tl.empty();});}
            scroll.append(dw);
        }
        this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){net.clear();scroll.destroy();};
    });
