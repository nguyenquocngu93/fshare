(function () {
    ...
    /* ============================================================
       TORRENT SOURCES — chỉ dùng API đã kiểm chứng hoạt động
    ============================================================ */

    /* Chuẩn hóa kết quả, lọc hash không hợp lệ */
    ...
    /* ── Knaben ─────────────────────────────────────────────────
       GET https://knaben.eu/api/v1?search=...&orderBy=seeders...
       Trả về { hits: [...] }  với field: title, infohash, seeders,
       leechers, bytes
    ──────────────────────────────────────────────────────────── */
    ...
    /* ── Apibay (The Pirate Bay) ─────────────────────────────────
       ...
       Trả về JSON array với: name, info_hash, seeders, leechers, size
    ──────────────────────────────────────────────────────────── */
    ...
    /* ── TorrentsCSV ─────────────────────────────────────────────
       GET https://torrents-csv.com/service/search?q=...&size=50
       Trả về { torrents: [...] }
    ──────────────────────────────────────────────────────────── */
    function fetchTorrentsCSV(query, cb) {
        ...
        reguest(url, function (data) {
            ...
            console.warn('[TorrentsCSV]', e);
            cb([]);
        });
    }

    /* ── Jackett — riêng, không trộn vào Torrent+ ───────────── */
    ...
    /* ============================================================
       MULTI-SOURCE SEARCH (Torrent+)
       Knaben + Apibay + TorrentsCSV + YTS(phim lẻ)
    ============================================================ */
    ...
    /* Hiển thị kết quả torrent — gom nhóm theo chất lượng */
    ...
    function searchTorrentMulti(card, season, episode) {
        ...
        searchAllSources(query, isMovie, function (results) {
            if (!results.length) {
                /* Thử query phụ (dùng title thay orig) */
                var q2 = title;
                if (type === 'series' && season && episode)
    ...
    /* ── Jackett entry point ───────────────────────────────────── */
    function searchJackett(card, season, episode) {
        var title = card.title || card.name || '';
        ...
        var url = getJackettUrl(), key = getJackettKey();
        if (!url || !key) {
            Lampa.Noty.show('Vào Settings → cấu hình Jackett!');
            return;
        }
    ...
    /* ============================================================
       HOOK UI — 5 nút: KKPhim | OPhim | Torrentio | Torrent+ | Jackett
    ============================================================ */
    Lampa.Listener.follow('full', function (e) {
        ...
        var $jk = mkBtn('view--kkparser-jackett',
            ...
            'Jackett',
            function () {
                if (isSeries) askEpisode(card, function (s, ep) { searchJackett(card, s, ep); });
                else searchJackett(card, null, null);
            }
        );
    ...
    function start() {
        initSettings();
        console.log('[KKPhim Parser] v3.0 — Torrent+: Knaben|TPB|TorrCSV|YTS ✅');
    }
