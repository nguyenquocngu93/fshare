/* KKPhim Plugin v4.5 - Ultimate 120Hz + Clean UI */
(function(){
'use strict';
...
function rafBatch(fn){
    // ✅ Mobile: execute immediately to reduce latency
    if(isMobile()){
        if(window.requestAnimationFrame){
            requestAnimationFrame(function(){try{fn();}catch(e){}});
        }else{
            setTimeout(function(){try{fn();}catch(e){}},16);
        }
        return;
    }
    _rafQueue.push(fn);
...
function bE(el,fn){
    ...
    if(dom&&dom.addEventListener){
        ...
        dom.addEventListener('touchend',function(e){if(mv)return;tc=true;if(e.cancelable)e.preventDefault();e.stopPropagation();fn.call(dom,e);setTimeout(function(){tc=false;},300);},{passive:true});
    }
...
function eScr(scroll){
    ...
    // ✅ Disable smooth scrolling on mobile - causes jank at 120Hz
    if(isMobile()){
...
// ═══ Play functions - clean, with preload ═══
...
// ═══ CSS with 120hz GPU acceleration - ULTIMATE OPTIMIZATION ═══
function inCSS(){
    ...
    $('head').append('<style id="kk-extra-css">'+
        /* Mobile performance base */
        '*{-webkit-tap-highlight-color:transparent;-webkit-touch-callout:none;}'+
        'html,body{overscroll-behavior-y:contain;}'+
        
        /* Remove ALL borders/lines - CLEAN UI */
        ...
        /* GPU Layer Promotion + Containment */
        ...
        /* Scroll optimization - DISABLE smooth on mobile */
        ...
        /* NO transitions on mobile for max 120Hz feel */
        '.kk-card:hover,.kk-card-h:hover,.kk-pfc:hover,.selector.focus{transition:none!important;}'+
        '.kk-card.focus,.kk-card-h.focus,.kk-pfc.focus{transform:translate3d(0,0,0) scale(1.02)!important;-webkit-transform:translate3d(0,0,0) scale(1.02)!important;}'+
        
        /* CW progress optimized */
        ...
        /* SIMILAR MOVIES - POSTER ONLY, NO INFO BELOW */
        '.kk-similar-list{display:flex;gap:12px;overflow-x:auto;padding:8px 0;}'+
        '.kk-similar-list .kk-card{min-width:140px;max-width:140px;margin:0!important;padding:0!important;background:transparent;}'+
        '.kk-similar-list .kk-card-body{display:none!important;}'+
        '.kk-similar-list .kk-card-img{border-radius:8px;overflow:hidden;}'+
        '.kk-similar-list .kk-card-q{position:absolute;top:6px;left:6px;z-index:2;}'+
        '.kk-similar-list .kk-card-ep--type{position:absolute;bottom:6px;right:6px;z-index:2;}'+
    '</style>');
}
...
function startPlugin(){
    ...
    // ✅ Force GPU compositing layer on mobile
    if(isMobile()){
    ...
    Lampa.Component.add('kkphim_settings',function(){
        ...
        w.append('<div class="kk-stg-ver">KKPhim v4.5</div>');scroll.append(w);comp.start();};
    ...
    // TMDB similar
    if(hasTmdb){var ts2=$('<div class="kk-section kk-similar"></div>').append('<div class="kk-block-title">🎬 TMDB gợi ý</div>');var tl=$('<div class="kk-similar-list"><div class="kk-ep-ld">⏳</div></div>');ts2.append(tl);dw.append(ts2);setTimeout(function(){tSimilar(tt,gTid(data),1).then(function(res){var items=(res.results||[]).slice(0,20);tl.empty();if(items.length){var cards=[];items.forEach(function(i){if(!i.media_type)i.media_type=tt;cards.push(mkTC(i));});batchAppend(tl,cards);}else tl.html('Không có');}).catch(function(){tl.empty();});},500);
    // Random row
