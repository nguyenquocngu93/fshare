...
// ══════════════════════════════════════════════════════════════
// SOURCES - Inline (hoặc có thể import từ sources.js)
// ══════════════════════════════════════════════════════════════
var SOURCES={
...
function srcKey(){var s=ls(),k=s.source||'ophim';if(!isSourceEnabled(k)){var en=Object.keys(SOURCES).find(function(x){return isSourceEnabled(x);});return en||'ophim';}return k;}
...
// History (không còn Continue Watching)
...
async function fSlugs(title,orig,year){
    var r={},enabledSources=getEnabledSources(),terms=[title];
    if(orig&&orig!==title)terms.push(orig);
    for(var i=0;i<terms.length;i++){
        for(var key in enabledSources){
            if(!r[key]){
                var f=mBest(await sSrc(enabledSources[key],terms[i]),title,orig,year);
                if(f&&f.slug)r[key]=f.slug;
            }
        }
        var allFound=true;
        for(var k in enabledSources){if(!r[k])allFound=false;}
        if(allFound)break;
    }
    return r;
}
...
function mkC(item){
    ...
    // Xóa origin_name nếu trùng với name
    ...
    // Xóa time (line thừa)
    // Xóa episode_total (line thừa)
    h+='</div>';
    // Xóa category (line thừa)
    h+='</div></div>';
    var c=$(h);
...
function mkCH(item){
    ...
    h+='<div class="kk-card-meta">';
    if(n.year)h+='<span class="kk-card-year">'+E(n.year)+'</span>';
    // Xóa time, episode_total, category (line thừa)
    h+='</div></div></div>';
    var c=$(h);
    bE(c,function(){if(n.slug){saveHistory(n);Lampa.Activity.push({url:'',title:n.name||'',component:'kkphim_detail',movie:n,page:1});}});
    return c;
...
function mkTCPF(item){
    ...
    var origName=item.original_title||item.original_name||'',typeLabel=d.media_type==='tv'?'TV Series':'Film';
    var card=$('<div class="kk-pfc selector"></div>');
    card.html('<div class="kk-pfc-bg">'+(bk?'<img src="'+bk+'" loading="lazy">':'<div class="kk-pfc-bg-empty"></div>')+'</div><div class="kk-pfc-overlay"></div><div class="kk-pfc-inner">'+(ps?'<div class="kk-pfc-poster"><img src="'+ps+'" loading="lazy"></div>':'<div class="kk-pfc-poster kk-pfc-poster--empty"></div>')+'<div class="kk-pfc-info"><div class="kk-pfc-badge">'+E(typeLabel)+'</div><div class="kk-pfc-title">'+E(d.name)+'</div>'+(origName&&origName!==d.name?'<div class="kk-pfc-origin">'+E(origName)+'</div>':'')+'<div class="kk-pfc-meta">'+(d.year?'<span class="kk-pfc-year">'+E(d.year)+'</span>':'')+(vote?'<span class="kk-pfc-vote kk-pfc-vote--'+vClass+'">⭐ '+vote+'</span>':'')+'</div></div></div>');
...
function mkCatGrid(items,isKK){
    ...
    if(cm==='hgrid'){
        ...
        // SỬA: Đổi từ 3 thành 2 cột cho "Ngang x2"
...
// ══════════════════════════════════════════════════════════════
// SETTINGS COMPONENT - Thêm bật/tắt sources
// ══════════════════════════════════════════════════════════════
Lampa.Component.add('kkphim_settings',function(){
    ...
    this.create=function(){
        ...
        // Nguồn phim mặc định (chỉ hiện những nguồn đã bật)
        var g0=mg('🎬 Nguồn phim mặc định');
        var cur=s.source||'ophim';
        var enabledSources=getEnabledSources();
        if(Object.keys(enabledSources).length===0){
            g0.append('<div class="kk-stg-item"><div class="kk-stg-label"><div class="kk-stg-label-name">⚠️ Chưa bật nguồn nào</div></div></div>');
        }else{
            Object.keys(enabledSources).forEach(function(k){
                var sc=enabledSources[k];
                g0.append(mo(sc.name,'Chọn làm nguồn mặc định',cur===k,function(){
                    ss({source:k});
                    Lampa.Noty.show('Đã chọn: '+sc.name);
                    comp.create();
                }));
            });
        }
        ...
        // SỬA: Đổi label từ "Ngang x2" thành đúng thực tế
        var gCat=mg('📋 Danh sách/Tìm kiếm - Kiểu card');var ctm=s.cat_mode||'hgrid';
        [{k:'hgrid',n:'📐 Ngang (2 cột)',d:'2 cột, backdrop ngang'},{k:'vgrid',n:'🖼️ Dọc (tùy chỉnh)',d:'Số cột theo cài đặt bên dưới'}].forEach(function(o){gCat.append(mo(o.n,o.d,ctm===o.k,function(){ss({cat_mode:o.k});comp.create();}));});
        w.append(gCat);
        ...
        w.append('<div class="kk-stg-ver">KKPhim v3.2</div>');
...
Lampa.Component.add('kkphim_person_films',function(obj){
    ...
    function rMore(){
        ...
        b.forEach(function(c){
            var card=cm==='hgrid'?mkTCH(c):mkTC(c);
            curGrid.append(card.addClass('kk-card--grid'));
        });
        rd+=b.length;
        spinner.hide();
...
// ══════════════════════════════════════════════════════════════
// MAIN COMPONENT - Đã xóa Continue Watching, thêm "Vì bạn đã xem"
// ══════════════════════════════════════════════════════════════
Lampa.Component.add('kkphim_main',function(){
    ...
    this.create=function(){
        ...
        var enabledSources=getEnabledSources();
        Object.keys(enabledSources).forEach(function(k){
            var s2=enabledSources[k],on=k===sc.key;
        ...
        // Vì bạn đã xem (thay thế Continue Watching)
        var hist=lastHistory();
        if(hist&&hist.category&&hist.category.length&&hist.category[0]&&hist.category[0].slug){
            fetch(sApi()+'v1/api/the-loai/'+hist.category[0].slug+'?page=1').then(function(r){return r.json();}).then(function(res){
                ...
                if(items.length){
                    ...
                    bE(moreRec,function(){Lampa.Activity.push({url:'',title:'Gợi ý từ '+(hist.name||''),component:'kkphim_category',mode:'category',category_slug:hist.category[0].slug,page_num:1});});
                    rowRec.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">💡 Vì bạn đã xem "'+E(hist.name||'')+'"</div>').append(moreRec));
                    rowRec.append(mkRowList(items.slice(0,rowCount()),true));
                    // Chèn sau source bar
                    scroll.render().find('.kk-srcbar').after(rowRec);
                }
            }).catch(function(){});
        }
...
Lampa.Component.add('kkphim_tmdb_main',function(){
    ...
    this.create=function(){
        ...
        var sb=$('<div class="kk-srcbar"></div>');
        var enabledSources=getEnabledSources();
        Object.keys(enabledSources).forEach(function(k){var s2=enabledSources[k];var btn=$('<div class="kk-srcbtn selector kk-srcbtn--off">'+E(s2.name)+'</div>');bE(btn,function(){ss({source:k});Lampa.Activity.push({url:'',title:'KKPhim',component:'kkphim_main',page:1});});sb.append(btn);});
        ...
        // THÊM MỚI: "Vì bạn đã xem" cho TMDB
        var hist=lastHistory();
        if(hist&&hist.tmdb&&hist.tmdb.id){
            var tmdbId=hist.tmdb.id;
            var tmdbType=hist.tmdb.type||'movie';
            tRec(tmdbType,tmdbId,1).then(function(res){
                var items=(res.results||[]).slice(0,cnt);
                if(items.length){
                    items.forEach(function(i){if(!i.media_type)i.media_type=tmdbType;});
                    var rowRec=$('<div class="kk-row"></div>');
                    var moreRec=$('<div class="kk-row-more selector">Xem thêm</div>');
                    bE(moreRec,function(){Lampa.Activity.push({url:'',title:'Gợi ý từ '+(hist.name||''),component:'kkphim_tmdb_list',listType:'trending',page_num:1});});
                    rowRec.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">💡 Vì bạn đã xem "'+E(hist.name||'')+'"</div>').append(moreRec));
                    rowRec.append(mkRowList(items,false));
                    scroll.render().find('.kk-srcbar').after(rowRec);
                }
            }).catch(function(){});
        }
...
Lampa.Component.add('kkphim_tmdb_genre',function(obj){
    ...
    var gridContainer=$('<div></div>'),ld=false,md=false,td=false,mp=1,tp=1,all=[],rs={};
    var curGrid=null;
    var spinner=$('<div class="kk-row-loading" style="display:none"><div class="kk-row-spinner"></div></div>');
    var endMsg=$('<div class="kk-loadmore" style="display:none;cursor:default;background:transparent">Đã hết</div>');
    function initGrid(){
    ...
    function doL(){if(ld)return;ld=true;spinner.show();var ps=[];
        ...
        Promise.all(ps).then(function(){all.sort(function(a,b){return(b.popularity||0)-(a.popularity||0);});
            var cm=catMode();
            for(var i=0;i<all.length;i++){var key=all[i].media_type+'_'+all[i].id;if(!rs[key]){rs[key]=true;
                var card=cm==='hgrid'?mkTCH(all[i]):mkTC(all[i]);
                curGrid.append(card.addClass('kk-card--grid'));
            }}
            spinner.hide();
            if(md&&td)endMsg.show();
            ld=false;comp.activity.loader(false);comp.start();
        }).catch(function(){ld=false;spinner.hide();comp.activity.loader(false);});
    }
    this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){scroll.destroy();};
});
...
Lampa.Component.add('kkphim_tmdb_detail',function(obj){
    ...
    async function bDet(tmdb,logos,slugs){
        ...
        // Chỉ hiện sources đã bật
        var enabledSources=getEnabledSources();
        if(enabledSources.kkphim&&slugs.kkphim){
            if(mt==='movie')act.append(bMovExp('kkphim','KKPhim',slugs.kkphim,t,'kk-src-btn--kkphim',{name:t,origin_name:o2,tmdb:{id:tid,type:mt},category:tmdb.genres||[]}));
            else act.append(bTVExp('kkphim','KKPhim',slugs.kkphim,t,o2,'kk-src-btn--kkphim',{name:t,origin_name:o2,tmdb:{id:tid,type:mt},category:tmdb.genres||[]}));
        }else if(enabledSources.kkphim){
            act.append('<div class="kk-src-btn kk-src-btn--no">KKPhim – N/A</div>');
        }
        
        if(enabledSources.ophim&&slugs.ophim){
            if(mt==='movie')act.append(bMovExp('ophim','OPhim',slugs.ophim,t,'kk-src-btn--ophim',{name:t,origin_name:o2,tmdb:{id:tid,type:mt},category:tmdb.genres||[]}));
            else act.append(bTVExp('ophim','OPhim',slugs.ophim,t,o2,'kk-src-btn--ophim',{name:t,origin_name:o2,tmdb:{id:tid,type:mt},category:tmdb.genres||[]}));
        }else if(enabledSources.ophim){
            act.append('<div class="kk-src-btn kk-src-btn--no">OPhim – N/A</div>');
        }
...
Lampa.Component.add('kkphim_category',function(obj){
    ...
    var gridContainer=$('<div></div>'),ld=false,done=false;
    var curGrid=null;
    var spinner=$('<div class="kk-row-loading" style="display:none"><div class="kk-row-spinner"></div></div>');
    var endMsg=$('<div class="kk-loadmore" style="display:none;cursor:default;background:transparent">Đã hết</div>');
    function initGrid(){
    ...
    this.create=function(){
        this.activity.loader(true);cScr(scroll);initGrid();
        scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">'+E(title)+'</div>').append(gridContainer).append(spinner).append(endMsg));
        var sb=scroll.render().find('.scroll__body');
    ...
    function hr(res){
        ...
        list.forEach(function(i){
            var card=cm==='hgrid'?mkCH(i):mkC(i);
            curGrid.append(card.addClass('kk-card--grid'));
        });
    ...
    function doL(){
        ...
        net.silent(url,hr,function(){ld=false;spinner.hide();endMsg.show().text('Lỗi');comp.activity.loader(false);});
    }
...
Lampa.Component.add('kkphim_search',function(obj){
    ...
    this.create=function(){
        this.activity.loader(true);cScr(scroll);initGrid();
        scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title">🔍 '+E(kw)+'</div>').append(gridContainer).append(spinner).append(endMsg));
        var sb=scroll.render().find('.scroll__body');
        sb.on('scroll',function(){if(ld||done)return;var el=sb[0];if(el.scrollTop+el.clientHeight>=el.scrollHeight-400)doL();});
        doL();
    };
    function hr(res){
        var list=((res&&res.items)||(res&&res.data&&res.data.items)||[]).map(nm).filter(function(i){return i&&i.slug;});
        spinner.hide();
        if(!list.length){done=true;endMsg.show().text(page===1?'Không có kết quả':'Đã hết');comp.activity.loader(false);ld=false;comp.start();return;}
        var cm=catMode();
        list.forEach(function(i){
            var card=cm==='hgrid'?mkCH(i):mkC(i);
            curGrid.append(card.addClass('kk-card--grid'));
    ...
    function doL(){
        ...
        net.silent(sApi()+'v1/api/tim-kiem?keyword='+encodeURIComponent(kw)+'&page='+page,hr,function(){
            net.silent(sApi()+'tim-kiem?keyword='+encodeURIComponent(kw)+'&page='+page,hr,function(){
                ld=false;spinner.hide();endMsg.show().text('Lỗi');comp.activity.loader(false);
            });
        });
    }
    this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){net.clear();scroll.destroy();};
...
Lampa.Component.add('kkphim_detail',function(obj){
    ...
    function bld(data,eps,tmdb,logos,tt){
        ...
        if(eps&&eps.length)act.append(bDetExp(eps,data.name||t,cSrc.name,sCss,data));else act.append('<div class="kk-src-btn kk-src-btn--no">⚠️ Không có tập</div>');
