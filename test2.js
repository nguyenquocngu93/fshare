 ...
 *
 * Version: 2.2.0 - Fix full() với Status, thêm seasons/parser
 */
(function () {
  'use strict';
  ...
  function full(params, oncomplite, onerror) {
    // Trang chi tiết phim - Lampa yêu cầu dùng Lampa.Status
    ...
    // Lampa dùng Lampa.Status(3) để gom 3 phần: movie, persons, simular
    var status = new Lampa.Status(3);
    status.onComplite = oncomplite;

    // 1. Lấy thông tin phim + episodes
    detailAPI(srcKey, slug).then(function (det) {
      ...
      var item = {
        ...
        // Quan trọng cho Lampa
        episodes: lampaEps,
        // Parser cho từng episode (Lampa gọi khi user bấm Play)
        episode_parser: function (ep, onpick) {
          // Trả về các file/quality cho user chọn
      ...
      status.append('movie', item);
    }, function () { status.error(); });

    // 2. Persons rỗng (KKPhim API không có)
    status.append('persons', { cast: [], crew: [] });

    // 3. Simular rỗng
    status.append('simular', { results: [] });
  }
  ...
  function addPlugin() {
    ...
    console.log('[KKPhim Parser] v2.2.0 OK — Full + episode parser ready');
  }
