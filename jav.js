/* KKPhim Plugin v4.1 - TMDB + Subtitle */
(function(){
'use strict';
...
function saveHistory(item){
    try{
        ...
        arr.unshift({
            id:id,tmdb_id:item.tmdb_id,media_type:item.media_type,
            name:item.name||item.title||'',poster_url:item.poster_url||'',
            year:item.year||'',time:Date.now()
        });
...
function mBest(items,title,orig,year){
    ...
    var scored=items.map(function(it){return{item:it,score:mScore(it,title,orig,year)};}).filter(function(x){return x.score>0;}).sort(function(a,b){return b.score-a.score;});
    return scored.length?scored[0].item:null;
}

async function fSlugs(title,orig,year){
    var r={},enabledSources=getEnabledSources();
    var terms=[title];
    if(orig&&orig!==title)terms.push(orig);
    if(year){terms.push(title+' '+year);if(orig&&orig!==title)terms.push(orig+' '+year);}
    for(var i=0;i<terms.length;i++){
        for(var key in enabledSources){if(!r[key]){var items=await sSrc(enabledSources[key],terms[i]);var f=mBest(items,title,orig,year);if(f&&f.slug)r[key]=f.slug;}}
        var allFound=true;for(var k in enabledSources){if(!r[k])allFound=false;}if(allFound)break;
    }
    return r;
}
...
function exSn(name,slug){
    ...
    if(nm2){var n=parseInt(nm2[1]);if(n>=2&&n<=30)return n;}
    return 1;
}
...
async function fSeasonSlugs(source,title,orig,targetSeason){
    var results=[];
    try{
        var searchTerms=[title];if(orig&&orig!==title)searchTerms.push(orig);
        var allItems=[];
        for(var t=0;t<searchTerms.length;t++){var items=await sSrc(source,searchTerms[t]);allItems=allItems.concat(items);}
        ...
        for(var i=0;i<allItems.length;i++){
            var it=allItems[i];if(!it.slug)continue;
        ...
        if(targetSeason){var exact=results.filter(function(r){return r.season===targetSeason;});if(exact.length)return exact;}
    }catch(e){}
    return results;
}
...
async function findTmdbId(item,type){
    ...
    try{
        var searchType=type||'multi',endpoint=searchType==='multi'?'/search/multi':'/search/'+searchType;
        ...
        for(var i=0;i<results.length;i++){
            var r=results[i];var rName=nStr(r.title||r.name||''),rOrig=nStr(r.original_title||r.original_name||'');
            var rYear=(r.release_date||r.first_air_date||'').slice(0,4);var nT=nStr(title),nO=nStr(orig);
            ...
            if(nameMatch&&yearMatch)return{id:r.id,type:r.media_type||(r.first_air_date?'tv':'movie')};
        }
...
// -- Subdl.com search (no API key needed) --
...
// -- OpenSubtitles.com search (needs API key) --
async function searchOpenSub(imdbId,title,season,episode){
    var apiKey=osubApiKey();if(!apiKey)return[];
    try{
        var params=[];
        ...
        var r=await fetch(url,{headers:{'Api-Key':apiKey,'Content-Type':'application/json','User-Agent':'KKPhimPlugin v4.1'}});
        ...
        return data.data.map(function(item){
            ...
            return{
                id:item.id||'',fileId:file.file_id||'',
                label:(attr.language||subLang()).toUpperCase()+' - '+(attr.release||'Subtitle')+(attr.hearing_impaired?' [CC]':'')+' ⬇'+(attr.download_count||0),
                language:attr.language||subLang(),
                format:file.file_name?(file.file_name.split('.').pop()||'srt'):'srt',
                source:'opensubtitles',
                downloads:attr.download_count||0,
                hi:attr.hearing_impaired||false,
                _needDownload:true
            };
        });
    }catch(e){return[];}
...
async function getOsubDownloadUrl(fileId){
    ...
    try{
        var r=await fetch(OSUB_API+'/download',{method:'POST',headers:{'Api-Key':apiKey,'Content-Type':'application/json','User-Agent':'KKPhimPlugin v4.1'},body:JSON.stringify({file_id:fileId})});
        if(!r.ok)return null;var data=await r.json();return data.link||null;
    }catch(e){return null;}
}
...
async function findSubtitles(imdbId,title,season,episode){
    ...
    if(src==='opensubtitles'){
        results=await searchOpenSub(imdbId,title,season,episode);
        if(!results.length)results=await searchSubdl(imdbId,title,season,episode);
    }else{
        results=await searchSubdl(imdbId,title,season,episode);
...
var SubOverlay={
    ...
    parseSRT:function(text){
        ...
        blocks.forEach(function(block){
            ...
            if(timeIdx<0)return;
            var tm=lines[timeIdx].match(/(\d{1,2}):(\d{2}):(\d{2})[,.](\d{3})\s*-->\s*(\d{1,2}):(\d{2}):(\d{2})[,.](\d{3})/);
            if(!tm)return;
    ...
    load:async function(url){
        try{
            var r=await fetch(url);if(!r.ok)return false;
            var text=await r.text();
            if(url.match(/\.vtt$/i)||text.trim().startsWith('WEBVTT'))this.cues=this.parseVTT(text);
            else this.cues=this.parseSRT(text);
            return this.cues.length>0;
        }catch(e){return false;}
    },
    createOverlay:function(){
        if(this.el)return;
        this.el=$('<div id="kk-sub-overlay"></div>');
        this.el.css({position:'fixed',bottom:'10%',left:'5%',right:'5%','text-align':'center','z-index':'999999','pointer-events':'none','font-size':'1.4em','line-height':'1.5','text-shadow':'2px 2px 4px rgba(0,0,0,0.9), -1px -1px 2px rgba(0,0,0,0.9)',color:'#ffffff','font-weight':'bold',display:'none'});
        $('body').append(this.el);
    },
    start:function(){this.createOverlay();this.active=true;this.sync();},
    sync:function(){
        if(!this.active)return;var self=this;
        var video=document.querySelector('video');
        if(video&&!video.paused){
            var ct=video.currentTime,found=false;
            for(var i=0;i<self.cues.length;i++){
                if(ct>=self.cues[i].start&&ct<=self.cues[i].end){self.el.html(self.cues[i].text.replace(/\n/g,'<br>')).show();found=true;break;}
            }
            if(!found)self.el.hide();
        }
    ...
    stop:function(){
        ...
        this.el=null;this.cues=[];
        if(this.playerListener){
            try{Lampa.Listener.remove('player',this.playerListener);}catch(e){}
            this.playerListener=null;
        }
    }
};

// -- Setup subtitle overlay as fallback --
function setupSubOverlay(subUrl){
    SubOverlay.stop();
    if(!subUrl)return;
    SubOverlay.playerListener=function(e){
        if(e.type==='start'||e.type==='playing'){
            setTimeout(async function(){
                var video=document.querySelector('video');if(!video)return;
                var hasTrack=false;
                if(video.textTracks&&video.textTracks.length){
                    for(var i=0;i<video.textTracks.length;i++){if(video.textTracks[i].mode==='showing'){hasTrack=true;break;}}
                }
                if(!hasTrack&&subUrl){
                    var loaded=await SubOverlay.load(subUrl);
                    if(loaded){SubOverlay.start();Lampa.Noty.show('📝 Subtitle overlay đang chạy');}
                }
            },3000);
        }
...
// -- Smart play with subtitle --
async function smartPlay(playObj,imdbId,season,episode){
    SubOverlay.stop();
    if(!subEnabled()||subMode()==='off'){Lampa.Player.play(playObj);return;}
    ...
    if(!subs.length){
        Lampa.Noty.show('Không tìm thấy subtitle');
        Lampa.Player.play(playObj);
        return;
    }
    
    if(sm==='auto'){
        var best=subs[0],subUrl=best.url;
        if(best._needDownload&&best.fileId){subUrl=await getOsubDownloadUrl(best.fileId);}
        if(subUrl){
            playObj.subtitles=[{label:best.label,url:subUrl,language:best.language}];
            playObj.subtitle=subUrl;playObj.subtitle_label=best.label;
    ...
    subs.forEach(function(sub){items.push({title:sub.label,value:sub});});
    
    Lampa.Select.show({
        title:'📝 Subtitle ('+subs.length+')',
        items:items,
        onSelect:async function(a){
            if(!a.value){Lampa.Player.play(playObj);return;}
            var sub=a.value,subUrl=sub.url;
            if(sub._needDownload&&sub.fileId){
                Lampa.Noty.show('⬇ Tải subtitle...');
                subUrl=await getOsubDownloadUrl(sub.fileId);
            }
            if(subUrl){
                playObj.subtitles=[{label:sub.label,url:subUrl,language:sub.language}];
                playObj.subtitle=subUrl;playObj.subtitle_label=sub.label;
                setupSubOverlay(subUrl);
            }
            Lampa.Noty.show('▶ Phát với subtitle');
            Lampa.Player.play(playObj);
...
// -- Simple play (no IMDB, no sub search) --
function simplePlay(title,url){
...
// ══════════════════════════════════════════════════════════════
// TORRENT STREAMS - with subtitle support
// ══════════════════════════════════════════════════════════════
...
function showStr(streams,title,poster,imdbId,season,episode){
    ...
    Lampa.Select.show({title:eng+': '+title+' ('+streams.length+')'+(ts?' → TS':''),items:streams.slice(0,40).map(function(s){return{title:fmtStream(s),value:s};}),onSelect:function(a){
        ...
        if(ts&&s.infoHash)playTS(s,title,poster,s.fileIdx);
            ...
            if(imdbId&&subEnabled()&&subMode()!=='off')smartPlay(playObj,imdbId,season,episode);
            else Lampa.Player.play(playObj);
...
function mkTC(item){
    ...
    h+='<div class="kk-card-meta">';if(d.year)h+='<span class="kk-card-year">'+E(d.year)+'</span>';
    h+='</div></div></div>';
    var c=$(h);
    bE(c,function(){saveHistory(d);Lampa.Activity.push({url:'',title:d.name||'',component:'kkphim_tmdb_detail',tmdb_id:d.tmdb_id,media_type:d.media_type,page:1});});
    return c;
}
...
function mkTCPF(item){
    var d=tNorm(item);if(!d||!d.tmdb_id)return $('<div></div>');
    var bk=d.backdrop_url||d.poster_url||'',ps=d.poster_url||'';
    var vote=d.vote||'',vNum=Number(vote),vClass=vNum>=7?'high':vNum>=5?'mid':'low';
    var typeLabel=d.media_type==='tv'?'TV Series':'Film';
    var card=$('<div class="kk-pfc selector"></div>');
    card.html('<div class="kk-pfc-bg">'+(bk?'<img src="'+bk+'" loading="lazy">':'<div class="kk-pfc-bg-empty"></div>')+'</div><div class="kk-pfc-overlay"></div><div class="kk-pfc-inner">'+(ps?'<div class="kk-pfc-poster"><img src="'+ps+'" loading="lazy"></div>':'<div class="kk-pfc-poster kk-pfc-poster--empty"></div>')+'<div class="kk-pfc-info"><div class="kk-pfc-badge">'+E(typeLabel)+'</div><div class="kk-pfc-title">'+E(d.name)+'</div>'+(d.origin_name&&d.origin_name!==d.name?'<div class="kk-pfc-origin">'+E(d.origin_name)+'</div>':'')+'<div class="kk-pfc-meta">'+(d.year?'<span class="kk-pfc-year">'+E(d.year)+'</span>':'')+(vote?'<span class="kk-pfc-vote kk-pfc-vote--'+vClass+'">⭐ '+vote+'</span>':'')+'</div></div></div>');
    bE(card,function(){saveHistory(d);Lampa.Activity.push({url:'',title:d.name||'',component:'kkphim_tmdb_detail',tmdb_id:d.tmdb_id,media_type:d.media_type,page:1});});
    return card;
}

// Source cards with TMDB lookup + direct play fallback
function openSourceDirect(item,source){
    var slug=item.slug;if(!slug){Lampa.Noty.show('Không có slug');return;}
    var title=item.name||item.title||'';
    Lampa.Noty.show('Đang tải...');
    fDet(source,slug).then(function(det){
        if(!det||!det.episodes||!det.episodes.length){Lampa.Noty.show('Không có tập phim');return;}
        var totalEps=0;det.episodes.forEach(function(sv){totalEps+=(sv.server_data||[]).length;});
        if(totalEps===0){Lampa.Noty.show('Không có tập phim');return;}
        if(totalEps===1){var ep=gEp1(det.episodes);if(ep){var link=ep.link_m3u8||ep.link_embed||'';if(link)simplePlay(title,link);else Lampa.Noty.show('Không có link');}return;}
...
function showServerSelect(episodes,title){
    Lampa.Select.show({title:'Chọn Server - '+title,items:episodes.map(function(sv){return{title:(sv.server_name||'Server')+' ('+(sv.server_data||[]).length+' tập)',value:sv};}),onSelect:function(a){showEpisodeSelect(a.value,title);},onBack:function(){Lampa.Controller.toggle('content');}});
}

function showEpisodeSelect(serverData,title){
    var eps=serverData.server_data||[];if(!eps.length){Lampa.Noty.show('Không có tập');return;}
    Lampa.Select.show({title:(serverData.server_name||'Server')+' - '+title,items:eps.map(function(ep){var link=ep.link_m3u8||ep.link_embed||'';return{title:(ep.name||'Tập')+(!link?' [N/A]':''),value:ep};}),onSelect:function(a){var ep=a.value,link=ep.link_m3u8||ep.link_embed||'';if(link)simplePlay(title+' - '+(ep.name||''),link);else Lampa.Noty.show('Không có link');},onBack:function(){Lampa.Controller.toggle('content');}});
}
...
function mkSourceCard(item,source){
    ...
    if(p)h+='<img src="'+p+'" loading="lazy">';else h+='<div class="kk-card-noposter"><span>No Poster</span></div>';
    if(item.quality)h+='<div class="kk-card-q">'+E(item.quality)+'</div>';
    h+='<div class="kk-card-ep kk-card-ep--type">'+typeLabel+'</div>';
    h+='</div><div class="kk-card-body"><div class="kk-card-name">'+E(item.name||item.title||'')+'</div>';
    if(item.origin_name&&item.origin_name!==(item.name||item.title))h+='<div class="kk-card-origin">'+E(item.origin_name)+'</div>';
    h+='<div class="kk-card-meta">';if(item.year)h+='<span class="kk-card-year">'+E(item.year)+'</span>';
    h+='</div></div></div>';
    var c=$(h);
    bE(c,async function(){
        if(c.attr('data-loading')==='true')return;c.attr('data-loading','true');
        Lampa.Noty.show('Tìm TMDB...');
        var type=item.type==='series'||item.type==='tvshows'||item.type==='hoathinh'?'tv':'movie';
        var tmdbInfo=await findTmdbId(item,type);
        if(tmdbInfo&&tmdbInfo.id)Lampa.Activity.push({url:'',title:item.name||item.title||'',component:'kkphim_tmdb_detail',tmdb_id:tmdbInfo.id,media_type:tmdbInfo.type,page:1});
        else{Lampa.Noty.show('TMDB không tìm thấy, phát trực tiếp...');openSourceDirect(item,source);}
...
function mkSourceCardH(item,source){
    ...
    if(p)h+='<img src="'+p+'" loading="lazy">';else h+='<div class="kk-card-h-noposter"><span>No Image</span></div>';
    if(item.quality)h+='<div class="kk-card-q">'+E(item.quality)+'</div>';
    h+='<div class="kk-card-ep kk-card-ep--type">'+typeLabel+'</div>';
    h+='</div><div class="kk-card-h-body"><div class="kk-card-name">'+E(item.name||item.title||'')+'</div>';
    if(item.origin_name&&item.origin_name!==(item.name||item.title))h+='<div class="kk-card-origin">'+E(item.origin_name)+'</div>';
    h+='<div class="kk-card-meta">';if(item.year)h+='<span class="kk-card-year">'+E(item.year)+'</span>';
    h+='</div></div></div>';
    var c=$(h);
...
function mkRowList(items,isTmdb,source){
    var cm=cardMode(),rl=$('<div class="kk-row-list"></div>');
    if(cm==='poster')items.forEach(function(i){rl.append(isTmdb?mkTCPF(i):mkSourceCard(i,source));});
    else items.forEach(function(i){rl.append(isTmdb?mkTCH(i):mkSourceCardH(i,source));});
    return rl;
}
...
function gHtml(genres,isTmdb){
    if(!genres||!genres.length)return'';var result='';
    for(var i=0;i<genres.length;i++){var g=genres[i];if(!g)continue;var gname=E(g.name||'');
...
function inCSS(){
    ...
    if(!$('#'+fixId).length){
        $('head').append('<style id="'+fixId+'">.kk-grid--cat-h{display:grid!important;grid-template-columns:repeat('+hgridCols()+',minmax(0,1fr))!important;}@media(orientation:landscape){.kk-grid--cat-h{grid-template-columns:repeat('+hgridCols()+',minmax(0,1fr))!important;}}</style>');
    }
}
...
function mkGridInfinite(name,fetchFn,titleFn){
    Lampa.Component.add(name,function(obj){
        ...
        var wrap=$('<div class="kk-grid-wrap"></div>'),titleEl=$('<div class="kk-grid-title">'+E(titleFn(obj))+'</div>');
        var gridContainer=$('<div></div>'),spinner=$('<div class="kk-row-loading" style="display:none"><div class="kk-row-spinner"></div></div>');
        ...
        this.create=function(){
            comp.activity.loader(true);cScr(scroll);wrap.append(titleEl).append(gridContainer).append(spinner).append(endMsg);scroll.append(wrap);initGrid();
            var sb=scroll.render().find('.scroll__body');sb.on('scroll',function(){if(ld||done)return;var el=sb[0];if(el.scrollTop+el.clientHeight>=el.scrollHeight-400)doL();});doL();
        };
...
// ══════════════════════════════════════════════════════════════
// EPISODE FILL - with subtitle support
// ══════════════════════════════════════════════════════════════

// Global context for current detail page (for subtitle lookup)
var _detCtx={imdbId:null,mediaType:null,tmdbId:null};

function fillE(c,eps,title,seasonNum){
    eps.forEach(function(sv){
        var sn2=sv.server_name||'Server',cnt=(sv.server_data||[]).length;
        var icon='📺',snl=sn2.toLowerCase();
        ...
        (sv.server_data||[]).forEach(function(ep,idx){
            ...
            bE(chip,function(){
                ...
                var playObj={title:title+' - '+(ep.name||''),url:link};
                var imdb=_detCtx.imdbId;
                if(imdb&&subEnabled()&&subMode()!=='off'){
                    var epNum=parseInt(ep.name)||idx+1;
                    smartPlay(playObj,imdb,seasonNum||null,epNum);
                }else{
                    Lampa.Player.play(playObj);
                }
            });
...
function bMovExp(sk,sn,slug,title,css){
    ...
    bE(btn,function(){
        ...
        if(op&&!ld){ld=true;box.html('<div class="kk-ep-ld">⏳ Đang tải...</div>');
        fDet(SOURCES[sk],slug).then(function(det){if(!det||!det.episodes||!det.episodes.length){box.html('<div class="kk-ep-er">❌ Không có tập</div>');return;}box.empty();fillE(box,det.episodes,title,null);}).catch(function(e){box.html('<div class="kk-ep-er">❌ '+E(e.message||'Lỗi')+'</div>');});}
    });
    return w;
}
...
function bTVExp(sk,sn,slug,title,orig,css,tmdbSeasons){
    ...
    bE(btn,function(){
        op=!op;btn.toggleClass('kk-open',op);box.toggleClass('kk-show',op);
        if(op&&!ld){ld=true;box.html('<div class="kk-ep-ld">⏳ Tìm seasons...</div>');
        var source=SOURCES[sk];
        fSeasonSlugs(source,title,orig,null).then(function(entries){
            ...
            if(tmdbSeasons&&tmdbSeasons.length){tmdbSeasons.forEach(function(ts){if(!sMap[ts.season_number]){sMap[ts.season_number]=[{slug:null,name:ts.name,season:ts.season_number,source:source,notFound:true}];if(sNums.indexOf(ts.season_number)===-1)sNums.push(ts.season_number);}});sNums.sort(function(a,b){return a-b;});}
...
function bTorBtn(mt,tid,title,poster,imdb){
    var eng=tEngine(),label=eng==='aio'?'🌊 AIOStreams':'🧲 Torrent';
    if(tsHost())label+=' → TS';
...
function shSn(c,sMap,sNums,title){
    c.empty();sNums.forEach(function(sn){
        var entries=sMap[sn],isNotFound=entries.length===1&&entries[0].notFound;
...
async function ldSn(c,entries,title,sNum,backFn){
    c.html('<div class="kk-ep-ld">⏳ Tải S'+sNum+'...</div>');
    for(var i=0;i<entries.length;i++){
        if(entries[i].notFound)continue;
        try{var det=await fDet(entries[i].source,entries[i].slug);
        if(det&&det.episodes&&det.episodes.length){c.empty();
        if(backFn){var bk=$('<div class="kk-ep-bk selector">← Quay lại</div>');bE(bk,backFn);c.append(bk);}
        fillE(c,det.episodes,title+' S'+pd(sNum),sNum);return;}}catch(e){}
    }
    c.html('<div class="kk-ep-er">❌ Không có tập</div>');
}

// ══════════════════════════════════════════════════════════════
// SETTINGS COMPONENT - with Subtitle settings
// ══════════════════════════════════════════════════════════════
Lampa.Component.add('kkphim_settings',function(){
    ...
    this.create=function(){
        ...
        Object.keys(SOURCES).forEach(function(k){var sc=SOURCES[k],enabled=isSourceEnabled(k);
        gSrc.append(mo(sc.name,'API: '+sc.api,enabled,function(){ss({['source_'+k+'_enabled']:!enabled});Lampa.Noty.show(sc.name+': '+(enabled?'Đã tắt':'Đã bật'));comp.create();}));});
        w.append(gSrc);
        ...
        [{k:'10',n:'10 phim'},{k:'15',n:'15 phim'},{k:'20',n:'20 phim'},{k:'30',n:'30 phim'},{k:'50',n:'50 phim'}].forEach(function(o){gRow.append(mo(o.n,'Số lượng phim mỗi row',rc===o.k,function(){ss({row_count:o.k});comp.create();}));});
        w.append(gRow);
        ...
        gSub.append(mo('Bật subtitle','Tự động tìm phụ đề khi phát',subOn,function(){ss({sub_enabled:!subOn});comp.create();}));
        
        if(subOn){
            // Mode
            var sm=s.sub_mode||'ask';
            [{k:'ask',n:'🔍 Hỏi trước khi phát',d:'Hiện menu chọn subtitle'},{k:'auto',n:'⚡ Tự động',d:'Gắn sub tốt nhất, không hỏi'},{k:'off',n:'❌ Tắt',d:'Không tìm subtitle'}].forEach(function(o){
                gSub.append(mo(o.n,o.d,sm===o.k,function(){ss({sub_mode:o.k});comp.create();}));
            });
            
            // Language
            var sl=s.sub_lang||'vi';
            [{k:'vi',n:'🇻🇳 Tiếng Việt'},{k:'en',n:'🇬🇧 English'},{k:'ja',n:'🇯🇵 日本語'},{k:'ko',n:'🇰🇷 한국어'},{k:'zh',n:'🇨🇳 中文'},{k:'th',n:'🇹🇭 ไทย'}].forEach(function(o){
                gSub.append(mo(o.n,'Ngôn ngữ subtitle',sl===o.k,function(){ss({sub_lang:o.k});comp.create();}));
            });
            
            // Source
            var ssrc=s.sub_source||'subdl';
            gSub.append(mo('Subdl.com','Không cần API key',ssrc==='subdl',function(){ss({sub_source:'subdl'});comp.create();}));
            gSub.append(mo('OpenSubtitles','Cần API key (miễn phí)',ssrc==='opensubtitles',function(){ss({sub_source:'opensubtitles'});comp.create();}));
            
            if(ssrc==='opensubtitles'){
                gSub.append(mi('API Key','Đăng ký tại opensubtitles.com',s.osub_api_key?'••••':'Chưa có','OpenSubtitles API Key','osub_api_key',s));
            }
            
            // Test subtitle
            var stSub=$('<div class="kk-stg-status" style="display:none"></div>');
            var tSub=si2('🧪 Test Subtitle','Thử tìm sub phim Inception','Test');
            bE(tSub,async function(){
                ...
                try{
                    var subs=await findSubtitles('tt1375666','Inception',null,null);
                    if(subs.length){
                        var html='<div class="kk-dbg-header"><span class="kk-dbg-total">✅ Tìm thấy <b>'+subs.length+'</b> subtitle</span></div><div class="kk-dbg-section">';
                        subs.slice(0,5).forEach(function(sub){html+='<div class="kk-dbg-row"><span class="kk-dbg-val">'+E(sub.label)+'</span></div>';});
                        if(subs.length>5)html+='<div class="kk-dbg-row"><span class="kk-dbg-val">... và '+(subs.length-5)+' sub khác</span></div>';
                        html+='</div>';
                        stSub.attr('class','kk-stg-status kk-stg-status--ok').html(html);
                    }else{
                        stSub.attr('class','kk-stg-status kk-stg-status--err').html('<span class="kk-dbg-err">❌ Không tìm thấy subtitle</span>');
                    }
        ...
        bE(tTS,async function(){
            ...
            try{var r=await fetch(tsU('/echo'),{method:'GET',headers:tsH()});
            if(r.ok){var txt=await r.text();stTS.attr('class','kk-stg-status kk-stg-status--ok').html('<span class="kk-dbg-total">✅ Kết nối thành công!</span>');}
            else stTS.attr('class','kk-stg-status kk-stg-status--err').html('<span class="kk-dbg-err">❌ HTTP '+r.status+'</span>');
            }catch(e){stTS.attr('class','kk-stg-status kk-stg-status--err').html('<span class="kk-dbg-err">❌ '+(e.message||'Lỗi')+'</span>');}
        });
        ...
        w.append('<div class="kk-stg-ver">KKPhim v4.1 + Subtitle</div>');
...
Lampa.Component.add('kkphim_person',function(obj){
    ...
    this.create=function(){
        comp.activity.loader(true);cScr(scroll);if(!pid){comp.activity.loader(false);comp.start();return;}
...
Lampa.Component.add('kkphim_person_films',function(obj){
    ...
    var all=[],rd=0,pp=20,gridContainer=$('<div></div>'),curGrid=null;
    ...
    this.create=function(){
        ...
        var sb=scroll.render().find('.scroll__body');sb.on('scroll',function(){if(rd>=all.length)return;var el=sb[0];if(el.scrollTop+el.clientHeight>=el.scrollHeight-400)rMore();});
    ...
    function initGrid(){var cm=catMode();if(cm==='hgrid')curGrid=$('<div class="kk-grid kk-grid--cat-h"></div>');else{curGrid=$('<div class="kk-grid"></div>');curGrid.css('grid-template-columns','repeat('+cardSt()+',minmax(0,1fr))');}gridContainer.empty().append(curGrid);}
    function rMore(){var b=all.slice(rd,rd+pp);if(!b.length){endMsg.show();return;}spinner.show();var cm=catMode();b.forEach(function(c){var card=cm==='hgrid'?mkTCH(c):mkTC(c);curGrid.append(card.addClass('kk-card--grid'));});rd+=b.length;spinner.hide();if(rd>=all.length)endMsg.show();}
    this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){scroll.destroy();};
});
...
Lampa.Component.add('kkphim_main',function(){
    var scroll=new Lampa.Scroll({mask:true,over:true}),comp=this;
    var srcCats=[{name:'🎬 Phim Mới Cập Nhật',api:'danh-sach/phim-moi-cap-nhat'},{name:'🎥 Phim Lẻ Mới',api:'v1/api/danh-sach/phim-le'},{name:'📺 Phim Bộ Mới',api:'v1/api/danh-sach/phim-bo'},{name:'🎭 Hoạt Hình',api:'v1/api/danh-sach/hoat-hinh'}];
    var tmdbSecs=[{name:'🔥 Xu hướng hôm nay',lt:'trending_day'},{name:'🌟 Xu hướng tuần',lt:'trending'},{name:'🎬 Chiếu rạp',lt:'now_playing'},{name:'📅 Sắp chiếu',lt:'upcoming'},{name:'🌟 Phim lẻ phổ biến',lt:'popular_movies'},{name:'📺 Phim bộ phổ biến',lt:'popular_tv'},{name:'📺 Đang phát sóng',lt:'on_the_air'},{name:'⭐ Top phim lẻ',lt:'top_movies'},{name:'⭐ Top phim bộ',lt:'top_tv'}];
    ...
    this.create=function(){
        ...
        bE($(tb.find('.kk-btn')[0]),oTSearch);bE($(tb.find('.kk-btn')[1]),function(){Lampa.Activity.push({url:'',title:'Cài đặt',component:'kkphim_settings'});});
        scroll.append(tb);
        var cm=cardMode(),cnt=cm==='poster'?12:rowCount();var totalSections=0,loadedSections=0;
        function checkDone(){loadedSections++;if(loadedSections>=totalSections){comp.activity.loader(false);comp.start();}}

        var hist=lastHistory();
        if(hist&&hist.tmdb_id){tRec(hist.media_type||'movie',hist.tmdb_id,1).then(function(res){var items=(res.results||[]).slice(0,cnt);if(items.length){items.forEach(function(i){if(!i.media_type)i.media_type=hist.media_type||'movie';});var rowRec=$('<div class="kk-row"></div>'),moreRec=$('<div class="kk-row-more selector">Xem thêm</div>');bE(moreRec,function(){Lampa.Activity.push({url:'',title:'Gợi ý',component:'kkphim_tmdb_list',listType:'trending',page_num:1});});rowRec.append($('<div class="kk-row-head"></div>').append('<div class="kk-row-title">💡 Vì bạn đã xem "'+E(hist.name||'')+'"</div>').append(moreRec));rowRec.append(mkRowList(items,true));scroll.render().find('.kk-topbar').after(rowRec);}}).catch(function(){});}
...
Lampa.Component.add('kkphim_source_list',function(obj){
    var scroll=new Lampa.Scroll({mask:true,over:true}),comp=this;
    var sk=obj.source_key||'ophim',apiPath=obj.api_path||'',source=SOURCES[sk]||SOURCES.ophim;
...
Lampa.Component.add('kkphim_tmdb_genre',function(obj){
    ...
    var gridContainer=$('<div></div>'),ld=false,md=false,td=false,mp=1,tp=1,all=[],rs={},curGrid=null;
    var spinner=$('<div class="kk-row-loading" style="display:none"><div class="kk-row-spinner"></div></div>');
    var endMsg=$('<div class="kk-loadmore" style="display:none;cursor:default;background:transparent">Đã hết</div>');
    function initGrid(){var cm=catMode();if(cm==='hgrid')curGrid=$('<div class="kk-grid kk-grid--cat-h"></div>');else{curGrid=$('<div class="kk-grid"></div>');curGrid.css('grid-template-columns','repeat('+cardSt()+',minmax(0,1fr))');}gridContainer.empty().append(curGrid);}
    this.create=function(){
        comp.activity.loader(true);cScr(scroll);var gb=$('<div class="kk-genre-bar"></div>');scroll.append(gb);initGrid();
        scroll.append($('<div class="kk-grid-wrap"></div>').append('<div class="kk-grid-title" id="kk-gtitle">'+E(obj.title||'')+'</div>').append(gridContainer).append(spinner).append(endMsg));
        var sb2=scroll.render().find('.scroll__body');sb2.on('scroll',function(){if(ld||(md&&td))return;var el=sb2[0];if(el.scrollTop+el.clientHeight>=el.scrollHeight-400)doL();});
    ...
    function doL(){if(ld)return;ld=true;spinner.show();var ps=[];
    ...
    Promise.all(ps).then(function(){all.sort(function(a,b){return(b.popularity||0)-(a.popularity||0);});var cm=catMode();for(var i=0;i<all.length;i++){var key=all[i].media_type+'_'+all[i].id;if(!rs[key]){rs[key]=true;var card=cm==='hgrid'?mkTCH(all[i]):mkTC(all[i]);curGrid.append(card.addClass('kk-card--grid'));}}spinner.hide();if(md&&td)endMsg.show();ld=false;comp.activity.loader(false);comp.start();}).catch(function(){ld=false;spinner.hide();comp.activity.loader(false);});}
    this.start=function(){aCtrl(scroll);eScr(scroll);};this.pause=function(){};this.stop=function(){};this.render=function(){return scroll.render();};this.destroy=function(){scroll.destroy();};
});

// ══════════════════════════════════════════════════════════════
// TMDB DETAIL - with subtitle context
// ══════════════════════════════════════════════════════════════
Lampa.Component.add('kkphim_tmdb_detail',function(obj){
    ...
    this.create=function(){
        ...
        tDetFull(mt,tid).then(async function(tmdb){
            ...
            saveHistory({tmdb_id:tid,media_type:mt,name:t,poster_url:tmdb.poster_path?TMDB_W500+tmdb.poster_path:'',year:y});
            Lampa.Noty.show('Tìm nguồn...');
            var slugs=await fSlugs(t,o,y);
            var tmdbSeasons=null;if(mt==='tv'){try{tmdbSeasons=await gSeasons(tid);}catch(e){}}
            
            // Set subtitle context
            var imdb=(tmdb.external_ids&&tmdb.external_ids.imdb_id)||null;
            if(!imdb)try{imdb=await gImdb(mt,tid);}catch(e){}
            _detCtx={imdbId:imdb,mediaType:mt,tmdbId:tid};
            
            bDet(tmdb,logos,slugs,tmdbSeasons);
        }).catch(function(e){comp.activity.loader(false);Lampa.Noty.show('Lỗi: '+(e.message||''));});
    };
    async function bDet(tmdb,logos,slugs,tmdbSeasons){
        cScr(scroll);
        ...
        var d=tmdb.overview||'Không có mô tả',v=tmdb.vote_average?Number(tmdb.vote_average).toFixed(1):'N/A';
        var y=(tmdb.release_date||tmdb.first_air_date||'').slice(0,4);
        ...
        var epExtra='';if(tmdb.number_of_episodes)epExtra=tmdb.number_of_episodes+' tập';if(tmdb.number_of_seasons)epExtra+=(epExtra?' | ':'')+tmdb.number_of_seasons+' season';
        var country='';if(tmdb.production_countries&&tmdb.production_countries.length)country=tmdb.production_countries.map(function(c){return c.iso_3166_1;}).join(', ');else if(tmdb.origin_country&&tmdb.origin_country.length)country=tmdb.origin_country[0];
        ...
        if(tmdb.credits){castArr=(tmdb.credits.cast||[]).slice(0,15);dirsArr=(tmdb.credits.crew||[]).filter(function(c){return c.job==='Director'||c.job==='Creator'||c.job==='Series Director';}).filter(function(p,i,a){return a.findIndex(function(x){return x.name===p.name;})===i;}).slice(0,8);}
        ...
        var hero=mkHero(bk,ps,logoH,tH,o2,{tagline:tmdb.tagline||'',vote:v,year:y,runtime:rt+(epExtra?' · '+epExtra:''),country:country,genres:genreStr,status:status});
        ...
        Object.keys(enabledSources).forEach(function(sk){
            var sn=enabledSources[sk].name,css=sk==='kkphim'?'kk-src-btn--kkphim':'kk-src-btn--ophim';
            if(slugs[sk]){if(mt==='movie')act.append(bMovExp(sk,sn,slugs[sk],t,css));else act.append(bTVExp(sk,sn,slugs[sk],t,o2,css,tmdbSeasons));}
            else{var naBtn=$('<div class="kk-src-btn kk-src-btn--no selector" style="width:100%"><div class="kk-sb-main">'+E(sn)+' – Không tìm thấy</div><div class="kk-sb-sub">Bấm để thử lại</div></div>');var naWrap=$('<div style="width:100%"></div>').append(naBtn);
            bE(naBtn,async function(){Lampa.Noty.show('Đang tìm lại...');var source=enabledSources[sk];var terms=[t,o2];if(y){terms.push(t+' '+y);terms.push(o2+' '+y);}var cleanT=t.replace(/[:\-–—]/g,' ').replace(/\s+/g,' ').trim();if(cleanT!==t)terms.push(cleanT);for(var i=0;i<terms.length;i++){if(!terms[i])continue;var items=await sSrc(source,terms[i]);var best=mBest(items,t,o2,y);if(best&&best.slug){slugs[sk]=best.slug;Lampa.Noty.show('Tìm thấy!');bDet(tmdb,logos||(tmdb.images||{}),slugs,tmdbSeasons);return;}}Lampa.Noty.show('Vẫn không tìm thấy');});
            act.append(naWrap);}
        });
...
// START
// ══════════════════════════════════════════════════════════════
function startPlugin(){inCSS();applyFontScale();addM();}
