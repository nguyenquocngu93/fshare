(function () {
    ...
    // ===================== NGUONC API ADAPTER =====================
    // NguonC có API khác format, cần adapter

    function isNguonC() { return getSourceKey() === 'nguonc'; }

    function nguoncApiUrl(path, page) {
        var base = SOURCES.nguonc.api;
        // NguonC API format:
        // Danh sách: https://phim.nguonc.com/api/films/danh-sach/phim-moi-cap-nhat?page=1
        // Chi tiết: https://phim.nguonc.com/api/film/{slug}
        // Tìm kiếm: https://phim.nguonc.com/api/films/search?keyword=xxx&page=1
        // Thể loại: https://phim.nguonc.com/api/films/the-loai/{slug}?page=1
        return base + path + (page ? (path.indexOf('?') > -1 ? '&' : '?') + 'page=' + page : '');
    }
    ...
    function nguoncNormalize(item) {
        ...
        return {
            ...
            tmdb: item.tmdb || {},
            category: (item.category || []).map(function (c) {
            ...
            episode_total: item.total_episodes ? String(item.total_episodes) : (item.episode_total || ''),
            type: item.type || ''
        };
    }

    function nguoncParseList(res) {
        // NguonC trả về: { status, items: [...] } hoặc { status, data: { items: [...] } }
        var items = [];
        ...
        return items.map(nguoncNormalize).filter(function (i) { return i && i.slug; });
    }

    function nguoncParseDetail(res) {
        // NguonC detail: { status, movie: {...}, episodes: [...] }
        var movie = res.movie || res.data || res || {};
        var episodes = [];

        // NguonC episodes format khác
        var rawEps = res.episodes || movie.episodes || [];
        if (rawEps.length) {
            episodes = rawEps.map(function (sv) {
                return {
                    server_name: sv.server_name || sv.name || 'Server',
                    server_data: (sv.items || sv.server_data || sv.episodes || []).map(function (ep) {
                        return {
                            name: ep.name || ep.episode || '',
                            slug: ep.slug || '',
                            link_m3u8: ep.link_m3u8 || ep.m3u8 || ep.url || '',
        ...
        return { movie: nguoncNormalize(movie), episodes: episodes };
    }
    ...
    function apiListUrl(catApi, page) {
        if (isNguonC()) {
            // Map category paths
            var nguoncMap = {
                'danh-sach/phim-moi-cap-nhat': 'films/danh-sach/phim-moi-cap-nhat',
                ...
                'v1/api/danh-sach/tv-shows': 'films/danh-sach/tv-shows'
            };
            var mapped = nguoncMap[catApi] || 'films/' + catApi;
            return nguoncApiUrl(mapped, page);
        }
        return SRC_API() + catApi + '?page=' + page;
    }

    function apiDetailUrl(slug) {
        if (isNguonC()) return nguoncApiUrl('film/' + slug);
        return SRC_API() + 'phim/' + slug;
    }

    function apiSearchUrl(keyword, page) {
        if (isNguonC()) return nguoncApiUrl('films/search?keyword=' + encodeURIComponent(keyword), page);
        return SRC_API() + 'v1/api/tim-kiem?keyword=' + encodeURIComponent(keyword) + '&page=' + page;
    }

    function apiCategoryUrl(slug, page) {
        if (isNguonC()) return nguoncApiUrl('films/the-loai/' + slug, page);
        return SRC_API() + 'v1/api/the-loai/' + slug + '?page=' + page;
    }

    function universalParseList(res) {
        if (isNguonC()) return nguoncParseList(res);
        return ((res && res.items) || (res && res.data && res.data.items) || []).map(norm).filter(function (i) { return i && i.slug; });
    }

    function universalParseDetail(res) {
        if (isNguonC()) return nguoncParseDetail(res);
        return { movie: norm(res.movie || res || {}), episodes: res.episodes || [] };
    }
    ...
    async function tsAdd(mag, title, poster) {
        var u = tsUrl('/torrents'); if (!u) throw new Error('TS chưa cấu hình');
    ...
    async function resolveSubUrl(sub) {
        ...
        if (!sub.isZip) return sub.url;
        try {
            await loadJSZip();
            var r = await fetch(sub.url), buf = await r.arrayBuffer(), zip = await JSZip.loadAsync(buf), srtFile = null;
            zip.forEach(function (path, entry) { if (!srtFile && !entry.dir && path.match(/\.(srt|vtt|ass|ssa|sub)$/i)) srtFile = entry; });
            if (srtFile) { var text = await srtFile.async('text'); return URL.createObjectURL(new Blob([text], { type: 'text/plain;charset=utf-8' })); }
        } catch (e) {}
        return sub.url;
    }
    ...
    async function playUrlWithSub(videoUrl, title, movieData, ttype, season, episode) {
        var mode = getSubMode();
        if (mode === 'off') { Lampa.Player.play({ title: title, url: videoUrl }); return; }
        ...
        if (!subs.length) { Lampa.Player.play({ title: title, url: videoUrl }); return; }
        ...
        if (mode === 'auto') {
            ...
            Lampa.Player.play({ title: title, url: videoUrl, subtitles: [{ label: auto.label, url: subUrl, language: auto.language }] });
            Lampa.Noty.show('▶ ' + auto.label);
            return;
        }
        ...
        Lampa.Select.show({
            ...
            onSelect: async function (a) {
                if (!a.value) { Lampa.Player.play({ title: title, url: videoUrl }); return; }
                Lampa.Noty.show('Tải phụ đề...');
                var su = await resolveSubUrl(a.value);
                Lampa.Player.play({ title: title, url: videoUrl, subtitles: [{ label: a.value.label, url: su, language: a.value.language }] });
                Lampa.Noty.show('▶ ' + a.value.label);
            },
            onBack: function () { Lampa.Player.play({ title: title, url: videoUrl }); }
        });
    }
    ...
    async function fetchTio(type, imdbId, s, e) {
        var r = await fetch(tioUrl(type, imdbId, s, e)); if (!r.ok) throw new Error('Tio ' + r.status);
        var d = await r.json();
    ...
    function showTioResults(streams, title, poster, movieData, ttype, season, episode) {
        ...
        Lampa.Select.show({
            ...
            onSelect: function (a) {
                ...
                else if (s.url) playUrlWithSub(s.url, title, movieData, ttype, season, episode);
    ...
    async function openTorrentSearch(tmdbId, ttype, data, episodes, poster) {
        ...
        try {
            ...
            showTioResults(streams, data.name || '', poster, data, ttype, null, null);
    ...
    function pickSeason(seasons, imdb, data, poster) {
        Lampa.Select.show({
            ...
            onSelect: function (a) { if (a.value.episode_count > 0) pickEpFrom(a.value, imdb, data, poster); else promptEp(a.value.season_number, imdb, data, poster); },
            onBack: function () { Lampa.Controller.toggle('content'); }
        });
    }
    ...
    function pickEpFrom(season, imdb, data, poster) {
        ...
        Lampa.Select.show({ title: '📺 ' + season.name, items: items, onSelect: function (a) { doTvSearch(imdb, data, a.value.s, a.value.e, poster); }, onBack: function () { Lampa.Controller.toggle('content'); } });
    }

    function pickEpFB(imdb, data, episodes, poster) {
        var eps = []; if (episodes) episodes.forEach(function (sv) { (sv.server_data || []).forEach(function (ep) { var n = parseInt((ep.name || '').replace(/[^\d]/g, '')) || 0; if (n > 0 && eps.indexOf(n) === -1) eps.push(n); }); });
        eps.sort(function (a, b) { return a - b; });
        if (!eps.length) { promptEp(1, imdb, data, poster); return; }
        Lampa.Select.show({ title: '📺 Chọn tập', items: eps.map(function (n) { return { title: 'S01E' + pad(n), value: { s: 1, e: n } }; }), onSelect: function (a) { doTvSearch(imdb, data, a.value.s, a.value.e, poster); }, onBack: function () { Lampa.Controller.toggle('content'); } });
    }

    function promptEp(ds, imdb, data, poster) {
        try { if (Lampa.Input && Lampa.Input.edit) { Lampa.Input.edit({ title: 'Season:Episode', value: ds + ':1', free: true }, function (v) { var p = String(v || ds + ':1').split(':'); doTvSearch(imdb, data, parseInt(p[0]) || ds, parseInt(p[1]) || 1, poster); }); return; } } catch (e) {}
        var v = window.prompt('Season:Episode', ds + ':1'), p = String(v || ds + ':1').split(':');
        doTvSearch(imdb, data, parseInt(p[0]) || ds, parseInt(p[1]) || 1, poster);
    }

    async function doTvSearch(imdb, data, s, e, poster) {
        var label = (data.name || '') + ' S' + pad(s) + 'E' + pad(e);
        Lampa.Noty.show('Tìm ' + label + '...');
        try { var streams = await fetchTio('tv', imdb, s, e); if (!streams.length) { Lampa.Noty.show('Không có torrent'); return; } showTioResults(streams, label, poster, data, 'tv', s, e); } catch (err) { Lampa.Noty.show('Lỗi: ' + (err.message || '')); }
    }
    ...
    function norm(i) { if (!i) return null; return { name: i.name || i.title || '', origin_name: i.origin_name || '', slug: i.slug || '', poster_url: i.poster_url || i.poster || '', thumb_url: i.thumb_url || i.thumb || '', year: i.year || '', quality: i.quality || '', episode_current: i.episode_current || '', tmdb: i.tmdb || {}, category: i.category || [], director: i.director || '', content: i.content || '', time: i.time || '', episode_total: i.episode_total || '', type: i.type || '' }; }
    ...
    function injectCSS() {
        ...
        $('head').append(`<style id="kk-css">
            .kk-topbar{display:flex;justify-content:space-between;align-items:center;padding:0 1.5em;margin-bottom:.8em;gap:1em;z-index:5}.kk-topbar-title{font-size:2em;font-weight:900;color:#fff;flex:1}.kk-topbar-btns{display:flex;gap:.5em}
            .kk-btn{display:inline-flex;align-items:center;gap:.45em;padding:.8em 1.1em;border-radius:999px;background:linear-gradient(180deg,rgba(255,255,255,.14),rgba(255,255,255,.08));border:1px solid rgba(255,255,255,.10);color:#fff;font-size:.98em;font-weight:800;cursor:pointer}.kk-btn.focus{background:#fff;color:#000}
            .kk-srcbar{display:flex;gap:.5em;padding:0 1.5em .7em;flex-wrap:wrap}
            .kk-srcbtn{padding:.6em 1em;border-radius:.75em;font-size:.95em;font-weight:700;cursor:pointer;border:1px solid rgba(255,255,255,.1)}.kk-srcbtn--on{background:rgba(99,102,241,.25);border-color:rgba(99,102,241,.5);color:#c4b5fd}.kk-srcbtn--off{background:rgba(255,255,255,.06);color:rgba(255,255,255,.45)}.kk-srcbtn.focus{background:rgba(99,102,241,.4);color:#fff}
            .kk-tsbar{padding:0 1.5em .5em}.kk-tsbadge{display:inline-flex;align-items:center;gap:.4em;padding:.45em .85em;border-radius:.65em;background:rgba(74,222,128,.1);border:1px solid rgba(74,222,128,.15);font-size:.88em;color:#4ade80;font-weight:700}
            .kk-row{margin-bottom:1.9em}.kk-row-head{display:flex;justify-content:space-between;align-items:center;padding:0 1.5em;margin-bottom:.85em}.kk-row-title{font-size:1.55em;font-weight:900;color:#fff}
            ...
            .kk-row-list{display:flex;gap:.9em;overflow-x:auto;overflow-y:hidden;padding:0 1.5em .2em;-webkit-overflow-scrolling:touch}.kk-row-list::-webkit-scrollbar,.kk-cast-list::-webkit-scrollbar,.kk-similar-list::-webkit-scrollbar{display:none}
            .kk-card{flex:0 0 auto;width:9.5em;cursor:pointer}.kk-card--grid{width:100%}.kk-card-img{position:relative;width:100%;aspect-ratio:2/3;border-radius:.9em;overflow:hidden;background:#242424}.kk-card-img img{width:100%;height:100%;object-fit:cover}
            .kk-card-q{position:absolute;top:.5em;left:.5em;padding:.2em .5em;border-radius:.4em;font-size:.7em;font-weight:800;background:#f6c344;color:#000}.kk-card-ep{position:absolute;top:.5em;right:.5em;padding:.2em .5em;border-radius:.4em;font-size:.7em;font-weight:800;background:#e53935;color:#fff}
            .kk-card-name{margin-top:.6em;font-size:1em;line-height:1.3;font-weight:700;color:#fff;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.kk-card-year{margin-top:.18em;font-size:.88em;color:rgba(255,255,255,.5)}
            .kk-grid-wrap{padding:0 1.5em}.kk-grid-title{font-size:1.9em;font-weight:900;color:#fff;margin:0 0 .85em}.kk-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.9em}
            ...
            .kk-hero{position:relative;overflow:hidden;background:#111}.kk-hero-bg{position:relative;height:25em}.kk-hero-bg img{width:100%;height:100%;object-fit:cover}
            ...
            .kk-hero-bottom{position:absolute;left:0;right:0;bottom:0;z-index:2;padding:1.4em 1.5em 1.2em}.kk-hero-flex{display:block}.kk-hero-poster{display:none}.kk-hero-info{min-width:0}
            ...
            .kk-title{font-size:2.5em;line-height:1.05;font-weight:900;color:#fff;margin-bottom:.2em}.kk-origin{font-size:1.15em;line-height:1.45;color:rgba(255,255,255,.82)}
            .kk-body{position:relative;z-index:3;padding:1.4em 1.5em 0;background:#141414}
            .kk-metas{display:flex;flex-wrap:wrap;gap:.65em;margin:0 0 1.1em}.kk-meta{padding:.55em .95em;border-radius:.8em;background:rgba(255,255,255,.08);color:#fff;font-size:1.1em;font-weight:800}
            .kk-genres{display:flex;flex-wrap:wrap;gap:.65em;margin:0 0 1.1em}.kk-genre{padding:.55em .95em;border-radius:.8em;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.08);color:rgba(255,255,255,.95);font-size:1.02em;font-weight:700;cursor:pointer}.kk-genre.focus{background:rgba(255,255,255,.18)}
            ...
            .kk-act--torrent{background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff}.kk-act--torrent.focus{background:linear-gradient(135deg,#818cf8,#a78bfa)}
            ...
            .kk-stg-wrap{padding:1.4em}.kk-stg-title{font-size:2em;font-weight:900;color:#fff;margin:0 0 1.3em}.kk-stg-group{margin-bottom:1.7em}.kk-stg-group-title{font-size:1.3em;font-weight:900;color:#fff;margin:0 0 .8em;display:flex;align-items:center;gap:.5em}
            .kk-stg-item{display:flex;align-items:center;gap:.9em;margin-bottom:.7em;padding:1em 1.1em;border-radius:.95em;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.06);cursor:pointer}.kk-stg-item.focus{background:rgba(99,102,241,.2);border-color:rgba(99,102,241,.45)}
            ...
            .kk-stg-status{margin-top:.7em;padding:.85em 1.1em;border-radius:.85em;font-size:.98em;font-weight:700}.kk-stg-status--ok{background:rgba(74,222,128,.12);color:#4ade80}.kk-stg-status--err{background:rgba(248,113,113,.12);color:#f87171}.kk-stg-status--loading{background:rgba(255,255,255,.06);color:rgba(255,255,255,.5)}
    ...
    function mkCard(item) {
        item = isNguonC() ? nguoncNormalize(item) : norm(item);
        if (!item) return $('<div></div>');
        var p = fullImg(item.poster_url || item.thumb_url);
        var c = $('<div class="kk-card selector"><div class="kk-card-img"><img src="' + p + '">' + (item.quality ? '<div class="kk-card-q">' + esc(item.quality) + '</div>' : '') + (item.episode_current ? '<div class="kk-card-ep">' + esc(item.episode_current) + '</div>' : '') + '</div><div class="kk-card-name">' + esc(item.name) + '</div><div class="kk-card-year">' + esc(item.year) + '</div></div>');
    ...
    function startPlugin() {
        ...
        Lampa.Component.add('kkphim_settings', function () {
            ...
            this.create = function () {
                ...
                var pwi = si('Mật khẩu', '', s.torrserver_password ? '••••' : 'Không');
                ...
                [{ k: 'ask', n: 'Hỏi mỗi lần', d: 'Chọn sub trước khi phát' }, { k: 'auto', n: 'Tự động (Việt)', d: 'Ưu tiên tiếng Việt' }, { k: 'off', n: 'Tắt', d: 'Không tìm sub' }].forEach(function (sm) {
                    var it = si(sm.n, sm.d, subM === sm.k ? '✅' : 'Chọn');
                    if (subM === sm.k) it.find('.kk-stg-value').css('color', '#4ade80');
                    bindEnter(it, function () { saveSettings({ sub_mode: sm.k }); Lampa.Noty.show('Sub: ' + sm.n); comp.create(); }); g3.append(it);
                });
        ...
        Lampa.Component.add('kkphim_main', function () {
            ...
            this.create = function () {
                ...
                cats.forEach(function (cat) {
                    var url = apiListUrl(cat.api, 1);
                    network.silent(url, function (res) {
                        var list = universalParseList(res);
                        if (list.length) {
        ...
        Lampa.Component.add('kkphim_category', function (obj) {
            ...
            function hr(res) { var list = universalParseList(res); if (!list.length) { hasMore = false; lm.text('Hết'); comp.activity.loader(false); loading = false; comp.start(); return; } list.forEach(function (i) { grid.append(mkCard(i).addClass('kk-card--grid')); }); page++; loading = false; lm.text('Tải thêm'); comp.activity.loader(false); comp.start(); }
            function doLoad() {
                loading = true; lm.text('Đang tải...');
                var url = (mode === 'category' && catSlug) ? apiCategoryUrl(catSlug, page) : apiListUrl(apiPath, page);
                network.silent(url, hr, function () {
                    // Fallback
                    if (mode === 'category' && catSlug && !isNguonC()) {
                        network.silent(SRC_API() + 'the-loai/' + catSlug + '?page=' + page, hr, function () { loading = false; lm.text('Lỗi'); comp.activity.loader(false); });
        ...
        Lampa.Component.add('kkphim_detail', function (obj) {
            ...
            var movie = isNguonC() ? nguoncNormalize(obj.movie) : norm(obj.movie);
            ...
            this.create = function () {
                ...
                network.silent(apiDetailUrl(movie.slug), function (res) {
                    if (rendered) return;
                    var parsed = universalParseDetail(res);
                    loadAll(parsed.movie, parsed.episodes);
                }, function () { comp.activity.loader(false); Lampa.Noty.show('Lỗi tải phim'); });
            };

            async function loadAll(data, episodes) {
                try {
                    var tid = getTmdbId(data), tt = detectType(data);
            ...
            function build(data, episodes, tmdb, logos, ttype) {
                ...
                if (pCats.length) ghtml = pCats.map(function (g) { return '<span class="kk-genre selector" data-slug="' + esc(g.slug || '') + '" data-title="' + esc(g.name || '') + '">' + esc(g.name || '') + '</span>'; }).join('');
                ...
                var subBtn = hasTmdb ? '<div class="kk-act-wrap"><div class="kk-act kk-act--sub selector">📝 Phụ đề</div></div>' : '';
                ...
                var body = $('<div class="kk-body"><div class="kk-metas"><span class="kk-meta">⭐ ' + esc(v) + '</span>' + (y ? '<span class="kk-meta">📅 ' + esc(y) + '</span>' : '') + (rt ? '<span class="kk-meta">⏱ ' + esc(rt) + '</span>' : '') + (epCur ? '<span class="kk-meta">🎬 ' + esc(epCur) + '</span>' : '') + '</div><div class="kk-genres">' + ghtml + '</div>' + crewH + '<div class="kk-desc">' + fmtTxt(d) + '</div><div class="kk-actions"><div class="kk-act-wrap"><div class="kk-act kk-act--play selector">▶ Xem phim</div></div>' + tBtn + subBtn + '</div></div>');

                bindEnter(body.find('.kk-act--play'), function () {
                    var f = getFirstEp(episodes); if (!f) { Lampa.Noty.show('Không có tập'); return; }
                    var link = f.link_m3u8 || f.link_embed || ''; if (!link) { Lampa.Noty.show('Không có link'); return; }
                    var epNum = parseInt((f.name || '').replace(/[^\d]/g, '')) || null;
                ...
                if (hasTmdb) {
                    ...
                    bindEnter(body.find('.kk-act--sub'), async function () {
                        ...
                        Lampa.Select.show({
                            title: '📝 Phụ đề (' + subs.length + ')',
                            items: sorted.slice(0, 20).map(function (s) { return { title: s.label, value: s }; }),
                    ...
                    episodes.forEach(function (sv) {
                        ...
                        (sv.server_data || []).forEach(function (ep) {
                            ...
                            bindEnter(b, function () {
                                ...
                                playUrlWithSub(link, (data.name || '') + ' - ' + (ep.name || ''), data, ttype, ttype === 'tv' ? 1 : null, epNum);
                            });
                            g.append(b);
                        });
                        ew.append(g);
                ...
                if (cats.length && cats[0].slug) {
                    var simUrl = apiCategoryUrl(cats[0].slug, 1);
                    network.silent(simUrl, function (r) {
                        var list = universalParseList(r).filter(function (i) { return i.slug !== movie.slug; }).slice(0, 12);
                        if (list.length) {
                            var row = $('<div class="kk-section kk-section--last kk-similar"></div>').append('<div class="kk-block-title">Liên quan</div>');
                            var rl = $('<div class="kk-similar-list"></div>'); list.forEach(function (i) { rl.append(mkCard(i)); }); row.append(rl); dw.append(row);
