(function () {
    ...
    function nguoncNormalize(item) {
        ...
        var tmdbData = {};
        try {
            if (item.tmdb && typeof item.tmdb === 'object') tmdbData = item.tmdb;
        } catch (e) {}
    ...
    function nguoncParseList(res) {
        ...
        return items.map(function (i) {
            try { return nguoncNormalize(i); } catch (e) { return null; }
        }).filter(function (i) { return i && i.slug; });
    }
    ...
    function nguoncParseDetail(res) {
        ...
        try {
            ...
            if (!Array.isArray(rawEps)) {
                if (rawEps && typeof rawEps === 'object') rawEps = Object.values(rawEps);
                else rawEps = [];
            }
                ...
                try {
                    ...
                    return {
                        ...
                        server_data: sd.map(function (ep) {
                            return {
                                name: ep.name || ep.episode || ep.title || '',
                                slug: ep.slug || '',
                                link_m3u8: ep.link_m3u8 || ep.m3u8 || ep.url || ep.link || '',
            ...
            console.log('[KKPhim] NguonC detail parse err:', e);
    ...
    function apiListUrl(catApi, page) {
        if (isNguonC()) {
            ...
            var path = m[catApi] || ('films/' + catApi);
            return SOURCES.nguonc.api + path + '?page=' + page;
        }
            ...
            function build(data, episodes, tmdb, logos, ttype) {
                ...
                bindEnter(body.find('.kk-act--sub'), async function () {
                    ...
                    Lampa.Select.show({
                        ...
                        onSelect: function (a) { Lampa.Noty.show('✅ ' + a.value.label + ' - Bấm ▶ để phát với sub này'); Lampa.Controller.toggle('content'); },
                        onBack: function () { Lampa.Controller.toggle('content'); }
                    });
                });
