...
/* FIX 1: Hàm lấy số cột poster ngang - luôn trả về 2, không phụ thuộc cardSt */
...
// ══════════════════════════════════════════════════════════════
// SOURCE SEARCH - IMPROVED with year matching & multi-season
// ══════════════════════════════════════════════════════════════
...
function exSn(name,slug){
    ...
    for(var i=0;i<patterns.length;i++){
        var m=(name+' '+slug).match(patterns[i]);
...
async function fSeasonSlugs(source,title,orig,targetSeason){
    ...
    try{
        ...
        if(orig&&orig!==title)searchTerms.push(orig);
        
        var allItems=[];
        for(var t=0;t<searchTerms.length;t++){
            var items=await sSrc(source,searchTerms[t]);
            allItems=allItems.concat(items);
        }
        
        var seen={};
        allItems=allItems.filter(function(it){
            if(!it.slug||seen[it.slug])return false;
            seen[it.slug]=true;
            return true;
        });
        ...
        for(var i=0;i<allItems.length;i++){
            ...
            if(!match&&results.length>0){
                var bs=nStr(results[0].slug);
                var cs=nStr(it.slug);
                if(cs.indexOf(bs)>-1||bs.indexOf(cs)>-1)match=true;
            }
            
            if(match){
                var sn=exSn(it.name||it.title||'',it.slug||'');
                results.push({
                    slug:it.slug,
        ...
        if(targetSeason){
            var exact=results.filter(function(r){return r.season===targetSeason;});
...
// Find TMDB ID from KKPhim/OPhim item
    ...
    try{
        ...
        for(var i=0;i<results.length;i++){
            ...
            if(nameMatch&&yearMatch){
                return{id:r.id,type:r.media_type||(r.first_air_date?'tv':'movie')};
            }
        }
        
        if(results.length){
            return{id:results[0].id,type:results[0].media_type||(results[0].first_air_date?'tv':'movie')};
        }
    }catch(e){}
    return null;
}
...
function mkTCPF(item){
    ...
    var vote=d.vote||'',vNum=Number(vote);
...
// ══════════════════════════════════════════════════════════════
// FIX 2: Card cho KKPhim/OPhim - fallback phát trực tiếp nếu
//        không tìm được TMDB
// ══════════════════════════════════════════════════════════════

/* Hàm mở phát trực tiếp từ source (không qua TMDB) */
function openSourceDirect(item,source){
    var slug=item.slug;
    if(!slug){Lampa.Noty.show('Không có slug');return;}
    var title=item.name||item.title||'';
    ...
    fDet(source,slug).then(function(det){
        ...
        // Phim bộ hoặc nhiều server → hiện menu chọn
        if(det.episodes.length===1&&det.episodes[0].server_data){
            // Chỉ 1 server → hiện danh sách tập
            showEpisodeSelect(det.episodes[0],title);
        }else{
            // Nhiều server → chọn server trước
            showServerSelect(det.episodes,title);
        }
    }).catch(function(e){
        Lampa.Noty.show('Lỗi: '+(e.message||'Không tải được'));
    });
}

function showServerSelect(episodes,title){
    var items=episodes.map(function(sv){
        var cnt=(sv.server_data||[]).length;
        var sn=sv.server_name||'Server';
        return{title:sn+' ('+cnt+' tập)',value:sv};
    });
    ...
    Lampa.Select.show({
        ...
        items:items,
        onSelect:function(a){
            showEpisodeSelect(a.value,title);
        },
        onBack:function(){
...
function mkSourceCard(item,source){
    ...
    bE(c,async function(){
        ...
        if(tmdbInfo&&tmdbInfo.id){
            ...
            // FIX: Không tìm được TMDB → phát trực tiếp từ source
            Lampa.Noty.show('TMDB không tìm thấy, phát trực tiếp...');
...
function mkCatGrid(items,isTmdb){
    var cm=catMode();
    if(cm==='hgrid'){
        var grid=$('<div class="kk-grid kk-grid--cat-h"></div>');
        items.forEach(function(i){
            var card=isTmdb?mkTCH(i):mkTC(i);
...
function inCSS(){
    ...
    /* FIX 1: Inject CSS fix cho poster ngang luôn 2 cột */
    var fixId='kk-hgrid-fix';
    if(!$('#'+fixId).length){
        var fixCSS=''
        +'.kk-grid--cat-h{'
        +'  display:grid!important;'
        +'  grid-template-columns:repeat('+hgridCols()+',minmax(0,1fr))!important;'
        +'}'
        ...
        +'  }'
        +'}';
        $('head').append('<style id="'+fixId+'">'+fixCSS+'</style>');
    }
}
...
function shSn(c,sMap,sNums,title){
    ...
    sNums.forEach(function(sn){
        ...
        if(!isNotFound){
            bE(item,function(){ldSn(c,entries,title,sn,function(){shSn(c,sMap,sNums,title);});});
        }
        c.append(item);
    });
}
...
// ══════════════════════════════════════════════════════════════
// SETTINGS COMPONENT - FIX 1: Sửa label poster ngang
// ══════════════════════════════════════════════════════════════
Lampa.Component.add('kkphim_settings',function(){
    ...
    this.create=function(){
        ...
        /* FIX 1: Sửa label hiển thị đúng x2 cho poster ngang */
        ...
        /* FIX 1: Sửa label hiển thị đúng x2 */
        ...
        /* FIX 1: Chỉ áp dụng cho poster dọc, ghi rõ */
        ...
        bE(tTS,async function(){
            ...
            try{
                ...
                if(r.ok){
                    var txt=await r.text();var statsHtml='';
...
Lampa.Component.add('kkphim_main',function(){
    ...
    this.create=function(){
        ...
        var totalSections=0,loadedSections=0;
        ...
        if(hist&&hist.tmdb_id){
            tRec(hist.media_type||'movie',hist.tmdb_id,1).then(function(res){
                ...
                if(items.length){
                    ...
                    bE(moreRec,function(){Lampa.Activity.push({url:'',title:'Gợi ý',component:'kkphim_tmdb_list',listType:'trending',page_num:1});});
                    ...
                    scroll.render().find('.kk-topbar').after(rowRec);
                }
            }).catch(function(){});
        }
        ...
        Promise.all([tRand('movie'),tRand('tv')]).then(function(res){
            ...
            randContainer.replaceWith(mkRowList(items.slice(0,cnt),true));
        }).catch(function(){});

        var enabledSources=getEnabledSources();
        var sourceKeys=Object.keys(enabledSources);
            ...
            srcCats.forEach(function(cat){
                ...
                fetch(source.api+cat.api+'?page=1').then(function(r){return r.json();}).then(function(res){
                    ...
                    if(list.length){
                        ...
                        var rowTitle=cat.name+' ('+source.name+')';
                        var mr=$('<div class="kk-row-more selector">Xem thêm</div>');
                        bE(mr,function(){Lampa.Activity.push({url:'',title:rowTitle,component:'kkphim_source_list',source_key:sk,api_path:cat.api,page_num:1});});
...
Lampa.Component.add('kkphim_source_list',function(obj){
    var scroll=new Lampa.Scroll({mask:true,over:true}),comp=this;
    var sk=obj.source_key||'ophim',apiPath=obj.api_path||'';
    ...
    function doL(){
        ...
        fetch(source.api+apiPath+'?page='+page).then(function(r){return r.json();}).then(function(res){
            ...
            list.forEach(function(i){
                var card=cm==='hgrid'?mkSourceCardH(i,source):mkSourceCard(i,source);
                curGrid.append(card.addClass('kk-card--grid'));
            });
...
Lampa.Component.add('kkphim_tmdb_genre',function(obj){
    ...
    function doL(){
        if(ld)return;ld=true;spinner.show();var ps=[];
        ...
        Promise.all(ps).then(function(){
            ...
            for(var i=0;i<all.length;i++){
                var key=all[i].media_type+'_'+all[i].id;
                if(!rs[key]){rs[key]=true;
...
Lampa.Component.add('kkphim_tmdb_detail',function(obj){
    ...
    this.create=function(){
        ...
        tDetFull(mt,tid).then(async function(tmdb){
            ...
            if(mt==='tv'){
                try{tmdbSeasons=await gSeasons(tid);}catch(e){}
            }
    ...
    async function bDet(tmdb,logos,slugs,tmdbSeasons){
        ...
        Object.keys(enabledSources).forEach(function(sk){
            ...
            if(slugs[sk]){
                if(mt==='movie'){
                    act.append(bMovExp(sk,sn,slugs[sk],t,css));
                }else{
                    act.append(bTVExp(sk,sn,slugs[sk],t,o2,css,tmdbSeasons));
                }
            }else{
                ...
                bE(naBtn,async function(){
                    ...
                    var source=enabledSources[sk];
                    var terms=[t,o2];
                    if(y){terms.push(t+' '+y);terms.push(o2+' '+y);}
                    var cleanT=t.replace(/[:\-–—]/g,' ').replace(/\s+/g,' ').trim();
                    if(cleanT!==t)terms.push(cleanT);
                    
                    for(var i=0;i<terms.length;i++){
                        if(!terms[i])continue;
                        var items=await sSrc(source,terms[i]);
                        var best=mBest(items,t,o2,y);
                        if(best&&best.slug){
                            slugs[sk]=best.slug;
                            Lampa.Noty.show('Tìm thấy!');
                            bDet(tmdb,logos||(tmdb.images||{}),slugs,tmdbSeasons);
                            return;
                        }
                    }
        ...
        try{
            ...
            if(simI.length)dw.find('.kk-similar').last().addClass('kk-section--last');
        }
