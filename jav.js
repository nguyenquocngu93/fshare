(function () {
    ...
    function getSubMode() { return loadSettings().sub_mode || 'ask'; } // ask, auto, off

    function fullImg(u) { if (!u) return ''; return u.indexOf('http') === 0 ? u : SRC_IMG() + u; }
    ...
    async function tsAdd(mag, title, poster) {
        ...
        if (!r.ok) throw new Error('TS add:' + r.status); return await r.json();
    }

    async function tsGetFiles(hash) {
        var u = tsUrl('/torrents'); if (!u) throw new Error('TS chưa cấu hình');
        var r = await fetch(u, { method: 'POST', headers: tsHdr(), body: JSON.stringify({ action: 'get', hash: hash }) });
        if (!r.ok) throw new Error('TS get:' + r.status); return await r.json();
    }
            ...
            if (!files.length) { playUrl = tsUrl('/stream/fname?link=' + hash + '&index=0&play'); }
                ...
                Lampa.Select.show({
                    ...
                    onSelect: async function (a) {
                        var pu = tsUrl('/stream/fname?link=' + hash + '&index=' + a.value.id + '&play');
                        await playUrlWithSub(pu, title + ' - ' + (a.value.path || '').split('/').pop(), movieData, ttype, season, episode);
                    },
            ...
            if (r.ok) {
                ...
                if (d.status && d.subtitles && d.subtitles.length) {
                    var results = d.subtitles.map(function (s) {
                        ...
                        return { label: label, url: SUBDL_CDN + s.url, language: lang, release: s.release_name || '', isZip: (s.url || '').endsWith('.zip') };
                    });
    ...
    async function resolveSubUrl(sub) {
        ...
        } catch (e) { console.log('[KKPhim] Unzip sub err:', e); }
        return sub.url;
    }
    ...
    async function playUrlWithSub(videoUrl, title, movieData, ttype, season, episode) {
        ...
        if (mode === 'auto') {
            // Auto: chọn sub Việt đầu tiên, nếu không có thì English
            ...
            Lampa.Player.play({
                title: title,
                url: videoUrl,
                subtitles: [{ label: auto.label, url: subUrl, language: auto.language }]
            });
        ...
        var sorted = subs.slice().sort(function (a, b) {
            var ord = { vi: 0, vietnamese: 0, en: 1, english: 1 };
            return (ord[a.language] !== undefined ? ord[a.language] : 2) - (ord[b.language] !== undefined ? ord[b.language] : 2);
        });

        var items = [{ title: '❌ Không dùng phụ đề', value: null }].concat(
        ...
        Lampa.Select.show({
            ...
            onSelect: async function (a) {
                ...
                Lampa.Noty.show('Đang tải phụ đề...');
                var subUrl = await resolveSubUrl(a.value);

                Lampa.Player.play({
                    title: title,
                    url: videoUrl,
                    subtitles: [{ label: a.value.label, url: subUrl, language: a.value.language }]
                });
    ...
    function pickSeason(seasons, imdb, data, poster) {
        Lampa.Select.show({
            ...
            onSelect: function (a) { if (a.value.episode_count > 0) pickEpFromSeason(a.value, imdb, data, poster); else promptEp(a.value.season_number, imdb, data, poster); },
            onBack: function () { Lampa.Controller.toggle('content'); }
        });
    }
    ...
    async function doTvSearch(imdb, data, s, e, poster) {
        ...
        try {
            ...
            if (!streams.length) { Lampa.Noty.show('Không có torrent'); return; }
            showTioResults(streams, label, poster, data, 'tv', s, e);
    ...
    function parseList(res) { return ((res && res.items) || (res && res.data && res.data.items) || []).map(norm).filter(function (i) { return i && i.slug; }); }
    ...
    function injectCSS() {
        ...
        $('head').append(`<style id="kk-css">
            ...
            .kk-row{margin-bottom:1.9em}.kk-row-head{display:flex;justify-content:space-between;align-items:center;padding:0 1.5em;margin-bottom:.85em;gap:.8em}
            ...
            .kk-similar-list{display:flex;gap:.9em;overflow-x:auto;-webkit-overflow-scrolling:touch}.kk-similar-list .kk-card{width:8.5em}
            .kk-stg-wrap{padding:1.4em}.kk-stg-title{font-size:2em;font-weight:900;color:#fff;margin:0 0 1.3em}
            .kk-stg-group{margin-bottom:1.7em}.kk-stg-group-title{font-size:1.3em;font-weight:900;color:#fff;margin:0 0 .8em;display:flex;align-items:center;gap:.5em}
            ...
            .kk-stg-preview{margin-top:.45em;padding:.65em .85em;border-radius:.65em;background:rgba(255,255,255,.04);font-family:monospace;font-size:.82em;color:rgba(255,255,255,.5);word-break:break-all}
    ...
    function mkCard(item) {
        item = norm(item); if (!item) return $('<div></div>');
    ...
    function startPlugin() {
        ...
        Lampa.Component.add('kkphim_settings', function () {
            ...
            this.create = function () {
                ...
                Object.keys(SOURCES).forEach(function (k) {
                    ...
                    bindEnter(it, function () { saveSettings({ source: k }); Lampa.Noty.show('Đã chọn ' + src.name); comp.create(); });
                    g0.append(it);
                }); w.append(g0);
                ...
                var subMode = s.sub_mode || 'ask';
                var subModes = [
                    { key: 'ask', name: 'Hỏi mỗi lần phát', desc: 'Hiện danh sách sub cho bạn chọn' },
                    { key: 'auto', name: 'Tự động (ưu tiên Việt)', desc: 'Tự chọn sub Việt, không có thì English' },
                    { key: 'off', name: 'Tắt phụ đề', desc: 'Không tìm phụ đề khi phát' }
                ];
                subModes.forEach(function (sm) {
                    var it = si(sm.name, sm.desc, subMode === sm.key ? '✅' : 'Chọn');
                    if (subMode === sm.key) it.find('.kk-stg-value').css('color', '#4ade80');
                    bindEnter(it, function () { saveSettings({ sub_mode: sm.key }); Lampa.Noty.show('Phụ đề: ' + sm.name); comp.create(); });
                    g3.append(it);
                });
                var testSub = si('🧪 Test phụ đề', 'Tìm sub Inception', 'Nhấn');
                var st3 = $('<div class="kk-stg-status" style="display:none"></div>');
                bindEnter(testSub, function () { testSubFn(st3); });
                g3.append(testSub).append(st3);
                w.append(g3);
            ...
            async function testSubFn(el) { el.show().attr('class', 'kk-stg-status kk-stg-status--loading').text('⏳ Đang tìm sub...'); try { var subs = await searchSubs('tt1375666', null, 'movie', null, null, 'Inception'); el.attr('class', 'kk-stg-status ' + (subs.length ? 'kk-stg-status--ok' : 'kk-stg-status--err')).text(subs.length ? '✅ ' + subs.length + ' phụ đề (Inception)' : '❌ Không tìm thấy'); } catch (e) { el.attr('class', 'kk-stg-status kk-stg-status--err').text('❌ ' + (e.message || '')); } }
        ...
        Lampa.Component.add('kkphim_main', function () {
            ...
            this.create = function () {
                ...
                var tb = $('<div class="kk-topbar"><div class="kk-topbar-title">' + esc(src.name) + '</div><div class="kk-topbar-btns"><div class="kk-btn selector">🔍 Tìm</div><div class="kk-btn selector">⚙️</div></div></div>');
                var btns = tb.find('.kk-btn');
                bindEnter($(btns[0]), openSearch);
                bindEnter($(btns[1]), function () { Lampa.Activity.push({ url: '', title: 'Cài đặt', component: 'kkphim_settings' }); });
                scroll.append(tb);
        ...
        Lampa.Component.add('kkphim_detail', function (obj) {
            ...
            var _data = null, _episodes = null, _ttype = 'movie';
            ...
            this.create = function () {
                ...
                network.silent(SRC_API() + 'phim/' + movie.slug, function (res) { if (rendered) return; loadAll(res.movie || res || {}, res.episodes || []); }, function () { comp.activity.loader(false); Lampa.Noty.show('Lỗi tải phim'); });
            };

            async function loadAll(data, episodes) {
                data = norm(data); _data = data; _episodes = episodes;
                try {
                    var tid = getTmdbId(data), tt = detectType(data); _ttype = tt;
            ...
            function build(data, episodes, tmdb, logos, ttype) {
                ...
                var subBtn = hasTmdb ? '<div class="kk-act-wrap"><div class="kk-act kk-act--sub selector">📝 Tìm phụ đề</div></div>' : '';
                ...
                bindEnter(body.find('.kk-act--play'), function () {
                    ...
                    var title = (data.name || movie.name || '') + ' - ' + (f.name || '');
                    var epNum = parseInt((f.name || '').replace(/[^\d]/g, '')) || null;
                    var season = ttype === 'tv' ? 1 : null;
                    playUrlWithSub(link, title, data, ttype, season, epNum);
                });
                ...
                if (hasTmdb) {
                    bindEnter(body.find('.kk-act--sub'), async function () {
                        ...
                        Lampa.Noty.show('Đang tìm phụ đề...');
                        ...
                        Lampa.Select.show({
                            ...
                            onSelect: function (a) {
                                Lampa.Noty.show('✅ Sẽ dùng: ' + a.value.label + '\nBấm ▶ Xem phim để phát với sub này');
                                // Lưu tạm sub đã chọn
                                saveSettings({ _preselected_sub: JSON.stringify(a.value) });
                    ...
                    episodes.forEach(function (sv) {
                        ...
                        (sv.server_data || []).forEach(function (ep) {
                            ...
                            bindEnter(b, function () {
                                ...
                                if (!link) { Lampa.Noty.show('Không có link'); return; }
                                var title = (data.name || movie.name || '') + ' - ' + (ep.name || '');
